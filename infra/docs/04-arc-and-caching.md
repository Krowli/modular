...
- **Scale set** (`modular-kata` release, ns `arc-runners`) - the `AutoscalingRunnerSet`
...
**Scale set** (`modular-kata`), using the runner cache PVC and values file from step 3:
```bash
helm install modular-kata \
  --namespace arc-runners --create-namespace \
  --version 0.14.2 \
  -f arc-modular-values.yaml \
...
# modular-kata  arc-runners   deployed  gha-runner-scale-set-0.14.2             0.14.2
...
# modular-kata-<hash>-listener                 1/1   Running
```

---

## 3. Scale-set values (`arc-modular-values.yaml`)
...
This is the live `arc-modular-values.yaml` verbatim, with only the repo owner/name in
`githubConfigUrl` redacted:

```yaml
githubConfigUrl: "https://github.com/<OWNER>/<REPO>"
githubConfigSecret: arc-github
runnerScaleSetName: modular-kata
minRunners: 0
maxRunners: 10
  ...
  spec:
    ...
    initContainers:
      - name: prepare-runner-cache
        image: modular-kata-runner:2026-06-15-002621
    ...
    containers:
      - name: runner
        ...
        # see /root/modular-kata-runner-image/. IfNotPresent uses the local image.
        image: modular-kata-runner:2026-06-15-002621
        imagePullPolicy: IfNotPresent
...
- **`githubConfigUrl`** - the repo (or org) the scale set serves. Jobs reach it
  with `runs-on: modular-kata`.
- **`githubConfigSecret: arc-github`** - the auth secret from [step 1](#1-github-app-and-the-arc-github-secret).
- **`runnerScaleSetName: modular-kata`** - the runner label. This is the string that
...
1. The **listener** (ns `arc-systems`) long-polls the GitHub Actions service for
   jobs whose `runs-on` matches `modular-kata`.
...
```bash
kubectl -n arc-runners get autoscalingrunnerset modular-kata
...
# default
# modular-kata-gha-rs-no-permission
```
...
Every runner pod gets the `sccache` S3 configuration via `envFrom` ([step 3](#3-scale-set-values-arc-modular-valuesyaml)).
...
`ubuntu-22.04`, so modular-kata only runs trusted `push`/main + release builds (see
...
- `$SCCACHE_BUCKET` set (modular-kata) - sccache reads `SCCACHE_BUCKET/ENDPOINT/REGION`
...
`target/`). On modular-kata the mounted Cargo registry handles crate downloads and
...
wraps `bun install --frozen-lockfile`. On modular-kata, the pod template mounts
...
that runs on `modular-kata`, and a poisoned entry could be consumed by a later
...
- `ci.yml` routes every pull-request job to GitHub-hosted `ubuntu-22.04`
  (`runs-on` resolves to `modular-kata` only for `push`/main, manual dispatch, and
...
modular-kata thus only ever serves trusted `push`/main + release builds. The
...
For the runner PVC, the safe cleanup is simple and coarse: scale `modular-kata` to
...
- **No cluster rights.** Jobs run under `modular-kata-gha-rs-no-permission` with no
...
```bash
kubectl -n arc-runners get autoscalingrunnerset modular-kata   # min/max/current runners
...
**Resize a job's VM** - edit the `resources` block in `arc-modular-values.yaml`
([step 3](#3-scale-set-values-arc-modular-valuesyaml); requests = guaranteed VM size,
limits = hotplug ceiling) and roll out:

```bash
helm upgrade modular-kata \
...
convenience wrapper, `modular-add-repo-runner <OWNER>/<REPO> [label]`, performs exactly
this install.)

**Uninstall** (leaves k3s/Kata in place):

```bash
helm uninstall modular-kata -n arc-runners
