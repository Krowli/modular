<p align="center">
  <img src="https://github.com/Krowli/modular/blob/main/assets/hero.svg?raw=true" alt="modular" width="900">
</p>
...
<p align="center">
  <a href="https://github.com/Krowli/modular/blob/main/packages/coding-agent/CHANGELOG.md"><img src="https://img.shields.io/badge/changelog-keep-E05735?style=flat&colorA=222222" alt="Changelog"></a>
  <a href="https://github.com/Krowli/modular/actions"><img src="https://img.shields.io/github/actions/workflow/status/Krowli/modular/ci.yml?style=flat&colorA=222222&colorB=3FB950" alt="CI"></a>
  <a href="https://github.com/Krowli/modular/blob/main/LICENSE"><img src="https://img.shields.io/github/license/Krowli/modular?style=flat&colorA=222222&colorB=58A6FF" alt="License"></a>
...
```sh
curl -fsSL https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.sh | sh
```

**Windows (PowerShell)**

```powershell
irm https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.ps1 | iex
```

**From source** (Bun ≥ 1.3.14)

```sh
curl -fsSL https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.sh | sh -s -- --source
```
...
`modular` generates its own completion scripts for **bash**, **zsh**, and **fish** from the live command/flag metadata, so they never drift from the actual CLI. Subcommands, flags, and enum values complete statically; model names (`--model`, `--smol`, `--slow`, `--plan`) resolve against the bundled model catalog and `--resume` against your on-disk sessions.

```sh
# zsh — add to ~/.zshrc (or write the output into a file on your $fpath)
eval "$(mod completions zsh)"

# bash — add to ~/.bashrc
eval "$(mod completions bash)"

# fish
mod completions fish > ~/.config/fish/completions/mod.fish
```

## Every tool, _benchmaxxed_.

Edits that land on the first attempt. Reads that summarize files instead of dumping their content. Searches that return instantly. Pick any model — modular will get it right.
...
Originally built on [Modular](https://github.com/modular)'s wonderful [Pi](https://github.com/Krowli/modular), modular adds everything you're missing.
...
/collab puts your live session on a relay and hands back a link — and a QR. A teammate joins from another terminal with modular join, or just opens it in a browser. Share read-write to pair on the same agent, or /collab view for a read-only link anyone can watch but no one can steer. Frames are sealed client-side; the relay never sees your keys.
...
Other agents shell out to rg, grep, find, and bash. On many machines those binaries don't exist, and on the ones where they do, every call costs a fork-exec round-trip. modular links the real implementations into the process. ripgrep, glob, find: in-process. brush is the bash, with sessions that survive across calls. The same modular binary runs on macOS, Linux, and Windows — no WSL bridge.
...
[Full reference →](https://github.com/Krowli/modular/tree/main/docs)
...
- **Custom providers** — Declare anything that speaks `openai-completions`, `openai-responses`, `openai-codex-responses`, `azure-openai-responses`, `anthropic-messages`, `google-generative-ai`, or `google-vertex` in `~/.modular/agent/models.yml`.
...
Full provider & routing reference at [Provider & routing docs](https://github.com/Krowli/modular/tree/main/docs).
...
[`web_search` reference ↗](https://github.com/Krowli/modular/tree/main/docs)
...
Same engine, four wrappers. `mod` runs the TUI. `mod -p` answers a single prompt and exits. The Node SDK embeds the session in your process. `mod --mode rpc` and `mod acp` hand the wheel to another program over stdio.
...
Full reference: [SDK docs](https://github.com/Krowli/modular/tree/main/docs).

## A harness worth keeping is one you _don't_ outgrow.

Pick it up at **[GitHub](https://github.com/Krowli/modular)**.

modular is a coding-first terminal agent — sessions, subagents, slash commands, extensions — all TypeScript, all Apache-2.0, all on [GitHub](https://github.com/Krowli/modular). Shape it from config, hook it from outside, or read the source when you need to.
...
On first run modular inherits whatever is already on disk: rules, skills, and MCP servers from `.claude`, `.cursor`, `.windsurf`, `.gemini`, `.codex`, `.cline`, `.github/copilot`, and `.vscode`. No migration script.

### Extensibility

Ask modular to write the piece you're missing, then `/reload-plugins`. Keep it local, ship it in a `marketplace`, or publish it to npm.

## Philosophy

modular is a terminal coding agent with a batteries-included workflow.
...
`bun setup` installs Bun workspaces and builds `@modular/pi-natives`. Re-run `bun run build:native` after changing Rust crates or `packages/natives`.
...
| Package                                                   | Description                                                                |
| --------------------------------------------------------- | -------------------------------------------------------------------------- |
| **[@modular/collab-web](packages/collab-web)**           | Browser guest client, mock host, and local relay for collab live sessions  |
| **[@modular/pi-ai](packages/ai)**                        | Multi-provider LLM client with streaming and model/provider integration    |
| **[@modular/pi-catalog](packages/catalog)**              | Model catalog: bundled model database, provider descriptors, and identity  |
| **[@modular/pi-agent-core](packages/agent)**             | Agent runtime with tool calling and state management                       |
| **[@modular/pi-coding-agent](packages/coding-agent)**    | Interactive coding agent CLI and SDK                                       |
| **[@modular/pi-tui](packages/tui)**                      | Terminal UI library with differential rendering                            |
| **[@modular/pi-natives](packages/natives)**              | N-API bindings for grep, shell, image, text, syntax highlighting, and more |
| **[@modular/modular-stats](packages/stats)**                 | Local observability dashboard for AI usage statistics                      |
| **[@modular/pi-utils](packages/utils)**                  | Shared utilities (logging, streams, dirs/env/process helpers)              |
| **[@modular/pi-wire](packages/wire)**                    | Shared collab live-session protocol types and relay constants              |
| **[@modular/hashline](packages/hashline)**               | Line-anchored patch language and applier behind the `edit` tool            |
| **[@modular/pi-mnemopi](packages/mnemopi)**              | Local SQLite memory engine for Modular agents                             |
| **[@modular/snapcompact](packages/snapcompact)**         | Bitmap-frame context compression package and SQuAD eval suite              |
| **[@modular/swarm-extension](packages/swarm-extension)** | Swarm orchestration extension package                                      |

### Rust Crates

| Crate                                              | Description                                                                                         |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **[pi-natives](crates/pi-natives)**                | Core Rust native addon (N-API `cdylib`) used by `@modular/pi-natives`; aggregates the crates below |
...
- [GitHub](https://github.com/Krowli/modular)
- [Changelog](https://github.com/Krowli/modular/blob/main/packages/coding-agent/CHANGELOG.md)
- [Apache-2.0](https://github.com/Krowli/modular/blob/main/LICENSE)
