...
mod tests {
	...
	fn bsd_t_prefix_creates_file_in_tmpdir() {
		...
		let (code, stdout, stderr) = run_in(root.clone(), tmpdir_env(&root), vec!["-t", "modular"]);
		assert_eq!(code, 0);
		assert_eq!(stderr, "");
		let printed = PathBuf::from(stdout.trim_end_matches('\n'));
		assert!(printed.is_file(), "printed path {printed:?} must be a regular file");
		...
		assert!(name.starts_with("modular."), "unexpected name {name}");
		assert_eq!(name.len(), "modular.".len() + 10);
	}
