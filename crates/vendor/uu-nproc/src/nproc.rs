...
// as a shell builtin. The MODULAR_NUM_THREADS and MODULAR_THREAD_LIMIT environment
...
fn nproc_main(matches: &ArgMatches) -> UResult<()> {
	...
	// pi-uutils: MODULAR_THREAD_LIMIT comes from the scope environment (the
	// shell's exported variables), not the host process environment.
	let limit = match pi_uutils_ctx::var("MODULAR_THREAD_LIMIT") {
		// Uses the OpenMP variable to limit the number of threads
		// If the parsing fails, returns the max size (so, no impact)
		// If MODULAR_THREAD_LIMIT=0, rejects the value
		Some(threads) => match threads.parse() {
			Ok(0) | Err(_) => usize::MAX,
			Ok(n) => n,
		},
		// the variable 'MODULAR_THREAD_LIMIT' doesn't exist
		// fallback to the max
		None => usize::MAX,
	};
	...
	let mut cores = if matches.get_flag(OPT_ALL) {
		...
		// MODULAR_NUM_THREADS doesn't have an impact on --all
		// pi-uutils: MODULAR_NUM_THREADS comes from the scope environment.
		match pi_uutils_ctx::var("MODULAR_NUM_THREADS") {
			// Uses the OpenMP variable to force the number of threads
			// If the parsing fails, returns the number of CPU
			Some(threads) => {
				// In some cases, MODULAR_NUM_THREADS can be "x,y,z"
				// In this case, only take the first one (like GNU)
				// If MODULAR_NUM_THREADS=0, rejects the value
			...
			// the variable 'MODULAR_NUM_THREADS' doesn't exist
...
mod tests {
	...
	fn scope_env_omp_num_threads_forces_count() {
		let env = HashMap::from([("MODULAR_NUM_THREADS".to_string(), "3".to_string())]);
	...
	fn omp_thread_limit_caps_omp_num_threads() {
		let env = HashMap::from([
			("MODULAR_NUM_THREADS".to_string(), "64".to_string()),
			("MODULAR_THREAD_LIMIT".to_string(), "2".to_string()),
		]);
		let (code, stdout, stderr) = run_in(env, vec![]);
	...
	fn all_prints_positive_integer_and_ignores_omp_num_threads() {
		// --all reports hardware CPUs; MODULAR_NUM_THREADS must not force it.
		let env = HashMap::from([("MODULAR_NUM_THREADS".to_string(), "0".to_string())]);
	...
	fn process_environment_is_not_consulted() {
		...
		unsafe { std::env::set_var("MODULAR_NUM_THREADS", "1234") };
		let (code, stdout, stderr) = run_in(HashMap::new(), vec![]);
		unsafe { std::env::remove_var("MODULAR_NUM_THREADS") };
	...
	fn ignore_subtracts_and_floors_at_one() {
		let env = HashMap::from([("MODULAR_NUM_THREADS".to_string(), "8".to_string())]);
		let (code, stdout, _) = run_in(env, vec!["--ignore=3"]);
		assert_eq!((code, stdout.as_str()), (0, "5\n"));

		let env = HashMap::from([("MODULAR_NUM_THREADS".to_string(), "2".to_string())]);
