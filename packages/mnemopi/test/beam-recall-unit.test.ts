...
import { factRecall, formatContext, recall, recallEnhanced } from "@modular/pi-mnemopi/core/beam/recall";
import { initBeam } from "@modular/pi-mnemopi/core/beam/schema";
import type { BeamMemoryState } from "@modular/pi-mnemopi/core/beam/types";
...
describe("beam recall free functions", () => {
	...
	it("keeps exact working-memory hits above weak matching facts in enhanced recall", async () => {
		...
		insertWorking(
			beam,
			"wm-quasar",
			"MNEMOPI FULL PIPELINE TEST 20260613: The user Verge prefers MODULAR memory to run at full power. Unique entity QuasarOtter owns SignalPineapple and uses RecallEngine-Seven.",
		);
		beam.db.run(
			"INSERT INTO facts (fact_id, session_id, subject, predicate, object, timestamp, confidence) VALUES (?, ?, ?, ?, ?, ?, ?)",
			[
