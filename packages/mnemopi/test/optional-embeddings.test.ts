...
import { getFastembedCacheDir } from "@modular/pi-utils";
import "./setup";
import {
	available,
	embed,
...
} from "@modular/pi-mnemopi/core/embeddings";
import { Mnemopi } from "@modular/pi-mnemopi/core/memory";
import { withMnemopiRuntimeOptions } from "@modular/pi-mnemopi/core/runtime-options";
	...
	it("calls an OpenAI-compatible custom embeddings endpoint without requiring an API key", async () => {
		...
		const server = Bun.serve({
			...
			fetch: async request => {
				...
				expect(request.headers.get("user-agent")).toBe(`Modular/${packageJson.version}`);
				expect(request.headers.get("http-referer")).toBe("https://modularcli.com/");
				expect(request.headers.get("x-openrouter-title")).toBe("Modular");
