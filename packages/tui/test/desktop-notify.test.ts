...
} from "@modular/pi-tui/desktop-notify";
import * as utils from "@modular/pi-utils";
...
describe("buildDesktopNotifyCommand", () => {
	...
	it("produces a freedesktop Notify call for gdbus including the urgency hint byte", () => {
		expect(buildDesktopNotifyCommand(gdbus, { title: "Modular", body: "ping", urgency: "low" })).toEqual([
			"/usr/bin/gdbus",
			"call",
			"--session",
			"--dest",
