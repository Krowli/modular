...
- Fixed large snapcompact archives being reconstructed into unbounded per-request image payloads by adding a frame base64 byte budget and omitting over-budget archive frames from prompt blocks. ([#3792](https://github.com/Krowli/modular/issues/3792))
...
- Added `archiveSourceText(archive)` to extract a persisted frame archive's source text as plain text for LLM summarization. ([#3561](https://github.com/Krowli/modular/pull/3561) by [@serverinspector](https://github.com/serverinspector))
...
- Added package-level TypeScript exports and publication config so consumers can import `@modular/snapcompact` with typed access to snapcompact APIs
- Published `@modular/snapcompact` as the reusable snapcompact compaction package, including bitmap-frame rendering helpers, archive helpers, and the local `snapcompactCompact()` strategy.
