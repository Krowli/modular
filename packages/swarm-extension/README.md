...
Multi-agent orchestration for modular. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full modular subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
modular-swarm path/to/swarm.yaml

# Background — survives terminal close:
nohup modular-swarm path/to/swarm.yaml \
...
Register the extension in your config (`~/.modular/config.json` or `.modular/config.json`):
...
Each agent is a full modular session. It can:
...
Any model configured in modular works. Set a swarm default and optionally override per agent:
...
src/swarm/
  ...
  executor.ts         Spawns agents via modular's runSubprocess
