...
} from "@modular/pi-agent-core/compaction/openai";
import * as ai from "@modular/pi-ai";
import { getOpenAICodexTransportDetails } from "@modular/pi-ai/providers/openai-codex-responses";
import type {
	AssistantMessage,
...
describe("requestRemoteCompaction wire formats", () => {
	...
	test("keeps the generic modular summarizer format for other endpoints", async () => {
		let sentBody: unknown;
		const fetchMock: FetchImpl = async (_input, init) => {
			if (typeof init?.body !== "string") throw new Error("missing remote compaction request body");
			sentBody = JSON.parse(init.body) as unknown;
