...
} from "@modular/pi-agent-core/agent-loop";
import type {
	AgentContext,
	AgentEvent,
	AgentLoopConfig,
	...
	AgentToolContext,
	ToolCallContext,
} from "@modular/pi-agent-core/types";
import type { AssistantMessage, AssistantMessageEvent, Message, ToolResultMessage } from "@modular/pi-ai";
...
declare module "@modular/pi-agent-core/types" {
...
describe("agentLoop kCursorExecResolved (issue #4348)", () => {
	it("skips execute for a toolCall block marked as already run by Cursor's exec channel", async () => {
		const { kCursorExecResolved } = await import("@modular/pi-ai/utils/block-symbols");
