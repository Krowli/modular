...
Stateful agent with tool execution and event streaming. Built on `@modular/pi-ai`.

## Installation

```bash
npm install @modular/pi-agent
```

## Quick Start

```typescript
import { Agent } from "@modular/pi-agent";
...
```typescript
declare module "@modular/pi-agent" {
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@modular/pi-ai`).
...
```typescript
import { Agent, streamProxy } from "@modular/pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@modular/pi-agent";
