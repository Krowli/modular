...
```bash
npm install @modular/pi-ai
```

## Quick Start

```typescript
import { z, getModel, stream, complete, Context, Tool } from "@modular/pi-ai";
...
```typescript
import { z, Tool } from "@modular/pi-ai";
...
```typescript
import { stream, validateToolCall, Tool } from "@modular/pi-ai";
...
```typescript
import * as fs from "node:fs";
import { getModel, complete } from "@modular/pi-ai";
...
```typescript
import { getModel, streamSimple, completeSimple } from "@modular/pi-ai";
...
```typescript
import { getModel, stream } from "@modular/pi-ai";
...
```typescript
import { getProviders, getModels, getModel } from "@modular/pi-ai";
...
```typescript
import { Model, stream } from "@modular/pi-ai";
...
```typescript
import { getModel, complete, Context } from "@modular/pi-ai";
...
```typescript
import { Context, getModel, complete } from "@modular/pi-ai";
...
bundles. Bun's `fetch` does not consume the env var natively, so modular injects
...
```typescript
import { getEnvApiKey } from "@modular/pi-ai";
...
Authenticate via the [`modular`](https://github.com/Krowli/modular) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
modular auth-broker login              # interactive provider selection
modular auth-broker login anthropic    # login to a specific provider
modular auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
modular auth-broker list               # list supported providers
modular auth-broker logout             # interactive — pick a stored credential to remove
```
...
```typescript
import { loginGitHubCopilot } from "@modular/pi-ai";
import * as fs from "node:fs";
...
```typescript
import { getModel, complete, getOAuthApiKey } from "@modular/pi-ai";
import * as fs from "node:fs";
