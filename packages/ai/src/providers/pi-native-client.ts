 ...
 *
 * Dispatches a {@link streamSimple}-shaped request to an `modular auth-gateway`
 ...
 * containerized modular deployments (robodular slots, the swarm extension) that
 * route every LLM call through a credential-holding sidecar so the slot
 * itself stays credential-free.
 */
import { readSseJson } from "@modular/pi-utils";
...
/**
 * Stream a turn through an `modular auth-gateway` over the pi-native protocol.
 *
