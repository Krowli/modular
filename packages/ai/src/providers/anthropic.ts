...
import { isOfficialAnthropicApiUrl } from "@modular/pi-catalog/compat/anthropic";
import { mapEffortToAnthropicAdaptiveEffort } from "@modular/pi-catalog/model-thinking";
import { calculateCost, getBundledModel } from "@modular/pi-catalog/models";
import { isAnthropicOAuthToken } from "@modular/pi-catalog/utils";
import { parseGitHubCopilotApiKey } from "@modular/pi-catalog/wire/github-copilot";
import {
	$env,
	getInstallId,
	isEnoent,
...
const CLAUDE_DEVICE_ID_INSTALL_HASH_DOMAIN = "modular-claude-device-id-v1:";
const CLAUDE_DEVICE_ID_ACCOUNT_HASH_DOMAIN = "modular-claude-device-id-v2";
...
export function maybeAddReplayUnsignedThinkingHint(model: Model<"anthropic-messages">, message: string): string {
	...
	const hint = `Provider "${model.provider}" looks like an Anthropic-compatible signing proxy: it rejected a replayed unsigned thinking block. Set \`compat.replayUnsignedThinking: false\` under \`providers.${model.provider}\` in your models.yml and retry. See https://github.com/Krowli/modular/issues/4297.`;
	return `${hint}\n\n${message}`;
}
...
/**
 * Serialize modular {@link Message}s to Anthropic wire messages.
 *
