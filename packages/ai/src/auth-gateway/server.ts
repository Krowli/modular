/**
 * modular auth-gateway HTTP server.
 *
 ...
 * pure protocol translation: foreign wire → modular Context → pi-ai stream() →
 * modular events → foreign wire.
 *
 * Endpoints:
...
import { extractHttpStatusFromError, extractRetryHint, logger } from "@modular/pi-utils";
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (modular re-stamps every parsed Message). role +
