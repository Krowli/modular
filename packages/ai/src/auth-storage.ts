...
import { getAgentDbPath, logger } from "@modular/pi-utils";
...
export type AuthStorageOptions = {
	 ...
	 *
	 * Examples:
	 * - `"local ~/.modular/agent/agent.db"`
	 * - `"broker http://modular.internal:8765"`
	 */
	sourceLabel?: string;
	/**
...
// `@modular/pi-ai` entrypoint and the in-module call sites below.
...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
	...
	static async open(dbPath: string = getAgentDbPath()): Promise<SqliteAuthCredentialStore> {
		...
		// Concurrent modular startups can race against WAL recovery and the schema
