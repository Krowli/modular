/**
 * HTTP client for the modular auth-broker server.
 *
 * Used by {@link RemoteAuthCredentialStore} (snapshot pulls) and by
 * `modular auth-broker status` (liveness checks). All endpoints except
 * `/v1/healthz` require a bearer token.
 */
import { readSseEvents } from "@modular/pi-utils";
