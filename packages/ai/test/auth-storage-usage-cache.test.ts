...
} from "@modular/pi-ai/auth-storage";
import type { UsageLimit, UsageReport } from "@modular/pi-ai/usage";
import * as claudeUsage from "@modular/pi-ai/usage/claude";
...
describe("AuthStorage usage cache: terminal refresh failure", () => {
	// Usage polling is non-critical: a refresh failure must not disable a
	...
	it("keeps credential and probes with current access after a definitive refresh failure", async () => {
		...
		try {
			...
			// Refresh failed, but the still-valid access token satisfies the probe:
			// the credential stays enabled and the report comes back.
