...
const MODEL = Bun.env.MODULAR_E2E_OPENAI_RESPONSES_MODEL ?? "gpt-5.3-codex";
...
the modular auth-gateway's OpenAI Responses prompt-caching pipeline. The same
...
describe.skipIf(!gateway.ok)("auth-gateway: openai-responses prompt caching e2e", () => {
	...
	it("automatically caches the instructions prefix across two turns", async () => {
		...
		const cacheKey = `modular-e2e-${nonce}`;
