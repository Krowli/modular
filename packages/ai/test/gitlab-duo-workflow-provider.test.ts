...
import { isContextOverflow } from "@modular/pi-ai/error";
import {
...
} from "@modular/pi-ai/providers/gitlab-duo-workflow";
import type {
	AssistantMessage,
	Context,
	FetchImpl,
	...
	Tool,
	ToolResultMessage,
} from "@modular/pi-ai/types";
import { AssistantMessageEventStream } from "@modular/pi-ai/utils/event-stream";
import { buildModel } from "@modular/pi-catalog/build";
import { extractHttpStatusFromError } from "@modular/pi-utils";
...
describe("GitLab Duo Workflow provider protocol", () => {
	...
	it("advertises MODULAR tools under their bare names with the official GitLab MCP schema", () => {
		...
		// bare name MODULAR's own tool docs use.
	...
	it("puts the MODULAR system prompt in the inline flow system slot with reasoning events", () => {
		const systemContext: Context = {
			systemPrompt: ["MODULAR authoritative operating rules. Bridge the local tools."],
		...
		// The system slot carries MODULAR's real system prompt verbatim — no gateway preamble.
		expect(prompt?.prompt_template.system).toContain("MODULAR authoritative operating rules.");
	...
	it("builds startRequest goal as a bare ChatML transcript with tool-run linkage", () => {
		...
		const replayContext: Context = {
			systemPrompt: [`MODULAR system instructions: preserve the local tool bridge. token ${patToken}`],
			messages: [
				{
					role: "user",
		...
		// no <instructions>. The MODULAR system prompt rides the flow config's system slot.
		...
		expect(payload.goal).not.toContain("MODULAR system instructions: preserve the local tool bridge.");
		...
		// rides the very next turn), so the MODULAR-internal call id is omitted from the
		...
		// The MODULAR system prompt lives in the flow config system slot, not the goal.
		const flowPrompt = payload.flowConfig?.prompts[0];
		expect(flowPrompt?.prompt_template.system).toContain(
			"MODULAR system instructions: preserve the local tool bridge.",
		);
	...
	it("strips the MODULAR-internal intent (i) field from replayed tool-call args", () => {
		const replayContext: Context = {
			systemPrompt: ["system"],
			messages: [
...
describe("GitLab Duo Workflow WebSocket state machine", () => {
			...
			channel_values: {
				ui_chat_log: [
					{ message_type: "user", content: "Question" },
					{
						...
						tool_info: { name: "mcp__modular__read", args: { path: "src/index.ts" } },
					},
			...
			channel_values: {
				ui_chat_log: [
					...
					{ message_type: "agent", content: "Found the target. Reading it now." },
					{
						...
						tool_info: { name: "mcp__modular__read", args: { path: "README.md" } },
					},
					{
						message_type: "tool",
						content: "README text",
	...
	it("emits standard tool calls instead of executing GitLab actions in the provider", async () => {
		...
		socket.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					requestID: "req-mcp-1",
					runMCPTool: { name: "mcp__modular__read", args: JSON.stringify({ path: "src/index.ts" }) },
				}),
			}),
		);
	...
	it("finalizes one assistant message per tool-call action (serial MCP dispatch)", async () => {
		...
		socket.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					requestID: "req-a",
					runMCPTool: { name: "mcp__modular__read", args: JSON.stringify({ path: "a.ts" }) },
				}),
			}),
		);
		...
		socket?.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					...
					runMCPTool: { name: "mcp__modular__read", args: JSON.stringify({ path: "README.md" }) },
				}),
			}),
		);
	...
	it("re-seeds a fresh workflow goal with the entire conversation history including prior tool results", async () => {
		...
		// by its tool-result turn); the MODULAR-internal id is omitted from the transcript.
