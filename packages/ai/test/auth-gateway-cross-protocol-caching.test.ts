 ...
 *     → openai-responses parser → modular Context
 ...
 * (override via `MODULAR_E2E_GATEWAY_URL`) AND the bearer token file exists at
 * `~/.modular/auth-gateway.token`.
 *
 * To run: `bun --cwd packages/ai test test/auth-gateway-cross-protocol-caching.test.ts`
 * with the gateway live (`modular auth-gateway serve` or pm2).
 */
...
const MODEL = Bun.env.MODULAR_E2E_ANTHROPIC_MODEL ?? "claude-sonnet-4-5";
...
the modular auth-gateway's cross-protocol prompt-caching pipeline. The request
