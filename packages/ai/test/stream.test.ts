...
import { __resetVertexTokenCache } from "@modular/pi-ai/providers/google-auth";
import { complete, getEnvApiKey, stream } from "@modular/pi-ai/stream";
import type { Api, Context, ImageContent, Model, OptionsForApi, Tool, ToolResultMessage } from "@modular/pi-ai/types";
import { buildModel } from "@modular/pi-catalog/build";
import { getBundledModel } from "@modular/pi-catalog/models";
import { $which } from "@modular/pi-utils";
...
describe("Generate E2E Tests", () => {
	...
	describe("google-vertex env auth", () => {
		...
		it("routes impersonated_service_account ADC through IAM to the Vertex request", async () => {
			...
			const originalApiKey = Bun.env.GOOGLE_CLOUD_API_KEY;
			const originalGac = Bun.env.GOOGLE_APPLICATION_CREDENTIALS;
			const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-vertex-impersonation-"));
