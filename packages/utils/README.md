...
Shared utilities for [modular](https://github.com/Krowli/modular) packages. Zero ceremony, Bun-first.

## Notable modules

| Module | Purpose |
| --- | --- |
| `logger` | Centralized logger writing to `~/.modular/logs/` with rotation (TUI-safe — never stdout) |
| `prompt` | Handlebars-based prompt templating and formatting helpers |
| `dirs` | Path helpers for modular config directories (`~/.modular`, XDG-aware on Linux) |
...
Import from the root barrel or per-module subpaths (`@modular/pi-utils/<module>`).

## Install

```sh
bun add @modular/pi-utils
```
...
- [Monorepo README](https://github.com/Krowli/modular#readme)
