...
import { Process, ProcessStatus } from "@modular/pi-natives";
...
export function getShellConfig(customShellPath?: string): ShellConfig {
	...
	if (customShellPath) {
		...
		throw new Error(
			`Custom shell path not found: ${customShellPath}\nPlease update shellPath in ~/.modular/agent/settings.json`,
		);
	}
	...
	if (process.platform === "win32") {
		...
		throw new Error(
			`No bash shell found. Options:\n` +
				...
				`  3. Set shellPath in ~/.modular/agent/settings.json\n\n` +
