/**
 * Centralized path helpers for modular config directories.
 *
 * Uses PI_CONFIG_DIR (default ".modular") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/modular/. This requires running `modular config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, modular trusts that the migration has been done.
 */
...
/** Config directory name (e.g. ".modular") */
export const CONFIG_DIR_NAME: string = ".modular";
...
const PROFILE_ENV_KEYS = ["MODULAR_PROFILE", "PI_PROFILE"] as const;
 ...
 *
 * Exported so consumers of `@modular/pi-utils/dirs` (CLI bootstrap, tests,
...
export function normalizeProfileName(profile: string | undefined): string | undefined {
	...
	if (
		...
		throw new Error(
			`Invalid MODULAR profile "${profile}". Profile names must match ${PROFILE_NAME_RE.source}, ` +
...
/**
 * Resolve the active profile from the two profile env vars. `MODULAR_PROFILE` is the
 * canonical variable and takes precedence; `PI_PROFILE` is the legacy
 * compatibility fallback, consulted only when `MODULAR_PROFILE` is undefined. An
 * explicitly-empty `MODULAR_PROFILE` therefore selects the default profile rather
 ...
 */
export function resolveProfileEnv(modular: string | undefined, pi: string | undefined): string | undefined {
	return normalizeProfileName(modular !== undefined ? modular : pi);
}

function getProfileFromEnv(): string | undefined {
	return resolveProfileEnv(process.env.MODULAR_PROFILE, process.env.PI_PROFILE);
}

/**
 * Module-load profile resolution. Unlike {@link getProfileFromEnv}, an invalid
 * MODULAR_PROFILE/PI_PROFILE value does NOT throw here — a bad env var must not
 ...
 * user still gets a clean "Invalid MODULAR profile" message.
 */
...
/** Get the config directory name relative to home (e.g. ".modular" or PI_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.PI_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".modular/agent" or PI_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all modular directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/modular/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/modular/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/modular once `modular config init-xdg` has migrated
		...
		// profile could resolve to `~/.modular/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/modular exists yet) and then silently move to
		// `$XDG_*_HOME/modular/profiles/<name>` the moment the base appeared, orphaning
		...
		if ((process.platform === "linux" || process.platform === "darwin") && isDefault) {
			const resolveIf = (envVar: string) => {
				...
				try {
					const appRoot = path.join(value, APP_DISPLAY_NAME);
					if (profile) {
		...
		// XDG flattens the agent/ prefix: ~/.modular/agent/sessions → $XDG_DATA_HOME/modular/sessions
		this.#agentDirs = {
 ...
 * `PI_PROFILE` that was bypassed because `MODULAR_PROFILE` explicitly selected the
 * default profile. Returns `undefined` in those cases so reset falls back to the
 * standard `~/.modular/agent`.
 */
 ...
 * `~/.modular/agent`. Captured at module load — ignoring a profile-derived value
...
/** Get the config root directory (~/.modular). */
...
export function setProfile(profile: string | undefined): void {
	...
	if (activeProfile) {
		...
		process.env.MODULAR_PROFILE = activeProfile;
...
/** Get the agent config directory (~/.modular/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.modular). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.modular/*)
// =============================================================================

/** Get the reports directory (~/.modular/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.modular/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get the path to a dated log file (~/.modular/logs/modular.YYYY-MM-DD.log). */
export function getLogPath(date = new Date()): string {
	return path.join(getLogsDir(), `${APP_NAME}.${date.toISOString().slice(0, 10)}.log`);
}

/**
 * Get the plugins directory (~/.modular/plugins or its XDG equivalent).
 *
...
/** Where npm installs packages (~/.modular/plugins/node_modules). */
export function getPluginsNodeModules(home?: string): string {
	return path.join(getPluginsDir(home), "node_modules");
}

/** Plugin manifest (~/.modular/plugins/package.json). */
export function getPluginsPackageJson(home?: string): string {
	return path.join(getPluginsDir(home), "package.json");
}

/** Plugin lock file (~/.modular/plugins/modular-plugins.lock.json). */
export function getPluginsLockfile(home?: string): string {
	return path.join(getPluginsDir(home), "modular-plugins.lock.json");
}

/** Get the remote mount directory (~/.modular/remote). */
 ...
 * (PR checkout, task isolation) and cleanup (`modular worktree`). A relative value
 * would resolve against whatever cwd happened to launch `modular`, so checkout and
 ...
 * isolation, and `modular worktree` cleanup all read the same base). Driven by the
 * `worktree.base` setting in coding-agent; pass `undefined`/empty to clear and
 * fall back to `MODULAR_WORKTREE_DIR` or the `~/.modular/wt` default.
 *
 ...
 * `MODULAR_WORKTREE_DIR` env var, then the {@link setWorktreesDir} override (the
 * `worktree.base` setting), then the `~/.modular/wt` default. The env var and the
 * override are both `~`-expanded and must be absolute; a relative value is
 * ignored and resolution falls through.
 */
export function getWorktreesDir(): string {
	return (
		resolveWorktreeBase(process.env.MODULAR_WORKTREE_DIR) ?? worktreesDirOverride ?? dirs.rootSubdir("wt", "data")
	);
}

/** Get the SSH control socket directory (~/.modular/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.modular/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.modular/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.modular/agent/python-gateway; XDG default: $XDG_STATE_HOME/modular/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.modular/puppeteer). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.modular/wt/`).
...
/** Get the path to a single worktree directory (~/.modular/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.modular/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.modular/cache/github-cache.db).
 * Honors the `MODULAR_GITHUB_CACHE_DB` env var when set so tests can isolate the
 * cache file without touching the rest of the config root.
 */
export function getGithubCacheDbPath(): string {
	const override = process.env.MODULAR_GITHUB_CACHE_DB;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "github-cache.db"), "cache");
}

/**
 * Get the encrypted auth-broker snapshot cache path (~/.modular/cache/auth-broker-snapshot.enc).
 * Honors the `MODULAR_AUTH_BROKER_SNAPSHOT_CACHE` env var when set so tests and
 * operators can isolate or relocate the cache file.
 */
export function getAuthBrokerSnapshotCachePath(): string {
	const override = process.env.MODULAR_AUTH_BROKER_SNAPSHOT_CACHE;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "auth-broker-snapshot.enc"), "cache");
}

/** Get the local FastEmbed model cache directory (~/.modular/cache/fastembed). */
export function getFastembedCacheDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed"), "cache");
}

/** Get the on-demand fastembed runtime install root (~/.modular/cache/fastembed-runtime). */
export function getFastembedRuntimeDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed-runtime"), "cache");
}

/** Get the natives directory (~/.modular/natives). */
export function getNativesDir(): string {
	return dirs.rootSubdir("natives", "cache");
}

/** Get the stats database path (~/.modular/stats.db). */
export function getStatsDbPath(): string {
	return dirs.rootSubdir("stats.db", "data");
}

/** Get the autoresearch state directory (~/.modular/autoresearch). */
export function getAutoresearchDir(): string {
	return dirs.rootSubdir("autoresearch", "state");
}

/** Get the per-project autoresearch state directory (~/.modular/autoresearch/<encoded-project>). */
export function getAutoresearchProjectDir(encodedProject: string): string {
	return path.join(getAutoresearchDir(), encodedProject);
}

/** Get the per-project autoresearch SQLite database path (~/.modular/autoresearch/<encoded-project>.db). */
export function getAutoresearchDbPath(encodedProject: string): string {
	return path.join(getAutoresearchDir(), `${encodedProject}.db`);
}

/** Get the per-run artifact directory (~/.modular/autoresearch/<encoded-project>/runs/<runId>). */
...
// =============================================================================
// Agent subdirectories (~/.modular/agent/*)
// =============================================================================
...
/** Get the last-seen-changelog-version marker file (~/.modular/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.modular/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.modular/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/modular/cache/document-conversions). */
...
/**
 * Persistent per-install UUID stored at `~/.modular/install-id`.
 *
 ...
 * `~/.modular/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.modular/install-id`) regardless of the
