...
export function parseEnvFile(filePath: string): Record<string, string> {
	...
	// MODULAR_ overrides PI_
	for (const k in result) {
		if (k.startsWith("MODULAR_")) {
			result[`PI_${k.slice("MODULAR_".length)}`] = result[k];
		}
	}
 ...
 *
 * All users should import this env module (import { $env } from "@modular/pi-utils")
