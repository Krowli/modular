 ...
 * - **Dev runs only.** In the compiled `modular` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("modular.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("modular.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("modular.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("modular.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
