...
import { Command, type CommandEntry, Flags, run } from "@modular/pi-utils/cli";
...
describe("run() per-command help", () => {
	// Contract: `modular <cmd> --help` must load only the requested command module.
	...
	it("loads only the requested command", async () => {
		...
		try {
			await run({ bin: "modular", version: "0.0.0", argv: ["good", "--help"], commands });
		} finally {
