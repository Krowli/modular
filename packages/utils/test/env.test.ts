...
import { filterProcessEnv, parseEnvFile } from "@modular/pi-utils/env";
...
describe("parseEnvFile", () => {
	...
	it("mirrors valid MODULAR_ variables to PI_ variables", () => {
		const filePath = writeTempEnv("MODULAR_FEATURE=enabled\nOMP_BAD=before\0after\n");

		expect(parseEnvFile(filePath)).toEqual({
			MODULAR_FEATURE: "enabled",
			PI_FEATURE: "enabled",
		});
	});
});
