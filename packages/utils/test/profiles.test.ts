...
import {
	...
	setAgentDir,
	setProfile,
} from "@modular/pi-utils/dirs";
import { Snowflake } from "@modular/pi-utils/snowflake";
...
describe("profile directories", () => {
	...
	beforeEach(async () => {
		...
		originalOmpProfileEnv = process.env.MODULAR_PROFILE;
		...
		configDir = `.modular-profile-test-${Snowflake.next()}`;
	...
	afterEach(async () => {
		...
		if (originalOmpProfileEnv === undefined) {
			delete process.env.MODULAR_PROFILE;
		} else {
			process.env.MODULAR_PROFILE = originalOmpProfileEnv;
		}
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "modular", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "modular", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "modular", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "modular", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "modular", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "modular", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		// Fresh install: XDG vars are set (typical Linux) but no $XDG/modular exists yet.
		...
		// Later, the base XDG app dir materializes (e.g. via `modular config init-xdg`
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "modular"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "modular"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "modular"), { recursive: true });
	...
	it("rejects path-like profile names", () => {
		expect(() => setProfile("../work")).toThrow("Invalid MODULAR profile");
		expect(() => setProfile("work/team")).toThrow("Invalid MODULAR profile");
	});
	...
	it("does not restore a profile-derived agent dir as the default baseline", () => {
		// Reproduces a child process that inherited MODULAR_PROFILE=work plus the
		...
		// Re-snapshot exactly as module load would, now that MODULAR_PROFILE and the
...
describe("profile env + name validation", () => {
	it("honors MODULAR_PROFILE precedence and treats empty/default as the default profile", () => {
		// MODULAR_PROFILE is canonical and wins over the legacy PI_PROFILE fallback.
		expect(resolveProfileEnv("work", "other")).toBe("work");
		// PI_PROFILE is consulted only when MODULAR_PROFILE is undefined.
		expect(resolveProfileEnv(undefined, "work")).toBe("work");
		// An explicitly-empty MODULAR_PROFILE selects the default profile; it must NOT
	...
	it("rejects uppercase profile names so isolation is filesystem-independent", () => {
		...
		expect(() => normalizeProfileName("WORK")).toThrow("Invalid MODULAR profile");
		expect(() => normalizeProfileName("Work")).toThrow("Invalid MODULAR profile");
...
describe("dirs module import behavior", () => {
	...
	it("exposes worker-host without loading agent env", async () => {
		...
		try {
			const workerHostUrl = import.meta.resolve("@modular/pi-utils/worker-host");
			const agentDir = path.join(root, "agent");
			await fs.mkdir(agentDir, { recursive: true });
			await Bun.write(path.join(agentDir, ".env"), "MODULAR_WORKER_HOST_PROBE=from-agent-env\n");
			const probePath = path.join(root, "probe.ts");
			await Bun.write(
				probePath,
				[
					...
					"process.stdout.write(JSON.stringify({",
					"	envProbe: process.env.MODULAR_WORKER_HOST_PROBE ?? null,",
			...
			delete childEnv.MODULAR_WORKER_HOST_PROBE;
			const proc = Bun.spawn([process.execPath, probePath], {
				stdout: "pipe",
				stderr: "pipe",
				env: childEnv,
	...
	it("ignores inherited profile agent dir when MODULAR_PROFILE explicitly selects default", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "pi-utils-dirs-default-profile-"));
		const probeConfigDir = `.modular-default-profile-${Snowflake.next()}`;
		try {
			const dirsUrl = url.pathToFileURL(path.join(import.meta.dir, "..", "src", "dirs.ts")).href;
			...
			for (const modProfile of ["", "default"]) {
				const probePath = path.join(root, `default-profile-${modProfile || "empty"}.ts`);
				await Bun.write(
					probePath,
					[
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		...
		const profileConfigDir = `.modular-env-xdg-${Snowflake.next()}`;
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "modular", "profiles", "work");
