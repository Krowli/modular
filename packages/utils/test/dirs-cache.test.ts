...
describe("document conversion cache directory", () => {
	...
	beforeEach(async () => {
		...
		originalOmpProfile = process.env.MODULAR_PROFILE;
	...
	afterEach(async () => {
		...
		restoreEnv("MODULAR_PROFILE", originalOmpProfile);
	...
	it("uses XDG_CACHE_HOME for the default agent dir when $XDG_CACHE_HOME/modular exists", async () => {
		...
		expect(getDocumentConversionCacheDir()).toBe(
			path.join(process.env.XDG_CACHE_HOME, "modular", "cache", "document-conversions"),
		);
	});
...
describe("test directory state cleanup", () => {
	it("restores the active profile from the current env after setAgentDir mutations", () => {
		...
		const originalOmpProfile = process.env.MODULAR_PROFILE;
		const originalPiProfile = process.env.PI_PROFILE;
		const originalXdgCacheHome = process.env.XDG_CACHE_HOME;
		try {
			process.env.MODULAR_PROFILE = "cache-profile";
