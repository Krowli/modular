...
import { formatHashlineHeader, InMemorySnapshotStore } from "@modular/hashline";
import type { AgentMessage, ResolvedThinkingLevel, ThinkingLevel } from "@modular/pi-agent-core";
import type { Model, ToolExample } from "@modular/pi-ai";
import { formatSessionDumpText, RpcClient } from "@modular/pi-coding-agent";
import { prompt } from "@modular/pi-utils";
import { diffLines } from "diff";
import { formatDirectory } from "@modular/typescript-edit-benchmark/formatter";
import {
	discoverSharedInfra,
	InProcessClient,
	type SharedInfra,
} from "@modular/typescript-edit-benchmark/in-process-client";
...
import type { EditTask } from "@modular/typescript-edit-benchmark/tasks";
import {
	verifyExpectedFileSubset,
	verifyExpectedFiles,
} from "@modular/typescript-edit-benchmark/verify";

const REPO_ROOT = path.resolve(import.meta.dir, "..", "..", "..", "..");
const RUNS_DIR = path.join(REPO_ROOT, "runs");
const TMP = path.join(RUNS_DIR, `rb-${Math.random().toString(36).slice(2, 10)}`);
const CLI_PATH = Bun.fileURLToPath(import.meta.resolve("@modular/pi-coding-agent/cli"));
