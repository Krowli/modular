 ...
 *
 * Auth: provider API keys resolve through modular's auth storage
 * (~/.modular/agent/agent.db: stored key, OAuth, or env var fallback).
 */

import { parseArgs } from "node:util";
import { type Api, AuthStorage, completeSimple, type Model, SqliteAuthCredentialStore } from "@modular/pi-ai";
import { type GeneratedProvider, getBundledModel } from "@modular/pi-catalog/models";
import { getAgentDbPath } from "@modular/pi-utils";
...
async function openModel(modelSpec: string, storage: AuthStorage): Promise<OpenedModel> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (run \`modular login\` or set the provider env var)`);
	}
