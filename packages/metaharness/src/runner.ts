...
/**
 * Harbor benchmark runner for the local `modular` build.
 *
 * Orchestrates Harbor (`harbor run`) against any Harbor dataset (default
 * terminal-bench-2) using a custom agent (`agent/modular_local.py`) that installs
...
const AGENT_IMPORT_PATH = "modular_local:ModularLocal";

/** Container-side mount points for `--install source` (must match modular_local.py defaults). */
const SOURCE_SRC_MOUNT = "/opt/modular/src";
const SOURCE_BIN_MOUNT = "/opt/modular/bin";
...
/**
 * Resolver injected into Apple Container runs (MODULAR_BENCH_CONTAINER_DNS overrides).
 ...
 */
const CONTAINER_DNS = process.env.MODULAR_BENCH_CONTAINER_DNS || "1.1.1.1";
...
export interface Config {
	...
	/** Extra args forwarded verbatim to the in-container modular CLI invocation (repeatable). */
	agentArgs: string[];
...
const HELP = `metaharness runner (local modular)
...
Model / agent:
  -m, --model <provider/model>   Model (repeatable). Default anthropic/claude-sonnet-4-6
      --agent <name>             modular (default) | oracle | nop | any harbor agent
      --install <source|local|published> modular install mode (default: source).
      ...
      --version <v>              modular version for published install (default: latest)
      --thinking <level>         off|minimal|low|medium|high|xhigh|max

      --tarball <path>           Reuse a prebuilt modular tarball (implies --install local, --no-build)
      --no-build                 Skip packing; reuse newest tarball in bench dir (--install local)
      --agent-arg <arg>          Extra arg forwarded verbatim to the in-container modular CLI (repeatable)
      --env <KEY[=VALUE]>        Forward env into modular container (repeatable).
...
Gateway (auth, no keys in container):
      ...
      --web-search               Enable modular web_search (off by default; can't auth via gateway)
 ...
 * `agent/modular.txt` JSONL. Only bytes appended since the previous call are read
 ...
 */
function probeTrialCost(modLogPath: string): CostProbe | null {
	let size: number;
	try {
		size = fs.statSync(modLogPath).size;
	} catch {
		return costProbes.get(modLogPath) ?? null;
	}
	let probe = costProbes.get(modLogPath);
	if (!probe || size < probe.offset) {
		...
		costProbes.set(modLogPath, probe);
	}
	if (size === probe.offset) return probe;
	let fd: number;
	try {
		fd = fs.openSync(modLogPath, "r");
	} catch {
		return probe;
	}
	try {
...
function parseTrial(dir: string, name: string): Trial | null {
	...
	if (!fs.existsSync(resultPath)) {
		...
		// Realtime cost from the live agent modular.txt log, parsed incrementally.
		const probe = probeTrialCost(path.join(dir, "agent", "modular.txt"));
	...
	costProbes.delete(path.join(dir, "agent", "modular.txt"));
...
function writeReport(st: RenderState, benchDir: string, exitCode: number): string {
	...
	const successPct = tot.done > 0 ? (tot.pass / tot.done) * 100 : 0;
	const lines: string[] = [];
	const isOmp = st.cfg.agent === "modular";
	const argsLabel = agentArgsLabel(st.cfg);
...
function buildTarball(benchDir: string): string {
	process.stdout.write(dim("packing local modular (bun pm pack)…\n"));
...
/** Linux deps tree + mount plan for running modular straight from the mounted repo. */
...
function writeComposeOverlay(benchDir: string, cfg: Config, source: SourceMount | null): string | null {
	...
	const file = path.join(benchDir, "modular-compose-overlay.yaml");
...
function buildHarborArgs(
	...
	if (cfg.agent === "modular") {
		// Config + secrets travel via env (MODULAR_BENCH_*); the agent reads os.environ.
...
/**
 * Env vars injected into the in-container modular run: every host `PI_*` knob (minus
...
export function buildHarborEnv(
	...
	// Drop any stale MODULAR_BENCH_FORWARD_ENV inherited from the caller's shell before
	// the agent-type early return, so it never leaks (incl. into the dry-run dump).
	delete env.MODULAR_BENCH_FORWARD_ENV;
	if (cfg.agent !== "modular") return env;
	const prepend = (k: string, v: string): void => {
		env[k] = env[k] ? `${v}:${env[k]}` : v;
	};
	prepend("PYTHONPATH", AGENT_DIR);
	env.MODULAR_BENCH_INSTALL = cfg.install;
	env.MODULAR_BENCH_VERSION = cfg.version ?? version;
	if (tarball) env.MODULAR_BENCH_TARBALL = tarball;
	if (source) {
		env.MODULAR_BENCH_SOURCE_DIR = SOURCE_SRC_MOUNT;
		env.MODULAR_BENCH_SOURCE_BUN = `${SOURCE_BIN_MOUNT}/bun`;
		env.MODULAR_BENCH_SOURCE_ARCH = source.arch;
	}
	if (cfg.binaryArm64) env.MODULAR_BENCH_BINARY_ARM64 = cfg.binaryArm64;
	if (cfg.binaryX64) env.MODULAR_BENCH_BINARY_X64 = cfg.binaryX64;
	if (cfg.thinking) env.MODULAR_BENCH_THINKING = cfg.thinking;
	if (cfg.agentArgs.length > 0) env.MODULAR_BENCH_AGENT_ARGS = JSON.stringify(cfg.agentArgs);
	if (cfg.webSearch) env.MODULAR_BENCH_WEB_SEARCH = "1";
	env.MODULAR_BENCH_GATEWAY = cfg.gateway ? "1" : "0";
	if (cfg.gateway) {
		env.MODULAR_BENCH_MODELS_YAML = modelsYaml;
		env.MODULAR_BENCH_GATEWAY_URL = cfg.gatewayUrl;
		env.MODULAR_BENCH_GATEWAY_TOKEN = cfg.gatewayToken;
		env.MODULAR_BENCH_GATEWAY_PROVIDERS = deriveProviders(cfg).join(",");
	}
	if (cfg.envType === "apple-container") env.MODULAR_BENCH_CONTAINER_DNS = CONTAINER_DNS;
	const forward = collectForwardEnv(cfg);
	if (Object.keys(forward).length > 0) env.MODULAR_BENCH_FORWARD_ENV = JSON.stringify(forward);
	return env;
}
...
async function runBenchmark(cfg: Config): Promise<BenchmarkRun> {
	...
	if (cfg.agent === "modular" && cfg.envType === "docker" && !which("docker")) {
	...
	if (cfg.agent === "modular" && cfg.install === "local" && !cfg.binaryArm64 && !cfg.binaryX64) {
		if (tarball) {
	...
	if (cfg.agent === "modular" && cfg.install === "source" && !cfg.binaryArm64 && !cfg.binaryX64) {
		source = prepareSourceDeps(cfg);
	}

	// models.yml (gateway)
	let modelsYaml = "";
	if (cfg.agent === "modular" && cfg.gateway) {
		modelsYaml = writeModelsYaml(benchDir, cfg);
		if (!gatewayHealthOk(cfg.gatewayUrl)) {
			process.stderr.write(
				yellow(
					`warning: gateway ${cfg.gatewayUrl} health check failed (continuing). Is the pm2 'modular-auth-gateway' running?\n`,
				),
			);
		}
	}
	...
	if (cfg.dryRun) {
		...
		process.stdout.write(bold("modular env:\n"));
		for (const key in harborEnv) {
			if (key === "MODULAR_BENCH_FORWARD_ENV") continue;
			if (key.startsWith("MODULAR_BENCH_") || key === "PYTHONPATH")
				process.stdout.write(`  ${key}=${harborEnv[key]}\n`);
		}
		if (harborEnv.MODULAR_BENCH_FORWARD_ENV) {
			const parsedForwardEnv: unknown = JSON.parse(harborEnv.MODULAR_BENCH_FORWARD_ENV);
			if (parsedForwardEnv !== null && typeof parsedForwardEnv === "object" && !Array.isArray(parsedForwardEnv)) {
				...
				process.stdout.write(`  MODULAR_BENCH_FORWARD_ENV=${keys.join(",")} (values hidden)\n`);
			}
		}
		process.stdout.write(`\njob dir: ${jobDir}\nbench dir: ${benchDir}\n`);
