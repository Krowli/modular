...
describe("generic agent-arg / env passthrough", () => {
	it("forwards repeated --agent-arg as a JSON array the in-container agent can parse", () => {
		...
		expect(JSON.parse(env.MODULAR_BENCH_AGENT_ARGS ?? "[]")).toEqual(cfg.agentArgs);
	});

	it("omits MODULAR_BENCH_AGENT_ARGS when no --agent-arg was passed", () => {
		const cfg = parseArgs(["--model", "anthropic/claude-opus-4-8"]);
		const env = buildHarborEnv(cfg, "/tmp/models.yml", null, "test");
		expect(env.MODULAR_BENCH_AGENT_ARGS).toBeUndefined();
	});
	...
	it("explicit --providers is authoritative; the default derives from the model", () => {
		...
		expect(new Set(envExplicit.MODULAR_BENCH_GATEWAY_PROVIDERS?.split(","))).toEqual(new Set(["google"]));
		...
		expect(new Set(envDerived.MODULAR_BENCH_GATEWAY_PROVIDERS?.split(","))).toEqual(new Set(["anthropic"]));
	});
...
describe("install modes", () => {
	it("defaults to source mode and publishes the mount contract to the agent", () => {
		...
		expect(env.MODULAR_BENCH_INSTALL).toBe("source");
		expect(env.MODULAR_BENCH_SOURCE_DIR).toBe("/opt/modular/src");
		expect(env.MODULAR_BENCH_SOURCE_BUN).toBe("/opt/modular/bin/bun");
		expect(env.MODULAR_BENCH_SOURCE_ARCH).toBe("arm64");
	});
	...
	it("omits source mount env when no mount was prepared (binary/local runs)", () => {
		...
		const env = buildHarborEnv(cfg, "/tmp/models.yml", "/tmp/modular.tgz", "test");
		expect(env.MODULAR_BENCH_INSTALL).toBe("local");
		expect(env.MODULAR_BENCH_SOURCE_DIR).toBeUndefined();
		expect(env.MODULAR_BENCH_SOURCE_ARCH).toBeUndefined();
	});

	it("--tarball implies a local (tarball) install", () => {
		const cfg = parseArgs(["--model", "anthropic/claude-opus-4-8", "--tarball", "/tmp/modular.tgz"]);
...
describe("live-trial cost probe", () => {
	...
	it("accumulates usage incrementally across appended transcript writes", () => {
		...
		try {
			...
			const log = path.join(agentDir, "modular.txt");
