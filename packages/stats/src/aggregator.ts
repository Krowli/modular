...
// JavaScript entry. Standalone source `modular-stats` keeps using this package's
...
function defaultWorkerCount(): number {
	...
	// the compiled `modular` binary. Keep macOS on the documented serial path.
 ...
 * self-dispatching CLI entry (modular in source, npm-bundle, or compiled form),
 * re-enter that entry with a worker argv selector; otherwise (standalone
 * modular-stats, bun test, SDK embedding) load the worker module directly, so this
 * package keeps zero runtime dependency on `@modular/pi-coding-agent`.
 */
function createSyncWorker(): Worker {
	const hostEntry = workerHostEntry();
	if (hostEntry) {
		return new Worker(hostEntry, { type: "module", argv: ["__modular_worker_stats_sync"] });
	}
 ...
 * then terminates. Used by `modular --smoke-test` so the install-method CI jobs
