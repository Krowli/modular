...
import { getStatsDbPath, isEnoent, logger } from "@modular/pi-utils";
...
const TEMP_PATH_RE = /(?:^|\/)(?:T|tmp|pi-bash-exec|modular-bash-exec|pi-bash-detach)(?:\/|$)|^\/var\/folders(?:\/|$)/;
 ...
 *
 * Rules are generic: modular internal wt paths are dropped; conventional worktree
...
export function normalizeProjectPath(p: string): string | null {
	...
	if (/\/\.modular\/wt\//u.test(clean)) return null;
