...
- Added a Tools tab to the `modular stats` dashboard (`/#/tools`): per-tool call counts, error rates, result/argument payload sizes, per-model breakdown, and a stacked calls-over-time chart. Token and cost columns attribute each invoking turn's real provider usage evenly across that turn's tool calls. Existing databases re-parse sessions once on the next sync to backfill historical tool calls.
...
- Fixed application crashes and Bun aborts on macOS and when parsing large stats session files, including during `modular --smoke-test` runs, by utilizing a more resilient serial parser and lenient line scanner.
...
- Added a Gain tab to the `modular stats` dashboard (`/#/gain`) to display snapcompact token-savings with project scoping from synced session folders.

## [16.1.17] - 2026-06-24

### Fixed

- Stats sync counted the same provider request multiple times when a forked or branched session file copied the parent's entries verbatim. Inserts now skip rows whose `(entry_id, timestamp)` already exists under a different `session_file`, and a one-shot migration on the next `modular stats` run collapses any pre-existing duplicates ([#3370](https://github.com/Krowli/modular/issues/3370)).
...
- Redesigned the local stats dashboard with an MODULAR-themed product shell, dedicated per-section views, accessible loading/empty/error states, and flicker-free navigation between screens and time ranges.
...
- Renamed `__modular_stats_sync_worker` to `__modular_worker_stats_sync`.
...
- Fixed the stats dashboard's SQLite init never setting `PRAGMA busy_timeout`, so a concurrent `modular` startup hitting WAL recovery could crash `initDb()` with `SQLITE_BUSY` instead of waiting through it. The busy handler is now installed before `PRAGMA journal_mode=WAL` ([#2421](https://github.com/Krowli/modular/issues/2421)).
...
- Bundled-model lookups (`getBundledModel`, `GeneratedProvider`) now import from the new `@modular/pi-catalog` package instead of the `@modular/pi-ai` barrel, which no longer re-exports catalog values
- The session-sync worker re-enters the host CLI entry (`workerHostEntry()` + `__modular_stats_sync_worker` argv selector) when running inside modular — source, npm bundle, or compiled binary — and keeps loading its own `sync-worker.ts` module directly for standalone `modular-stats`, bun test, and SDK hosts

## [15.1.6] - 2026-05-19

### Fixed

- Fixed `modular stats` crashing on first session sync in published `modular-{linux,darwin,windows}-*` binaries with `BuildMessage: ModuleNotFound resolving "./packages/stats/src/sync-worker.ts"`; the release build script now lists the stats sync, browser tab, and JS eval workers as explicit `--compile` entrypoints so Bun emits them into bunfs, matching the dev build script and the AGENTS.md worker spawn contract. ([#1150](https://github.com/Krowli/modular/issues/1150))
...
- Fixed `modular stats` in compiled binaries by using the serial sync path instead of spawning a raw file-asset worker that cannot import bundled parser code.
...
- Fixed `modular stats` failing to build the client on globally-installed installs by promoting `tailwindcss` from `devDependencies` to `dependencies` (the client build runs at runtime)
...
- Include subtask session files in usage stats ([#250](https://github.com/Krowli/modular/issues/250))
