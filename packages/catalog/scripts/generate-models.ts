...
import { discoverAuthStorage } from "@modular/pi-ai/auth-broker/discover";
import type { OAuthAccess } from "@modular/pi-ai/auth-storage";
import type { OAuthProvider } from "@modular/pi-ai/oauth/types";
import { getGitLabDuoModels } from "@modular/pi-ai/providers/gitlab-duo";
import { $env } from "@modular/pi-utils";
...
async function fetchAntigravityModels(): Promise<ModelSpec<"google-gemini-cli">[]> {
	...
	if (!access) {
		...
		console.log("Tip: If you are logged in under a specific profile, run with MODULAR_PROFILE=<name>.");
		return [];
	}
	try {
