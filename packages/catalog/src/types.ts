...
// Re-exported from @modular/pi-utils so the whole workspace shares one
// `fetch`-compatible signature (tls-fetch's wrappers produce/accept it).
export type { FetchImpl } from "@modular/pi-utils";
...
export interface Model<TApi extends Api = Api> {
	 ...
	 * than MODULAR's general image pipeline. `stb` local backends reject WebP.
	 */
	 ...
	 * output limit MODULAR cannot discover — sending the wrong value triggers 400s
	 ...
	 * provider client. The `baseUrl` must point at an `modular auth-gateway`
	 * (or compatible) host; `headers.Authorization` (or `apiKey` resolved by
	 * the registry) carries the gateway bearer.
	 *
	 * Used by containerized modular installs (e.g. robodular slots) to route every
