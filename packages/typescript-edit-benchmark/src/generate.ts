#!/usr/bin/env bun
/**
 * Generate edit benchmark cases from TypeScript sources in a modular-style repo.
 *
...
using DEFAULT_SOURCE_REPO_DIR = TempDir.createSync("@modular-source");
...
const DEFAULT_SOURCE_REPO_URL = "https://github.com/Krowli/modular.git";
...
async function ensureSourceRepo(typescriptDir: string): Promise<void> {
	...
	console.log(`Cloning modular repository to ${typescriptDir}…`);
	...
	if (result.exitCode !== 0) {
		...
		throw new Error(`Failed to clone modular: ${stderr.trim()}`);
	}
