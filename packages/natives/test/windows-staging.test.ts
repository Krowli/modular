/**
 * Regression for the Windows `bun install -g` update path: when an `modular`
 * process is running, bun cannot overwrite a locked
 * `node_modules/@modular/pi-natives/native/pi_natives.win32-x64.node` during
 ...
 *      (per-package-version cache under `~/.modular/natives/<version>/`) so the
...
const winNodeModulesNativeDir = "C:\\Users\\Admin\\node_modules\\@modular\\pi-natives\\native";
const winWorkspaceNativeDir = "C:\\Users\\Admin\\dev\\modular\\packages\\natives\\native";
const posixNodeModulesNativeDir = "/home/u/proj/node_modules/@modular/pi-natives/native";
...
describe("windows native addon staging", () => {
	...
	it("prepends versionedDir candidates ahead of node_modules when staging on Windows", () => {
		const versionedDir = "C:\\Users\\Admin\\.modular\\natives\\15.0.1";
		const userDataDir = "C:\\Users\\Admin\\AppData\\Local\\modular";
	...
	it("falls back to the node_modules-only candidate list when staging is off", () => {
		...
		const versionedDir = "/home/u/.modular/natives/15.0.1";
		const candidates = resolveLoaderCandidates({
	...
	it("removes stale version directories after the current native version loads", async () => {
		const nativesDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-natives-cache-"));
		try {
