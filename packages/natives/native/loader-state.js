...
/**
 * Native addon loader for `@modular/pi-natives`.
 *
...
function getNativesDir() {
	...
	if (xdgDataHome && fs.existsSync(path.join(xdgDataHome, "modular"))) {
		return path.join(xdgDataHome, "modular", "natives");
	}
	return path.join(os.homedir(), ".modular", "natives");
}

function resolveLeafPackageDir(platformTag) {
	try {
		const require_ = createRequire(import.meta.url);
		return path.dirname(require_.resolve(`@modular/pi-natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
 ...
 * into the per-version cache directory (`~/.modular/natives/<version>/`) before loading.
 *
 * Windows-only safety net for `bun install -g` updates: when a previous `modular`
 * process is running, bun cannot overwrite the locked `.node` inside
 * `node_modules/@modular/pi-natives/native/`, leaving an old binary next to a
 * newer `index.js` and producing `<sym> is not a function` crashes on the next
 * launch. Staging into the version-pinned cache:
 *   1. Gives every package version its own filesystem path, so concurrent modular
...
function validateLoadedBindings(ctx, bindings, candidate) {
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @modular/pi-natives@${ctx.packageVersion} ` +
 ...
 * deadlocks on some hosts — so it exposes `__modInstallTokioRuntime` for the
...
function installNativeTokioRuntime(bindings) {
	const install = bindings.__modInstallTokioRuntime;
...
function buildHelpMessage(ctx) {
	if (ctx.isCompiledBinary) {
		...
		const downloadHints = ctx.addonFilenames
			.map(filename => {
				const downloadUrl = `https://github.com/Krowli/modular/releases/latest/download/${filename}`;
				const targetPath = path.join(ctx.versionedDir, filename);
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @modular/pi-natives\n" +
...
function initLoaderContext() {
	...
	const userDataDir =
		process.platform === "win32"
			? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "modular")
