# Developing `@modular/pi-coding-agent`

This package is the `modular` CLI. This file is a **developer's map**: where things live
...
| Build the `dist/modular` binary | `bun run build` |
...
and dispatches the hidden `__modular_worker_*` argv selectors before loading the command
...
- [porting-from-modular.md](../../docs/porting-from-modular.md)
