/**
 * Bun `--preload` shim for the modular dev launcher (`scripts/modular`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the modular CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@modular/pi-utils/dirs`).
 */
const launchCwd = process.env.MODULAR_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.MODULAR_LAUNCH_CWD;
	try {
