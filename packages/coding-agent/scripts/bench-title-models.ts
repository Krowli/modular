 ...
 *   bun scripts/bench-title-models.ts --db ~/.modular/agent/history.db --out bench.json
 */
...
function parseArgs(argv: string[]): BenchConfig {
	...
	return {
		dbPath: (get("--db") ?? path.join(os.homedir(), ".modular/agent/history.db")).replace(/^~/, os.homedir()),
