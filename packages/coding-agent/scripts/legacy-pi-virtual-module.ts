...
import { isEnoent } from "@modular/pi-utils/fs-error";

/** Build-time specifier resolved to bundled legacy Pi module namespaces. */
export const LEGACY_PI_MODULES_SPECIFIER = "modular-legacy-pi-modules";

const VIRTUAL_NAMESPACE = "modular-legacy-pi-modules-build";
...
export async function createLegacyPiVirtualModulePlugin(): Promise<Bun.BunPlugin> {
	...
	return {
		name: "modular:legacy-pi-modules",
		setup(build) {
			build.onResolve({ filter: /^modular-legacy-pi-modules$/ }, () => ({
