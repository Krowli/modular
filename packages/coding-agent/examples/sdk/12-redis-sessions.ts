 ...
 * `BlobStore` keep writing to `~/.modular/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@modular/pi-coding-agent";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "modular:sessions:", // optional, this is the default
});
