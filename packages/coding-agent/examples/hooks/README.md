...
Example hooks for modular-coding-agent.

## Usage

```bash
# Load a hook with --hook flag
modular --hook examples/hooks/permission-gate.ts

# Or copy to hooks directory for auto-discovery
cp permission-gate.ts ~/.modular/agent/hooks/
```
...
```typescript
import type { HookAPI } from "@modular/pi-coding-agent/hooks";
