...
import * as mcpClient from "@modular/pi-coding-agent/mcp/client";
import * as oauthFlow from "@modular/pi-coding-agent/mcp/oauth-flow";
import type { MCPServerConfig } from "@modular/pi-coding-agent/mcp/types";
import { MCPCommandController } from "@modular/pi-coding-agent/modes/controllers/mcp-command-controller";
...
describe("/mcp auth commands", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-mcp-reauth-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-mcp-reauth-agent-"));
