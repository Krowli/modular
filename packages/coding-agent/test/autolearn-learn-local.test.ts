...
import {
	...
	saveLearnedLesson,
} from "@modular/pi-coding-agent/memories";
import { localBackend } from "@modular/pi-coding-agent/memory-backend/local-backend";
...
describe("learned-lesson storage (local backend)", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "modular-learned-"));
...
describe("learned-lesson read-back", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "modular-learned-read-"));
		agentDir = path.join(tmp, "agent");
	});
	afterEach(async () => {
		await removeWithRetries(tmp);
...
describe("learn tool (local backend)", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "modular-learn-local-"));
		agentDir = path.join(tmp, "agent");
		projCwd = path.join(tmp, "proj");
		learnedFile = path.join(getMemoryRoot(agentDir, projCwd), "learned.md");
	});
