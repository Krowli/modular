...
import { buildSkillPromptMessage, type Skill } from "@modular/pi-coding-agent/extensibility/skills";
...
async function createSkill(body: string): Promise<{ dir: string; skill: Skill }> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), `modular-skill-prompt-${Snowflake.next()}-`));
