...
import { resetSettingsForTest, Settings, type ShellMinimizerSettings } from "@modular/pi-coding-agent/config/settings";
import { buildMinimizerOptions, executeBash } from "@modular/pi-coding-agent/exec/bash-executor";
import { DEFAULT_MAX_BYTES } from "@modular/pi-coding-agent/session/streaming-output";
import * as shellSnapshot from "@modular/pi-coding-agent/utils/shell-snapshot";
import type { Shell } from "@modular/pi-natives";
import * as piNatives from "@modular/pi-natives";
...
function makeTempDir(): string {
	return fs.mkdtempSync(path.join(os.tmpdir(), "modular-bash-exec-"));
}
...
describe("executeBash", () => {
	...
	it("runs non-bash shellPath commands through the configured shell", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-shellpath-"));
...
`,
	...
	it("uses executable SHELL for user-shell shortcut commands", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-env-shell-"));
...
`,
	...
	it("loads zshrc aliases for user-shell shortcut commands", async () => {
		...
		const shellDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-zsh-shellpath-"));
