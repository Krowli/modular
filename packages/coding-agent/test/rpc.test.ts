...
import type { AgentEvent, AgentMessage } from "@modular/pi-agent-core";
import { type AssistantMessage, Effort, type TextContent } from "@modular/pi-ai";
import {
...
import type { BashExecutionMessage } from "@modular/pi-coding-agent/session/messages";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("RPC mode", () => {
	...
	beforeEach(() => {
		sessionDir = path.join(os.tmpdir(), `modular-rpc-test-${Snowflake.next()}`);
