...
describe("advisor watchdog prompt discovery", () => {
	...
	it("discovers user-level and native project-level watchdog files", async () => {
		...
		tempDirs.push(tempDir);
		const cwd = tempDir.join("project-root");
		const modDir = path.join(cwd, ".modular");
		...
		fs.writeFileSync(path.join(modDir, "WATCHDOG.md"), nativeWatchdogContent, "utf8");
		...
		try {
			...
			// Check ordering: user-level should appear first, then native project level (.modular/WATCHDOG.md has depth 0),
