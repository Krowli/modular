...
describe("global --profile flag", () => {
	...
	beforeEach(() => {
		...
		configDir = `.modular-profile-cli-test-${Snowflake.next()}`;
	...
	it("activates a profile inherited from MODULAR_PROFILE at run time", async () => {
		const writeSpy = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
		setProfile(undefined);
		process.env.MODULAR_PROFILE = "work";
		delete process.env.PI_PROFILE;
	...
	it("installs a shell alias and exits before command dispatch", async () => {
		const installSpy = vi.spyOn(profileAliasCli, "installProfileAlias").mockResolvedValue({
			...
			profile: "work",
			command: "modular --profile=work",
			reloadedWith: ". '/home/me/.bashrc'",
		});
		const outSpy = vi.spyOn(process.stdout, "write").mockImplementation(() => true);

		await runCli(["--profile", "work", "--alias", "modular-work", "--version"]);
		...
		expect(output).toContain("Created modular-work");
	...
	it("installs a shell alias when launch is explicit", async () => {
		...
		await runCli(["launch", "--profile", "work", "--alias", "modular-work", "--version"]);
	...
	it("installs a shell alias when acp is explicit", async () => {
		...
		await runCli(["acp", "--profile", "work", "--alias", "modular-work", "--version"]);
	...
	it("loads profile agent .env before command modules import pi-utils env", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "modular-profile-cli-env-"));
		try {
			const home = path.join(root, "home");
			const configDir = ".modular-profile-cli-env";
			...
			await Bun.write(path.join(defaultAgentDir, ".env"), "MODULAR_PROFILE_BOOTSTRAP_SENTINEL=default\n");
			await Bun.write(path.join(profileAgentDir, ".env"), "MODULAR_PROFILE_BOOTSTRAP_SENTINEL=work\n");
			...
			await Bun.write(
				...
				[
					...
					'process.stdout.write("\\nSENTINEL=" + (Bun.env.MODULAR_PROFILE_BOOTSTRAP_SENTINEL ?? ""));',
				].join("\n"),
			);
			...
			delete childEnv.MODULAR_PROFILE;
			delete childEnv.PI_PROFILE;
			delete childEnv.PI_CODING_AGENT_DIR;
			delete childEnv.MODULAR_PROFILE_BOOTSTRAP_SENTINEL;
	...
	it("surfaces an invalid MODULAR_PROFILE env as a clean error, not an import crash", async () => {
		const root = await fs.mkdtemp(path.join(os.tmpdir(), "modular-profile-cli-env-bad-"));
		try {
			const home = path.join(root, "home");
			...
			const childEnv: Record<string, string | undefined> = {
				...process.env,
				HOME: home,
				PI_CONFIG_DIR: ".modular-profile-cli-env-bad",
				MODULAR_PROFILE: "..",
				NO_COLOR: "1",
			};
			delete childEnv.PI_PROFILE;
			...
			expect(stderr).toContain("Invalid MODULAR profile");
