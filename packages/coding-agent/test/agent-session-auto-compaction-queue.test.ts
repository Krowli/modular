...
import * as unexpectedStopClassifier from "@modular/pi-coding-agent/session/unexpected-stop-classifier";
import { getProjectAgentDir, TempDir, withTimeout } from "@modular/pi-utils";
import * as logger from "@modular/pi-utils/logger";

const runtimeSignalStoreKey = "__modRuntimeSignals";
	...
	beforeEach(async () => {
		...
		fs.writeFileSync(
			...
			[
				...
				"\t\tconst gate = globalThis.__modManualCompactGate;",
				"\t\tif (gate) await gate;",
				"\t\treturn {",
				"\t\t\tcompaction: {",
				'\t\t\t\tsummary: "compacted",',
	...
	afterEach(async () => {
		try {
			...
			try {
				...
				getRuntimeSignals().length = 0;
				(globalThis as typeof globalThis & { __modManualCompactGate?: Promise<void> }).__modManualCompactGate =
					undefined;
				vi.restoreAllMocks();
			}
		}
