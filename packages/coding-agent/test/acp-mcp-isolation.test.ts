 ...
 *
 * `modular acp` must not auto-discover host `.mcp.json` servers when creating a
...
import { createAcpSessionFactory } from "@modular/pi-coding-agent/main";
...
describe("createAcpSessionFactory TITLE_SYSTEM.md per-cwd resolution (PR #3736)", () => {
	it("re-resolves the title prompt for the per-session cwd instead of inheriting the launch cwd's override", async () => {
		...
		try {
			...
			await Bun.write(`${projectDir}/.modular/TITLE_SYSTEM.md`, "Project-specific title policy.");
