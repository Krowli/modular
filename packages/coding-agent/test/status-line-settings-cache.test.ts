...
import { StatusLineComponent, type StatusLineSettings } from "@modular/pi-coding-agent/modes/components/status-line";
import { STATUS_LINE_PRESETS } from "@modular/pi-coding-agent/modes/components/status-line/presets";
import { initTheme } from "@modular/pi-coding-agent/modes/theme/theme";
import * as git from "@modular/pi-coding-agent/utils/git";
import { removeSyncWithRetries, setProjectDir } from "@modular/pi-utils";
...
beforeEach(async () => {
	...
	projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-status-line-settings-cache-"));
