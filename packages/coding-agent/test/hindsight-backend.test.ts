...
import { hindsightBackend, reloadMentalModelsForSession } from "@modular/pi-coding-agent/hindsight/backend";
import { HindsightApi } from "@modular/pi-coding-agent/hindsight/client";
import type { HindsightSessionState } from "@modular/pi-coding-agent/hindsight/state";
import type { AgentSessionEventListener } from "@modular/pi-coding-agent/session/agent-session";
...
describe("hindsightBackend live bank routing", () => {
	...
	// kept landing in the stale bank ("modular") instead of the new one
	...
	it("rebuilds the primary state when hindsight.bankId changes mid-session", async () => {
		...
		settings.set("hindsight.bankId", "modular");
		...
		expect(initial?.bankId).toBe("modular");
	...
	it("rebuilds the primary state when hindsight.scoping changes mid-session", async () => {
		...
		const next = session.getHindsightSessionState();
		expect(next).toBeDefined();
		expect(next?.bankId).toBe("modular-proj");
		expect(next).not.toBe(initial);
	});
	...
	it("does not rebuild when the bank-routing setting is rewritten with the same value", async () => {
		...
		const initial = session.getHindsightSessionState();
		settings.set("hindsight.bankId", "modular"); // unchanged
		await Bun.sleep(0);
	...
	it("rebuilds when hindsight.bankId is reset to blank after a non-empty value", async () => {
		...
		expect(next).toBeDefined();
		expect(next).not.toBe(initial);
		// With scoping=per-project the base falls back to the default ("modular"),
		// so the reset bank id picks up the project suffix from cwd.
		expect(next?.bankId).toBe("modular-_NEW_XenGameKit");

		next!.enqueueRetain("post-reset fact", "reset routing");
		await next!.flushRetainQueue();

		expect(retainBatchSpy).toHaveBeenCalledTimes(1);
		expect(retainBatchSpy.mock.calls[0][0]).toBe("modular-_NEW_XenGameKit");
	});

	// Companion case: when `hindsight.scoping` is `global`, clearing the
	// non-empty bankId should restore the bare `modular` default — the operator's
	// stated expectation in the live repro from #1902.
	it("routes future retains to the bare modular bank when bankId is cleared in global scoping", async () => {
		const retainBatchSpy = vi.spyOn(HindsightApi.prototype, "retainBatch").mockResolvedValue({} as never);
		vi.spyOn(HindsightApi.prototype, "createBank").mockResolvedValue({} as never);
		const settings = Settings.isolated({
			"memory.backend": "hindsight",
		...
		const next = session.getHindsightSessionState();
		expect(next?.bankId).toBe("modular");

		next!.enqueueRetain("post-reset global fact");
		await next!.flushRetainQueue();

		expect(retainBatchSpy).toHaveBeenCalledTimes(1);
		expect(retainBatchSpy.mock.calls[0][0]).toBe("modular");
	});
