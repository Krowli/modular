...
import { discoverAndLoadExtensions } from "@modular/pi-coding-agent/extensibility/extensions/loader";
import { getAgentDir, getPluginsDir, removeSyncWithRetries, setAgentDir, TempDir } from "@modular/pi-utils";

const currentPiCodingAgentPath = Bun.resolveSync("@modular/pi-coding-agent", import.meta.dir);
const currentPiExtensionsPath = Bun.resolveSync("@modular/pi-coding-agent/extensibility/extensions", import.meta.dir);
...
describe("plugin extension discovery", () => {
	...
	beforeEach(() => {
		...
		// resolves into `<tempHome>/.modular/plugins` on every platform. Two things are needed:
		//  - mock os.homedir() so configRoot = `<tempHome>/.modular` (the previous
		//    XDG_DATA_HOME redirect was a no-op on Windows, where these tests then wrote
		//    into and rm'd the developer's real `~/.modular/plugins`);
		//  - clear the XDG_* vars, because on Linux/macOS the resolver prefers
		//    `$XDG_DATA_HOME/modular` over the home config root when that dir exists, so an
		...
		setAgentDir(path.join(tempHome, ".modular", "agent"));
		...
		// ~/.modular must fail loudly here instead of clobbering the developer's plugins.
	...
	it("loads installed legacy Pi plugin extensions from Windows drive-letter paths", async () => {
		...
		fs.writeFileSync(
			...
			[
				...
				'import { isToolCallEventType as legacyRoot } from "@modular/pi-coding-agent";',
				'import { isToolCallEventType as legacyExtensions } from "@modular/pi-coding-agent/extensibility/extensions";',
	...
	it("rewrites side-effect imports of package-import aliases and legacy Pi scopes", async () => {
		...
		fs.writeFileSync(
			...
			[
				...
				// `@modular` import inside `register.ts` resolves to the
				// host `@modular` package.
				'import "#src/register";',
				'import "./marker";',
				"",
	...
	it("resolves a sub-extension directory via its own package.json manifest over a decoy index", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "feature-ext", version: "1.0.0", modular: { extensions: ["./dist/real-ext.ts"] } }),
		);
		fs.writeFileSync(
			realEntry,
			[
