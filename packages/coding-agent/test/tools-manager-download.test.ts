...
import { downloadFile } from "@modular/pi-coding-agent/utils/tools-manager";
...
describe("tool asset downloads", () => {
	...
	it("writes a completed response body to disk", async () => {
		using tempDir = TempDir.createSync("@modular-tool-download-");
	...
	it("aborts a stalled response body and removes the partial file", async () => {
		using tempDir = TempDir.createSync("@modular-tool-download-stall-");
		const dest = tempDir.join("tool.bin");
