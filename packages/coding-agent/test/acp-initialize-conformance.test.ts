...
import { AcpAgent } from "@modular/pi-coding-agent/modes/acp/acp-agent";
import { ACP_TERMINAL_AUTH_FLAG, prepareAcpTerminalAuthArgs } from "@modular/pi-coding-agent/modes/acp/terminal-auth";
...
import { getConfigRootDir, setAgentDir, VERSION } from "@modular/pi-utils";
...
async function createAgent(): Promise<AcpAgent> {
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "modular-acp-init-"));
