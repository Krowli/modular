...
import type { Model } from "@modular/pi-ai";
...
} from "@modular/pi-coding-agent/modes/acp/acp-agent";
import type { PlanModeState } from "@modular/pi-coding-agent/plan-mode/state";
import type { AgentSession, AgentSessionEvent } from "@modular/pi-coding-agent/session/agent-session";
import { SILENT_ABORT_MARKER } from "@modular/pi-coding-agent/session/messages";
import { SessionManager } from "@modular/pi-coding-agent/session/session-manager";
import { DEFAULT_STT_MODEL_KEY, STT_MODEL_OPTIONS } from "@modular/pi-coding-agent/stt/models";
import {
	DEFAULT_TTS_LOCAL_MODEL_KEY,
	DEFAULT_TTS_VOICE,
	TTS_LOCAL_MODELS,
	TTS_LOCAL_VOICE_OPTIONS,
} from "@modular/pi-coding-agent/tts/models";
import { getConfigRootDir, setAgentDir } from "@modular/pi-utils";
...
async function createHarness(
	...
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "modular-acp-test-"));
...
describe("ACP agent", () => {
	...
	it("accepts MODULAR extension methods and rejects unknown unprefixed methods", async () => {
		const harness = await createHarness();

		const result = await harness.agent.extMethod("_mod/sessions/listAll", { limit: 2 });

		expect(Array.isArray(result.sessions)).toBe(true);
		expect(typeof result.total).toBe("number");
		await expect(harness.agent.extMethod("modular/sessions/listAll", { limit: 2 })).rejects.toThrow(
