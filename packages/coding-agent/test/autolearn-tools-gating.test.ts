...
import { resetActiveSkillsForTests, type Skill, setActiveSkills } from "@modular/pi-coding-agent/extensibility/skills";
import type { HindsightSessionState } from "@modular/pi-coding-agent/hindsight/state";
import type { MnemopiSessionState } from "@modular/pi-coding-agent/mnemopi/state";
import { createTools, type ToolSession } from "@modular/pi-coding-agent/tools";
import { LearnTool } from "@modular/pi-coding-agent/tools/learn";
import { ManageSkillTool } from "@modular/pi-coding-agent/tools/manage-skill";
...
describe("manage_skill execute", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "modular-manage-skill-"));
...
describe("learn execute", () => {
	...
	beforeEach(async () => {
		originalAgentDir = getAgentDir();
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "modular-learn-"));
		spyOn(os, "homedir").mockReturnValue(tempHome);
		setAgentDir(path.join(tempHome, ".modular", "agent"));
		remembered = [];
	});
