...
import { getProjectDir, removeSyncWithRetries, setProjectDir } from "@modular/pi-utils";
...
function createFakeHome(): { home: string; projectsRoot: string } {
	...
	const home = fs.mkdtempSync(path.join(homeRoot, "modular-status-line-home-"));
...
describe("status line path segment", () => {
	it("strips the Projects root for symlink-equivalent aliases", () => {
		...
		const realProjectDir = fs.mkdtempSync(path.join(projectsRoot, "modular-status-line-"));
		const nestedDir = path.join(realProjectDir, "nested");
		const aliasRoot = fs.mkdtempSync(path.join(os.tmpdir(), "modular-status-line-alias-"));
	...
	it("strips the scratch root and shows only the trailing folder inside the OS tmp dir", () => {
		const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-status-line-scratch-"));
		try {
	...
	it("keeps nested subpaths visible under a scratch root", () => {
		const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-status-line-scratch-nest-"));
	...
	it("keeps the folder icon for scratch paths when stripWorkPrefix is disabled", () => {
		const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-status-line-scratch-noprefix-"));
		try {
			setProjectDir(scratchDir);
	...
	it("keeps the folder icon for paths outside any scratch root", () => {
		const { home, projectsRoot } = createFakeHome();
		const realProjectDir = fs.mkdtempSync(path.join(projectsRoot, "modular-status-line-real-"));
		try {
	...
	it("renders the active nested repo suffix after the parent cwd", () => {
		const parentDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-status-line-parent-"));
...
describe("status line path segment in a linked worktree", () => {
	...
	it("falls back to the on-disk path when stripWorkPrefix is disabled", () => {
		const scratchDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-status-line-wt-noprefix-"));
		try {
			setProjectDir(scratchDir);
