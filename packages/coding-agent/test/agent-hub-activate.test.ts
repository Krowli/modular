...
import { AgentHubOverlayComponent } from "@modular/pi-coding-agent/modes/components/agent-hub";
import { SelectorController } from "@modular/pi-coding-agent/modes/controllers/selector-controller";
import { SessionObserverRegistry } from "@modular/pi-coding-agent/modes/session-observer-registry";
...
describe("Agent hub Enter activation", () => {
	...
	it("lists persisted subagent session files after restart", async () => {
		using tempDir = TempDir.createSync("@modular-agent-hub-persisted-");
...
describe("Agent hub double-← gating", () => {
	...
	it("requireContent opens the hub after persisted subagents load", async () => {
		using tempDir = TempDir.createSync("@modular-agent-hub-require-content-");
		const sessionFile = path.join(tempDir.path(), "main.jsonl");
		const workerSessionFile = path.join(tempDir.path(), "main", "Worker.jsonl");
		await Bun.write(sessionFile, "");
		await Bun.write(workerSessionFile, "");
