...
import type { AgentTool, AgentToolResult } from "@modular/pi-agent-core";
import { Settings } from "@modular/pi-coding-agent/config/settings";
import { disposeAllVmContexts } from "@modular/pi-coding-agent/eval/js/context-manager";
import { executeJs, type JsResult } from "@modular/pi-coding-agent/eval/js/executor";
...
describe("executeJs", () => {
	...
	it("does not expose the final expression marker as a global property", async () => {
		...
		const marker = await executeJs("return Object.hasOwn(globalThis, '__modular_final_expr__');", {
			sessionId,
			session,
			sessionFile,
		});
	...
	it("ignores user-assigned final expression markers without a rewritten final expression", async () => {
		const result = await executeJs("globalThis.__modular_final_expr__ = 'manual'; return 'actual';", {
			sessionId,
			session,
			sessionFile,
		});
	...
	it("captures promise-valued final expression before promise callbacks can mutate the marker", async () => {
		const result = await executeJs(
			"const pending = Promise.resolve(1).then(value => { globalThis.__modular_final_expr__ = 999; return value; }); pending;",
			{ sessionId, session, sessionFile },
		);
	...
	it("exposes common web globals and the raw Node fs module", async () => {
		const result = await executeJs(
			[
				...
				"const base = __modular_session__.cwd;",
