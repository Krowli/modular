...
import { QrCode, type QrEcLevel, renderQrHalfBlocks } from "@modular/pi-coding-agent/utils/qrcode";
...
describe("QR encoder", () => {
	...
	const vectors: ReadonlyArray<{
		...
		{
			text: "https://my.modular.dev/#mgAYTZwEnpRQtca0CTgn-Q.gdJU",
			ecl: "M",
			mask: 4,
			version: 4,
			size: 33,
			hash: "f84b10722374963e",
		},
		{
	...
	it("deterministically selects a penalty-minimizing mask when none is forced", () => {
		...
		const qr = QrCode.encodeText("https://my.modular.dev/#demo", "M");
		expect(qr.mask).toBe(3);
		expect(matrixFingerprint(qr)).toBe("4ccea5470d6d252f");
	});
...
describe("renderQrHalfBlocks", () => {
	it("frames the symbol in a light quiet zone wide enough for the margin", () => {
		const qr = QrCode.encodeText("https://modular.dev/#demo", "M");
		const margin = 3;
