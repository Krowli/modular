...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@modular/pi-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.modular/`), plus
//    the legacy non-XDG `~/.modular/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.modular/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/modular/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.modular` when
...
// symlinks `~/.modular/agent/extensions/foo.ts` to a dotfiles repo would see the
