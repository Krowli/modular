...
import { HookSelectorComponent } from "@modular/pi-coding-agent/modes/components/hook-selector";
import { getThemeByName, setThemeInstance, theme } from "@modular/pi-coding-agent/modes/theme/theme";
...
describe("HookSelectorComponent", () => {
	...
	it("wraps outlined option text without omitting the tail", () => {
		const options = [
			"Option A: Move to MODULAR-native only by migrating reusable shared AI instructions into .modular/AGENTS.md, .modular/rules, .modular/skills, and .modular/agents while deliberately not creating a root .github directory.",
			"Option B: Keep dual support by migrating canonical instructions into .modular while also maintaining a root .github/copilot-instructions.md compatibility bridge for editors that do not understand MODULAR resources yet.",
		];
		const component = new HookSelectorComponent(
		...
		expect(normalizedPlain).toContain("do not understand MODULAR resources yet");
		for (const line of lines) {
			expect(visibleWidth(Bun.stripANSI(line))).toBeLessThanOrEqual(width);
		}
	});
		...
		const options = [
			{
				...
				description:
					"Authenticate via the provider keys and OAuth state already configured under ~/.modular without opening a new browser-based setup flow.",
			},
			{
				label: "Set up Modular in terminal",
				description:
