...
import { tryRunRpcSkillCommand } from "@modular/pi-coding-agent/modes/rpc/rpc-mode";
import { type CustomMessage, SKILL_PROMPT_MESSAGE_TYPE } from "@modular/pi-coding-agent/session/messages";
import { removeWithRetries, Snowflake } from "@modular/pi-utils";

describe("tryRunRpcSkillCommand", () => {
	test("dispatches registered /skill commands as skill prompt messages", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), `modular-rpc-skill-${Snowflake.next()}-`));
