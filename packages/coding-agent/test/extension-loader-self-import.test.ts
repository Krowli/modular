...
import * as PiCodingAgent from "@modular/pi-coding-agent";
import { loadCustomCommands } from "@modular/pi-coding-agent/extensibility/custom-commands/loader";
import { loadCustomTools } from "@modular/pi-coding-agent/extensibility/custom-tools/loader";
import { loadExtensions } from "@modular/pi-coding-agent/extensibility/extensions/loader";
import { loadHooks } from "@modular/pi-coding-agent/extensibility/hooks/loader";
import { TempDir } from "@modular/pi-utils";

declare global {
	var __modHostPiForLoaderIdentityTest: typeof PiCodingAgent | undefined;
}
...
describe("extension loader host runtime binding", () => {
	...
	beforeEach(() => {
		...
		globalThis.__modHostPiForLoaderIdentityTest = PiCodingAgent;
	});

	afterEach(() => {
		projectDir?.removeSync();
		projectDir = undefined;
		globalThis.__modHostPiForLoaderIdentityTest = undefined;
	});
	...
	const identityGuard = `
		const expectedPi = globalThis.__modHostPiForLoaderIdentityTest;
