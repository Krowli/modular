...
import { applyResolvedSystemPromptInputs, submitInteractiveInput } from "@modular/pi-coding-agent/main";
import type { SubmittedUserInput } from "@modular/pi-coding-agent/modes/types";
import type { CreateAgentSessionOptions } from "@modular/pi-coding-agent/sdk";
import { discoverTitleSystemPromptFile } from "@modular/pi-coding-agent/system-prompt";
...
describe("discoverTitleSystemPromptFile", () => {
	it("discovers TITLE_SYSTEM.md from the project modular config directory", async () => {
		const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-title-system-"));
		cleanupDirs.push(projectDir);
		const configDir = path.join(projectDir, ".modular");
