...
import { buildSystemPrompt } from "@modular/pi-coding-agent/system-prompt";
import { usesCodexTaskPrompt } from "@modular/pi-coding-agent/task/prompt-policy";
...
it("renders the prompt date in the startup timezone", async () => {
	setSystemTime(new Date(process.env.MODULAR_TEST_NOW!));
	try {
		...
		expect(rendered).toContain(\`Today is \${process.env.MODULAR_EXPECTED_DATE}\`);
		expect(rendered).not.toContain(\`Today is \${process.env.MODULAR_REJECTED_DATE}\`);
	} finally {
		setSystemTime();
	}
});
`,
	...
	const child = Bun.spawn([process.execPath, "test", scenarioPath], {
		...
		env: {
			...
			TZ: options.timeZone,
			MODULAR_TEST_NOW: options.now,
			MODULAR_EXPECTED_DATE: options.expectedDate,
			MODULAR_REJECTED_DATE: options.rejectedDate,
		},
		stdout: "pipe",
		stderr: "pipe",
	});
