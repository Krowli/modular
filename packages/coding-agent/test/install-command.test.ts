/**
 * Regression test for #1496 (bug 2): `modular install ./my-extension` used to be
...
import { commands, isSubcommand, resolveCliArgv } from "@modular/pi-coding-agent/cli-commands";
import { looksLikeLocalPath } from "@modular/pi-coding-agent/commands/install";
...
describe("install command is registered as a top-level subcommand", () => {
	...
	test("CLI runner rejects only bare reserved management words", () => {
		expect(resolveCliArgv(["extensions"])).toEqual({
			error: '`modular extensions` is not a management command. Use `modular plugin list` / `modular plugin install`, or run `modular launch extensions` if you meant to send "extensions" as a prompt.',
		});
...
describe("looksLikeLocalPath", () => {
	...
	test("npm specs and marketplace refs are remote", () => {
		expect(looksLikeLocalPath("@modular/exa")).toBe(false);
	...
	test("bare names that exist as a local directory are treated as local", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-install-test-"));
		try {
