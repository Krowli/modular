...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through modular", async () => {
		const files = new Map<string, string>();

		const result = await installProfileAlias({
			profile: "work",
			aliasName: "modular-work",
			shellPath: "/bin/bash",
			platform: "linux",
			homeDir: "/home/me",
		...
		expect(result.command).toBe("modular --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("modular-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command modular --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed modular binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("modular-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("modular-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/modular-profiles.fish") ?? "";
		expect(content).toContain("function modular-work --wraps modular");
		expect(content).toContain("command modular --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/modular-profiles.fish");
		expect(files.get(result.configPath)).toContain("function modular-work --wraps modular");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function modular-work");
		expect(content).toContain("& modular --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& modular --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> modular profile alias: modular-work >>>",
					"alias modular-work='command modular --profile=old'",
					"# <<< modular profile alias: modular-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command modular --profile=work "$@"');
	...
	it("refuses to shadow the base modular command case-insensitively", async () => {
		for (const aliasName of ["modular", "MODULAR"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid MODULAR profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/modular-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/modular-profiles.fish'");
	});
