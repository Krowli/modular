...
import { AgentDashboard } from "@modular/pi-coding-agent/modes/components/agent-dashboard";
import { initTheme } from "@modular/pi-coding-agent/modes/theme/theme";
import * as discovery from "@modular/pi-coding-agent/task/discovery";
...
async function makeTempCwd(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-agent-dashboard-"));
	tempDirs.push(dir);
	return dir;
}
