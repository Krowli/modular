...
} from "@modular/pi-coding-agent/utils/image-loading";
...
describe("modelLacksWebpSupport", () => {
	...
	test("webpExclusionForModel yields true|undefined so the MODULAR_NO_WEBP fallback survives", () => {
...
describe("normalizeModelContextImages model-aware WebP exclusion", () => {
	const prior = Bun.env.MODULAR_NO_WEBP;

	beforeEach(() => {
		delete (Bun.env as Record<string, string | undefined>).MODULAR_NO_WEBP;
	});

	afterEach(() => {
		if (prior === undefined) delete (Bun.env as Record<string, string | undefined>).MODULAR_NO_WEBP;
		else Bun.env.MODULAR_NO_WEBP = prior;
	});
	...
	test("keeps WebP for a WebP-capable model when MODULAR_NO_WEBP is unset", async () => {
