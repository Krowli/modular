...
import { Effort } from "@modular/pi-ai";
import { getBundledModel } from "@modular/pi-catalog/models";
import * as autoThinkingClassifier from "@modular/pi-coding-agent/auto-thinking/classifier";
...
describe("AgentSession magic keyword settings", () => {
	...
	beforeEach(async () => {
		root = await fs.mkdtemp(path.join(os.tmpdir(), "modular-magic-keywords-"));
	});
