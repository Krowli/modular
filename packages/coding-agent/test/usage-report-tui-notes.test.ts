...
import type { UsageReport } from "@modular/pi-ai";
import { renderUsageReports } from "@modular/pi-coding-agent/modes/controllers/command-controller";
...
describe("renderUsageReports (#3268 TUI aggregate)", () => {
	it("renders provider-wide UsageReport.notes exactly once for multiple accounts", () => {
		const disclaimer = "MODULAR-observed spend only; OpenCode usage outside MODULAR is not included.";
