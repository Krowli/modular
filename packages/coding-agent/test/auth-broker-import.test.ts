...
import { AuthStorage, SqliteAuthCredentialStore } from "@modular/pi-ai";
import { type AuthBrokerServerHandle, startAuthBroker } from "@modular/pi-ai/auth-broker";
import { runAuthBrokerCommand } from "@modular/pi-coding-agent/cli/auth-broker-cli";
import { getAgentDbPath, removeWithRetries, setAgentDir } from "@modular/pi-utils";
...
describe("auth-broker import (CLIProxyAPI)", () => {
	...
	beforeEach(async () => {
		originalAgentDir = process.env.MODULAR_AGENT_DIR;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-import-agent-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-import-cliproxy-"));
		setAgentDir(agentDir);
	});

	afterEach(async () => {
		process.stdout.write = ORIGINAL_STDOUT_WRITE;
		if (originalAgentDir === undefined) delete process.env.MODULAR_AGENT_DIR;
		else process.env.MODULAR_AGENT_DIR = originalAgentDir;
	...
	test("imports a directory of CLIProxyAPI JSONs and maps types to modular providers", async () => {
...
describe("auth-broker import (broker-routed)", () => {
	...
	beforeEach(async () => {
		savedEnv.MODULAR_AUTH_BROKER_URL = process.env.MODULAR_AUTH_BROKER_URL;
		savedEnv.MODULAR_AUTH_BROKER_TOKEN = process.env.MODULAR_AUTH_BROKER_TOKEN;
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-import-client-"));
		brokerAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-import-broker-"));
		cliproxyDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-import-cliproxy-broker-"));
		setAgentDir(agentDir);
		...
		process.env.MODULAR_AUTH_BROKER_URL = handle.url;
		process.env.MODULAR_AUTH_BROKER_TOKEN = token;
	});
	...
	afterEach(async () => {
		...
		await removeWithRetries(cliproxyDir);
		for (const key of ["MODULAR_AUTH_BROKER_URL", "MODULAR_AUTH_BROKER_TOKEN"] as const) {
