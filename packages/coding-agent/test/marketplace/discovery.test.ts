/**
 * Discovery integration tests for MODULAR plugin registry reading.
 *
 * NOTE: listClaudePluginRoots() lives in discovery/helpers.ts which imports
 * @modular/pi-natives (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. MODULAR registry lives at path.join(home, ".modular", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".modular")
...
import type { InstalledPluginEntry } from "@modular/pi-coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
	readInstalledPluginsRegistry,
	writeInstalledPluginsRegistry,
} from "@modular/pi-coding-agent/extensibility/plugins/marketplace";
import { removeSyncWithRetries } from "@modular/pi-utils";

// ── Inline validator ───────────────────────────────────────────────────────────
//
// Mirrors parseClaudePluginsRegistry() in discovery/helpers.ts exactly.
// Kept here to avoid importing helpers.ts (which pulls in @modular/pi-natives).
...
// Matches getConfigDirName() — single source of truth is in @modular/pi-utils,
// but we know the value is ".modular" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const MODULAR_CONFIG_DIR = ".modular";
...
let tmpHome: string;
/** ~/.modular/plugins/installed_plugins.json inside tmpHome */
let modRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "modular-discovery-test-"));
	modRegistryPath = path.join(tmpHome, MODULAR_CONFIG_DIR, "plugins", "installed_plugins.json");
	fs.mkdirSync(path.dirname(modRegistryPath), { recursive: true });
});
...
describe("MODULAR registry path contract", () => {
	it("MODULAR registry lives at home/.modular/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".modular", "plugins", "installed_plugins.json");
		expect(modRegistryPath).toBe(expected);
	});
});

// ── Format compatibility ───────────────────────────────────────────────────────

describe("MODULAR registry format compatibility with Claude parser", () => {
	it("empty registry written by writeInstalledPluginsRegistry passes validator", async () => {
		await writeInstalledPluginsRegistry(modRegistryPath, { version: 2, plugins: {} });

		const content = fs.readFileSync(modRegistryPath, "utf8");
	...
	it("registry with installed plugin passes validator", async () => {
		...
		let reg = await readInstalledPluginsRegistry(modRegistryPath);
		reg = addInstalledPlugin(reg, pluginId, entry);
		await writeInstalledPluginsRegistry(modRegistryPath, reg);
...
describe("MODULAR registry round-trip", () => {
	it("reads back what was written — single plugin", async () => {
		...
		const readBack = await readInstalledPluginsRegistry(modRegistryPath);
...
//
// listClaudePluginRoots must replace Claude entries with MODULAR entries when the same
...
describe("MODULAR precedence contract (registry structure)", () => {
	it("same plugin ID in both registries — MODULAR entry has required fields for deduplication", () => {
		// The replacement logic: roots.filter(r => r.id !== pluginId) keyed by id.
		// MODULAR entries must have installPath so they can be added to roots[].
		const id = buildPluginId("shared-plugin", "common-mkt");
		const modEntry = makeEntry("/modular/cached/path");

		// MODULAR registry entry has installPath (required by listClaudePluginRoots)
		expect(modEntry.installPath).toBeTruthy();
		expect(typeof modEntry.installPath).toBe("string");
