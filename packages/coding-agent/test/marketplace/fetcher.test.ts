...
describe("fetchMarketplace", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-fetcher-test-"));
	});
	...
	it("loads catalog from .modular-plugin/marketplace.json when present", async () => {
		const root = path.join(tmpDir, "modular-only");
		fs.mkdirSync(path.join(root, ".modular-plugin"), { recursive: true });
		const catalog = {
			name: "modular-only-marketplace",
			owner: { name: "Test" },
			plugins: [{ name: "modular-plugin", source: "./plugins/modular-plugin", description: "x" }],
		};
		fs.writeFileSync(path.join(root, ".modular-plugin", "marketplace.json"), JSON.stringify(catalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("modular-only-marketplace");
		expect(result.catalog.plugins[0].name).toBe("modular-plugin");
	});

	it("prefers .modular-plugin/marketplace.json over .claude-plugin/marketplace.json when both exist", async () => {
		const root = path.join(tmpDir, "both-catalogs");
		fs.mkdirSync(path.join(root, ".modular-plugin"), { recursive: true });
		fs.mkdirSync(path.join(root, ".claude-plugin"), { recursive: true });
		const modCatalog = {
			name: "from-modular-plugin",
			owner: { name: "Test" },
		...
		fs.writeFileSync(path.join(root, ".modular-plugin", "marketplace.json"), JSON.stringify(modCatalog));
		fs.writeFileSync(path.join(root, ".claude-plugin", "marketplace.json"), JSON.stringify(claudeCatalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("from-modular-plugin");
	});

	it("falls back to .claude-plugin/marketplace.json when .modular-plugin is absent", async () => {
	...
	it("error message names both candidate paths when neither exists", async () => {
		...
		await expect(fetchMarketplace(empty, tmpDir)).rejects.toThrow(
			/\.modular-plugin[\\/]marketplace\.json.*\.claude-plugin[\\/]marketplace\.json/,
		);
	});
