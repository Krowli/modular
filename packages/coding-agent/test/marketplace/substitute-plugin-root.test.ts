...
import { substitutePluginRoot } from "@modular/pi-coding-agent/discovery/substitute-plugin-root";
...
const MODULAR_VAR = "$" + "{MODULAR_PLUGIN_ROOT}";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces MODULAR_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${MODULAR_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${MODULAR_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
