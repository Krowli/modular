...
import { listOmpExtensionRoots } from "@modular/pi-coding-agent/discovery/modular-extension-roots";
import { getEnabledPlugins } from "@modular/pi-coding-agent/extensibility/plugins/loader";
import { PluginManager } from "@modular/pi-coding-agent/extensibility/plugins/manager";
import {
	MarketplaceManager,
...
function buildMinimalFixture(): string {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "modular-mgr-fixture-"));
...
function createTestContext(): TestContext {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-mgr-test-"));
...
function mockPluginManagerPaths(root: string) {
	return [
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(root, "modular-plugins.lock.json")),
...
describe("MarketplaceManager", () => {
	...
	it("installPlugin → plugin in cache + in registry", async () => {
		...
		const runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "modular-plugins.lock.json")).json();
	...
	it("installPlugin exposes marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "modular-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".modular", "plugins");
			const manager = new MarketplaceManager({
				marketplacesRegistryPath: path.join(tmpHome, ".modular", "marketplaces.json"),
	...
	it("installPlugin keeps marketplace packages out of MODULAR extension roots", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "modular-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".modular", "plugins");
			const manager = new MarketplaceManager({
	...
	it("installPlugin with scope:project exposes the marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "modular-mgr-home-"));
		const projectAnchor = fs.mkdtempSync(path.join(os.tmpdir(), "modular-mgr-project-"));
		try {
			const userPluginsDir = path.join(tmpHome, ".modular", "plugins");
			const projectPluginsDir = path.join(projectAnchor, ".modular", "plugins");
			...
			const projectLock = await Bun.file(path.join(projectPluginsDir, "modular-plugins.lock.json")).json();
			...
			expect(fs.existsSync(path.join(userPluginsDir, "modular-plugins.lock.json"))).toBe(false);
		} finally {
			fs.rmSync(tmpHome, { recursive: true, force: true });
	...
	it("setPluginEnabled → persisted in registry", async () => {
		...
		let runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "modular-plugins.lock.json")).json();
		expect(runtimeConfig.plugins["hello-plugin"].enabled).toBe(false);
		...
		runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "modular-plugins.lock.json")).json();
	...
	it("installPlugin scope:project when no projectInstalledRegistryPath → throws", async () => {
		const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "modular-mgr-noproj-"));
		try {
