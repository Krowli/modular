 ...
 *
 * Note: helpers.ts imports @modular/pi-natives (Rust addon via glob).
...
} from "@modular/pi-coding-agent/discovery/helpers";
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-proj-scope-"));
	});
	...
	it("walk-up finds nearest .modular/ directory", async () => {
		// Layout: tmpDir/.modular/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .modular/ there.
		fs.mkdirSync(path.join(tmpDir, ".modular"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".modular", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .modular/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.modular/   +   tmpDir/sub/.modular/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.modular/, not climb further to tmpDir/.modular/.
		fs.mkdirSync(path.join(tmpDir, ".modular"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".modular"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".modular", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .modular/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .modular/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .modular/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .modular/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.modular/.../installed_plugins.json.
		const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-proj-scope-home-"));
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeOmpPath = path.join(homeDir, ".modular", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listClaudePluginRoots reads the user MODULAR registry. */
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "modular-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "modular-shadow-proj-"));

		// Create .modular/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".modular", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".modular", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".modular", "plugins", "installed_plugins.json");
	});
