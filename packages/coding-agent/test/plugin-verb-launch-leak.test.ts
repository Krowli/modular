/**
 * Regression test for #2935: the plugins docs advertise `modular list` / `modular remove`
 * as top-level commands, but only `modular install` is registered. Before the fix,
 * `resolveCliArgv(["list"])` rewrote the bare verb to `["launch", "list"]`, so
 * `modular list` silently started an interactive agent session with "list" as the
 * initial LLM prompt instead of managing plugins (the real command is
 * `modular plugin list`). Same footgun for `modular remove`.
 *
 * These tests pin the chosen bugfix: a bare, single-arg documented plugin verb
 * yields a helpful hint pointing at the real `modular plugin <action>` command
 ...
 *
 * Imported via a relative path (not the `@modular/pi-coding-agent` alias) so the
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `modular list` hints at `modular plugin list` instead of launching with 'list' as the prompt", () => {
		...
		expect("error" in result && result.error).toContain("modular plugin list");
	});

	test("bare `modular remove` hints at `modular plugin uninstall` instead of launching with 'remove' as the prompt", () => {
		const result = resolveCliArgv(["remove"]);
		expect(result).not.toEqual({ argv: ["launch", "remove"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("modular plugin uninstall");
	});
