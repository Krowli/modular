...
import * as pluginCli from "@modular/pi-coding-agent/cli/plugin-cli";
import * as updateCli from "@modular/pi-coding-agent/cli/update-cli";
import {
	buildBunInstallArgs,
	...
	sweepStaleBackups,
} from "@modular/pi-coding-agent/cli/update-cli";
import Update from "@modular/pi-coding-agent/commands/update";
import { removeWithRetries } from "@modular/pi-utils";
import type { CliConfig } from "@modular/pi-utils/cli";

const tempDirs: string[] = [];

async function makeTempDir(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-update-test-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("update-cli install target detection", () => {
	it("uses bun update when prioritized modular is inside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.bun/bin/modular", "/Users/test/.bun/bin");

		expect(method).toBe("bun");
	});

	it("uses npm update when prioritized modular is inside an npm global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.npm-global/bin/modular", undefined, {
	...
	it("uses npm update for Windows npm command shims even when no package-manager bin dirs were detected", () => {
		const method = resolveUpdateMethodForTest("C:\\Users\\test\\AppData\\Roaming\\npm\\modular.cmd", undefined);

		expect(method).toBe("npm");
	});

	it("uses binary update when prioritized modular is outside bun global bin", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/modular", "/Users/test/.bun/bin");

		expect(method).toBe("binary");
	});

	it("uses binary update when bun global bin cannot be resolved", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/bin/modular", undefined);

		expect(method).toBe("binary");
	});

	it("uses Homebrew update when prioritized modular resolves into the Homebrew formula", async () => {
		const dir = await makeTempDir();
		const prefix = path.join(dir, "opt", "modular");
		const linkedBin = path.join(dir, "bin");
		await fs.mkdir(path.join(prefix, "bin"), { recursive: true });
		await fs.mkdir(linkedBin, { recursive: true });
		await Bun.write(path.join(prefix, "bin", "modular"), "binary");
		await fs.symlink(path.join(prefix, "bin", "modular"), path.join(linkedBin, "modular"));

		const method = resolveUpdateMethodForTest(path.join(linkedBin, "modular"), "/Users/test/.bun/bin", {
	...
	it("uses mise update when prioritized modular is in an active mise bin path", () => {
		const method = resolveUpdateMethodForTest(
			"/Users/test/.local/share/mise/installs/github-modular-modular/latest/bin/modular",
			undefined,
			{
				miseBinDirs: ["/Users/test/.local/share/mise/installs/github-modular-modular/latest/bin"],
			},
		);
	...
	it("uses mise update when prioritized modular is a mise shim", () => {
		const method = resolveUpdateMethodForTest("/Users/test/.local/share/mise/shims/modular", undefined, {
...
describe("update-cli package manager commands", () => {
	it("targets the Homebrew tap formula and switches to reinstall for forced updates", () => {
		expect(buildHomebrewUpdateArgs(false)).toEqual(["upgrade", "modular/tap/modular"]);
		expect(buildHomebrewUpdateArgs(true)).toEqual(["reinstall", "modular/tap/modular"]);
	});

	it("targets the mise GitHub backend tool and force-reinstalls the checked version when requested", () => {
		expect(buildMiseUpgradeArgs()).toEqual(["upgrade", "github:Krowli/modular", "--bump"]);
		expect(buildMiseForceInstallArgs("15.10.5")).toEqual(["install", "--force", "github:Krowli/modular@15.10.5"]);
	});
	...
	it("pins npm package installs to the official registry and the checked native package versions", () => {
		...
		expect(args).toContain("@modular/pi-coding-agent@16.3.15");
		expect(args).toContain("@modular/pi-natives@16.3.15");
		expect(args).toContain("@modular/pi-natives-win32-x64@16.3.15");
	});
});
...
describe("update-cli bun install command", () => {
	it("pins the official npm registry and bypasses the manifest cache so a stale mirror or snapshot cannot mask a freshly published version", () => {
		// Regression: modular queries https://registry.npmjs.org/<pkg>/latest directly.
		...
		//     the release.
		// See https://github.com/Krowli/modular/issues/1686.
		...
		expect(args.slice(0, 5)).toEqual([
			...
			"@modular/pi-coding-agent@15.7.6",
		]);
	});
	...
	it("pins the native addon core and the platform-specific leaf to the same version so the loader sentinel cannot drift on supported tags", () => {
		...
		// package, leaving @modular/pi-natives and @modular/pi-natives-<tag>
		...
		// https://github.com/Krowli/modular/issues/1824.
		for (const tag of ["linux-x64", "linux-arm64", "darwin-x64", "darwin-arm64", "win32-x64"]) {
			const args = buildBunInstallArgs("15.9.0", tag);
			expect(args).toContain("@modular/pi-natives@15.9.0");
			expect(args).toContain(`@modular/pi-natives-${tag}@15.9.0`);
		}
	});
	...
	it("omits the leaf on unsupported platform tags so an EBADPLATFORM swap does not mask the underlying `no matching version` error", () => {
		...
		expect(args).toContain("@modular/pi-natives@15.9.0");
		expect(args.some(arg => arg.startsWith("@modular/pi-natives-"))).toBe(false);
	});
...
describe("update-cli bun cache pruning", () => {
	it("keeps only the newest cached version for filtered global install packages", async () => {
		...
		await Bun.write(path.join(dir, "@modular", "pi-utils", "15.7.6@@@1"), "");
		await Bun.write(path.join(dir, "@modular", "pi-utils", "15.8.0@@@1"), "");
		await Bun.write(
			path.join(dir, "@modular", "pi-utils@15.7.6@@@1", "package.json"),
			JSON.stringify({ name: "@modular/pi-utils", version: "15.7.6" }),
		);
		await Bun.write(
			path.join(dir, "@modular", "pi-utils@15.8.0@@@1", "package.json"),
			JSON.stringify({ name: "@modular/pi-utils", version: "15.8.0" }),
		);
		...
		const result = await pruneBunInstallCache(dir, new Set(["react", "@modular/pi-utils"]));
		...
		expect(await Bun.file(path.join(dir, "@modular", "pi-utils", "15.7.6@@@1")).exists()).toBe(false);
		expect(await Bun.file(path.join(dir, "@modular", "pi-utils@15.7.6@@@1", "package.json")).exists()).toBe(false);
		expect(await Bun.file(path.join(dir, "@modular", "pi-utils", "15.8.0@@@1")).exists()).toBe(true);
		expect(await Bun.file(path.join(dir, "@modular", "pi-utils@15.8.0@@@1", "package.json")).exists()).toBe(true);
...
describe("update-cli binary replacement", () => {
	it("restores the previous binary when the replacement fails verification", async () => {
		const dir = await makeTempDir();
		const targetPath = path.join(dir, "modular");
		...
		).rejects.toThrow("restored previous mod binary");
...
describe("update-cli binary replacement on locked backups", () => {
	it("treats an EPERM on backup cleanup as a successful, completed update", async () => {
		...
		const dir = await makeTempDir();
		const targetPath = path.join(dir, "modular.exe");
		const tempPath = `${targetPath}.new`;
