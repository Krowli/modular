...
import { getSessionsDir, removeSyncWithRetries, Snowflake } from "@modular/pi-utils";
...
describe("createAgentSession session storage isolation", () => {
	...
	it("loads obfuscator only when secrets exist", async () => {
		await withClearedSecretEnv(async () => {
			...
			fs.mkdirSync(path.join(cwd, ".modular"), { recursive: true });
			fs.writeFileSync(
				path.join(cwd, ".modular", "secrets.yml"),
