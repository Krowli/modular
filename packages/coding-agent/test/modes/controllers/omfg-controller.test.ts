...
import type { AgentMessage } from "@modular/pi-agent-core";
import type { AssistantMessage, Usage } from "@modular/pi-ai";
import type { Rule } from "@modular/pi-coding-agent/capability/rule";
import { OmfgController } from "@modular/pi-coding-agent/modes/controllers/omfg-controller";
import { initTheme } from "@modular/pi-coding-agent/modes/theme/theme";
import type { InteractiveModeContext } from "@modular/pi-coding-agent/modes/types";
import { Container, type TUI } from "@modular/pi-tui";
import { removeWithRetries } from "@modular/pi-utils";

const PROJECT_OPTION = "This project (.modular/rules)";
const GLOBAL_OPTION = "Global — all projects (~/.modular/agent/rules)";
...
describe("OmfgController", () => {
	it("saves a matching generated rule under project rules and registers it live", async () => {
		...
		const savedPath = path.join(harness.projectDir, ".modular", "rules", "ts-no-any.md");
		...
		expect(rendered).toContain(path.join(".modular", "rules", "ts-no-any.md"));
	...
	it("reiterates when the first valid rule does not match history", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".modular", "rules", "ts-no-any.md")).exists()).toBe(true);
	});
	...
	it("asks before saving when validation never confirms a match", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".modular", "rules", "no-match.md")).exists()).toBe(false);
	});
	...
	it("lets the user amend from the save selector before writing the rule", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".modular", "rules", "ts-any-broad.md")).exists()).toBe(
			false,
		);
		expect(await Bun.file(path.join(harness.projectDir, ".modular", "rules", "ts-no-explicit-any.md")).exists()).toBe(
			true,
		);
	});
	...
	it("clears the panel and aborts the inner request on Escape", async () => {
		...
		expect(controller.hasActiveRequest()).toBe(false);
		expect(await Bun.file(path.join(harness.projectDir, ".modular", "rules", "ts-no-any.md")).exists()).toBe(false);
	});
});
