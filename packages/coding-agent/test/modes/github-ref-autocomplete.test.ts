...
import { KeybindingsManager as AppKeybindingsManager } from "@modular/pi-coding-agent/config/keybindings";
import { getGithubRefContext, getGithubRefSuggestions } from "@modular/pi-coding-agent/modes/github-ref-autocomplete";
import { createPromptActionAutocompleteProvider } from "@modular/pi-coding-agent/modes/prompt-action-autocomplete";
...
describe("github-ref autocomplete — token detection", () => {
	...
	it("requires a token boundary before # so embedded hashes don't match", () => {
		...
		expect(getGithubRefContext("https://github.com/Krowli/modular#3164")).toBeNull();
