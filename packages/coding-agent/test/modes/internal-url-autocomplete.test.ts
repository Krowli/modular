...
import { resetActiveRulesForTests, setActiveRules } from "@modular/pi-coding-agent/capability/rule";
...
import { resetActiveSkillsForTests, setActiveSkills } from "@modular/pi-coding-agent/extensibility/skills";
...
import {
	...
	isInternalUrlPrefix,
} from "@modular/pi-coding-agent/modes/internal-url-autocomplete";
import { PromptActionAutocompleteProvider } from "@modular/pi-coding-agent/modes/prompt-action-autocomplete";
...
describe("internal-url-autocomplete", () => {
	beforeEach(() => {
		...
		setActiveRules([rule("python", "robodular rules"), rule("style")]);
	});
	...
	describe("getInternalUrlSuggestions", () => {
		...
		it("carries the candidate description through", async () => {
			...
			expect(result!.items[0]).toMatchObject({ value: "rule://python", description: "robodular rules" });
		});
