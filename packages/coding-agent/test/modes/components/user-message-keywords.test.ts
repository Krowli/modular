...
import { UserMessageComponent } from "@modular/pi-coding-agent/modes/components/user-message";
import { getEditorTheme, initTheme } from "@modular/pi-coding-agent/modes/theme/theme";
...
describe("UserMessageComponent magic-keyword highlighting", () => {
	...
	it("wraps image references in file hyperlinks when a blob path is available", () => {
		const imagePath = path.resolve("/tmp/modular-image.png");
