...
import { PluginManager } from "@modular/pi-coding-agent/extensibility/plugins";
import {
...
} from "@modular/pi-coding-agent/modes/components/plugin-settings";
...
describe("PluginListComponent", () => {
	...
	it("empty-state mentions both npm and marketplace install commands", () => {
		...
		expect(text).toContain("modular plugin install <package>");
		expect(text).toContain("modular plugin install <name>@<marketplace>");
	});
...
describe("MarketplacePluginDetailComponent", () => {
	...
	it("shortens home-relative install paths to ~ before rendering", () => {
		...
		const installPath = `${home}/.modular/cache/plugins/sample@mkt`;
		...
		expect(text).toContain("~/.modular/cache/plugins/sample@mkt");
