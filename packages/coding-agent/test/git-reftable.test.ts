...
describe.skipIf(!supportsReftable)("git reftable support", () => {
	...
	beforeAll(async () => {
		...
		sharedRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-reftable-"));
		...
		worktreeDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-reftable-wt-"));
		...
		configRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-reftable-cfg-"));
