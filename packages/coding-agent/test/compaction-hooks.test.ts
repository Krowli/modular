...
import {
	...
	type SessionEvent,
} from "@modular/pi-coding-agent/extensibility/hooks";
import { theme } from "@modular/pi-coding-agent/modes/theme/theme";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("Compaction hooks", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `modular-compaction-hooks-test-${Snowflake.next()}`);
