...
import { ThinkingLevel } from "@modular/pi-agent-core";
import { getBundledModel } from "@modular/pi-catalog/models";
import { type Args, parseArgs } from "@modular/pi-coding-agent/cli/args";
import { ModelRegistry } from "@modular/pi-coding-agent/config/model-registry";
import type { ScopedModel } from "@modular/pi-coding-agent/config/model-resolver";
import { Settings } from "@modular/pi-coding-agent/config/settings";
import { buildSessionOptions } from "@modular/pi-coding-agent/main";
import { type CreateAgentSessionOptions, createAgentSession } from "@modular/pi-coding-agent/sdk";
import type { AgentSession } from "@modular/pi-coding-agent/session/agent-session";
import { AuthStorage } from "@modular/pi-coding-agent/session/auth-storage";
import { CURRENT_SESSION_VERSION, type SessionHeader } from "@modular/pi-coding-agent/session/session-entries";
...
describe("provider prompt-cache key session affinity", () => {
	...
	it("creates an agent whose prompt-cache key can differ from provider request lineage", async () => {
		using tempDir = TempDir.createSync("@modular-prompt-cache-sdk-");
	...
	it("initializes a full fork with child request lineage and parent prompt-cache affinity", async () => {
		using tempDir = TempDir.createSync("@modular-prompt-cache-fork-");
	...
	it("does not auto-inherit parent prompt-cache affinity when fork startup changes request-shaping inputs", async () => {
		...
		for (const entry of cases) {
			using tempDir = TempDir.createSync(`@modular-prompt-cache-fork-${entry.name}-`);
	...
	it("does not pre-pin parent prompt-cache affinity when a scoped model selects the startup route", async () => {
		using tempDir = TempDir.createSync("@modular-prompt-cache-scoped-model-");
