...
import { applyStartupCwd } from "@modular/pi-coding-agent/cli/startup-cwd";
import { getProjectDir, normalizePathForComparison, setProjectDir } from "@modular/pi-utils";
...
describe("parseArgs — --cwd flag", () => {
	...
	it("applies --cwd before session lookup callers read the project directory", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-cwd-launch-"));
		const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-cwd-target-"));
	...
	it("normalizes a relative --cwd target to the resolved absolute path", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-cwd-rel-"));
