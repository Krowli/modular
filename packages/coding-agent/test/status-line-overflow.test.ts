...
import type { StatusLineSegmentId } from "@modular/pi-coding-agent/config/settings-schema";
import { StatusLineComponent } from "@modular/pi-coding-agent/modes/components/status-line";
import type { SegmentContext } from "@modular/pi-coding-agent/modes/components/status-line/segments";
import { renderSegment } from "@modular/pi-coding-agent/modes/components/status-line/segments";
import { initTheme, theme } from "@modular/pi-coding-agent/modes/theme/theme";
import { getSessionAccentAnsi, getSessionAccentHex } from "@modular/pi-coding-agent/utils/session-color";
import { visibleWidth } from "@modular/pi-tui";
import { getProjectDir, setProjectDir } from "@modular/pi-utils";
...
describe("path segment truncation at varying maxLength", () => {
	...
	beforeAll(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-overflow-very-long-directory-name-for-testing-"));
...
describe("overflow: path shrinks before git is dropped", () => {
	...
	beforeAll(() => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-overflow-a-very-long-worktree-directory-name-here-"));
		setProjectDir(tmpDir);
	});
	...
	it("shrinks a short path when maxLength exceeds actual path length", () => {
		...
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-short-"));
	...
	it("preserves git when overflow is only 1-2 columns", () => {
		const shortDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-narrow-ovf-"));
		setProjectDir(shortDir);
		try {
...
describe("overflow: path survives before model", () => {
	it("drops the model segment before the cwd path when both cannot fit", () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "modular-statusline-overflow-"));
