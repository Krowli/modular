...
import { type Api, type AssistantMessage, Effort, type Model } from "@modular/pi-ai";
...
import { type CreateAgentSessionResult, createAgentSession } from "@modular/pi-coding-agent/sdk";
...
import { getRestorableSessionModels } from "@modular/pi-coding-agent/session/session-context";
import { EPHEMERAL_MODEL_CHANGE_ROLE } from "@modular/pi-coding-agent/session/session-entries";
...
describe("AgentSession model persistence", () => {
	...
	it("marks an incomplete process-exit transcript aborted during SDK resume without dropping history", async () => {
		...
		expect(messages.at(-1)).toMatchObject({
			...
			content: [],
			stopReason: "aborted",
			errorMessage: "Previous MODULAR process exited before completing the turn.",
		});
		expect(
			messages.some(
				message =>
