...
import { DEFAULT_FUZZY_THRESHOLD, executePatchSingle, executeReplaceSingle } from "@modular/pi-coding-agent/edit";
import { HashlineFilesystem } from "@modular/pi-coding-agent/edit/hashline/filesystem";
import { resolveLocalUrlToPath } from "@modular/pi-coding-agent/internal-urls";
import type { WritethroughCallback } from "@modular/pi-coding-agent/lsp";
import type { PlanModeState } from "@modular/pi-coding-agent/plan-mode/state";
import type { ClientBridge } from "@modular/pi-coding-agent/session/client-bridge";
...
describe("HashlineFilesystem ACP fs routing", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-acp-hashline-"));
	...
	it("keeps a local sandbox artifact addressed by absolute path off the ACP bridge", async () => {
		...
		// the MODULAR-owned artifact must be written to disk, never pushed to the editor.
		const { bridge, spy: bridgeSpy } = makeBridge();
		const session = createSession(tmpDir, {
			bridge,
...
describe("executeReplaceSingle ACP fs routing", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-acp-replace-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
...
describe("executePatchSingle ACP fs routing", () => {
	...
	beforeEach(async () => {
		resetSettingsForTest();
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-acp-patch-"));
		await Settings.init({ inMemory: true, cwd: tmpDir });
	});
