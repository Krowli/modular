...
describe("AgentStorage model perf aggregates", () => {
	...
	async function openStorage(): Promise<AgentStorage> {
		tempDir = TempDir.createSync("@modular-agent-storage-perf-");
	...
	it("backfills perf aggregates from an modular stats database, excluding errored and stale turns", async () => {
		const storage = await openStorage();
