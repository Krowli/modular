...
import { Settings, settings } from "@modular/pi-coding-agent/config/settings";
import * as downloader from "@modular/pi-coding-agent/stt/downloader";
import * as recorder from "@modular/pi-coding-agent/stt/recorder";
import { STTController } from "@modular/pi-coding-agent/stt/stt-controller";
import { getTinyModelsCacheDir, removeWithRetries, setAgentDir } from "@modular/pi-utils";
...
describe("isSttModelCached completeness", () => {
	...
	beforeEach(async () => {
		...
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "modular-stt-cache-"));
		setAgentDir(tmp);
