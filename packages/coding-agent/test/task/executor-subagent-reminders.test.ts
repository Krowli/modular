...
import { AgentBusyError, type AgentTelemetryConfig, type Tracer } from "@modular/pi-agent-core";
import { type AssistantMessage, Effort } from "@modular/pi-ai";
import { Settings } from "@modular/pi-coding-agent/config/settings";
import type { ExtensionActions, LoadExtensionsResult } from "@modular/pi-coding-agent/extensibility/extensions/types";
...
import type { AuthStorage } from "@modular/pi-coding-agent/session/auth-storage";
import {
	finalizeSubprocessOutput,
	runSubprocess,
	SUBAGENT_WARNING_MISSING_YIELD,
} from "@modular/pi-coding-agent/task/executor";
...
describe("runSubprocess yield reminders", () => {
	...
	const baseOptions = {
		...
		modelRegistry: {
			refresh: async () => {},
		} as unknown as import("@modular/pi-coding-agent/config/model-registry").ModelRegistry,
		enableLsp: false,
	};
