...
} from "@modular/pi-coding-agent/task/isolation-runner";
import type { SingleResult } from "@modular/pi-coding-agent/task/types";
import * as worktreeModule from "@modular/pi-coding-agent/task/worktree";
import * as gitModule from "@modular/pi-coding-agent/utils/git";
...
async function seedFooRepo(finalContent: string): Promise<{ repoRoot: string; patchPath: string }> {
	const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "modular-isolation-merge-"));
...
describe("runIsolatedSubprocess", () => {
	...
	it("preserves branch-mode output as a patch when branch transfer fails", async () => {
		const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "modular-isolation-run-"));
		tempRoots.push(repoRoot);
		...
		expect(deleteSpy).toHaveBeenCalledWith(repoRoot, "modular/task/PreserveBranchFailure");
