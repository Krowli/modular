...
import { disableProvider, enableProvider } from "@modular/pi-coding-agent/capability";
import { clearCache as clearFsCache } from "@modular/pi-coding-agent/capability/fs";
import {
	clearOmpExtensionCliRoots,
	injectOmpExtensionCliRoots,
} from "@modular/pi-coding-agent/discovery/modular-extension-roots";
import { discoverAgents } from "@modular/pi-coding-agent/task/discovery";
import { removeWithRetries } from "@modular/pi-utils";

const MODULAR_AGENT_MD = [
	"---",
	"name: modular-test-agent",
	"description: MODULAR-native test agent.",
	"---",
	"You are an MODULAR task agent.",
].join("\n");

const MODULAR_PLUGIN_AGENT_MD = [
	"---",
...
async function writeOmpPluginAgent(home: string): Promise<void> {
	const userPluginsRoot = path.join(home, ".modular", "plugins");
	...
	await fs.writeFile(
		...
		JSON.stringify({ name: "loom", version: "1.0.0", modular: { version: "1.0.0" } }),
	);
	await fs.writeFile(
		path.join(userPluginsRoot, "package.json"),
		JSON.stringify({
			name: "modular-plugins-root",
			version: "0.0.0",
			dependencies: { loom: "1.0.0" },
		}),
	);
	await fs.writeFile(path.join(pluginRoot, "agents", "loom-verify-spec.md"), MODULAR_PLUGIN_AGENT_MD);
}
...
describe("discoverAgents", () => {
	...
	beforeEach(async () => {
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "modular-task-agent-discovery-"));
		projectDir = path.join(tempHome, "project");
		await fs.mkdir(projectDir, { recursive: true });
	});

	afterEach(async () => {
		enableProvider("modular-plugins");
		clearOmpExtensionCliRoots();
		clearFsCache();
		await removeWithRetries(tempHome);
	});

	test("loads MODULAR agents but skips Claude Code custom agents", async () => {
		await fs.mkdir(path.join(projectDir, ".modular", "agents"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".modular", "agents", "modular-test-agent.md"), MODULAR_AGENT_MD);
		...
		expect(names).toContain("modular-test-agent");
		expect(names).not.toContain("cc-test-agent");
		expect(projectAgentsDir).toBe(path.join(projectDir, ".modular", "agents"));
	});

	test("loads agents from MODULAR npm plugins under <home>/.modular/plugins/node_modules", async () => {
	...
	test("excludes MODULAR npm plugin agents when modular-plugins is disabled", async () => {
		await writeOmpPluginAgent(tempHome);
		disableProvider("modular-plugins");
	...
	test("CLI extension agents win over project `extensions:` settings on dedup", async () => {
		...
		// the skills/hooks/tools surface in discovery/modular-plugins.ts.
		...
		await fs.mkdir(path.join(projectDir, ".modular"), { recursive: true });
		await fs.writeFile(
			path.join(projectDir, ".modular", "settings.json"),
