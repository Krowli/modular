...
import type { AssistantMessage } from "@modular/pi-ai";
import type { ModelRegistry } from "@modular/pi-coding-agent/config/model-registry";
import { Settings } from "@modular/pi-coding-agent/config/settings";
import type { LoadExtensionsResult } from "@modular/pi-coding-agent/extensibility/extensions/types";
import type { PlanModeState } from "@modular/pi-coding-agent/plan-mode/state";
import type { CreateAgentSessionOptions, CreateAgentSessionResult } from "@modular/pi-coding-agent/sdk";
import * as sdkModule from "@modular/pi-coding-agent/sdk";
import type { AgentSession, AgentSessionEvent, PromptOptions } from "@modular/pi-coding-agent/session/agent-session";
import { TaskTool } from "@modular/pi-coding-agent/task";
import * as discoveryModule from "@modular/pi-coding-agent/task/discovery";
import type { AgentDefinition, TaskParams } from "@modular/pi-coding-agent/task/types";
import type { IsolationHandle, WorktreeBaseline } from "@modular/pi-coding-agent/task/worktree";
import * as worktreeModule from "@modular/pi-coding-agent/task/worktree";
import type { ToolSession } from "@modular/pi-coding-agent/tools";
import { removeWithRetries } from "@modular/pi-utils";
import "@modular/pi-coding-agent/tools/yield";
import { EventBus } from "@modular/pi-coding-agent/utils/event-bus";
...
describe("subagent LSP availability", () => {
	...
	it("opens isolated persisted subagent sessions with the worktree cwd", async () => {
		...
		mockIsolation();
		const { getOptions } = mockCreateAgentSession();
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-isolated-session-cwd-"));
		try {
