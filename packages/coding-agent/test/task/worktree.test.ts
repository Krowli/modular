...
import {
	...
	mergeTaskBranches,
	parseIsolationMode,
} from "@modular/pi-coding-agent/task/worktree";
import * as git from "@modular/pi-coding-agent/utils/git";
import * as jj from "@modular/pi-coding-agent/utils/jj";
import * as natives from "@modular/pi-natives";
import { removeWithRetries, setWorktreesDir } from "@modular/pi-utils";
...
async function createGitRepo(): Promise<{ baseBranch: string; repo: string }> {
	const repo = await fs.mkdtemp(path.join(os.tmpdir(), "modular-worktree-"));
	tempDirs.push(repo);
...
describe("worktree isolation helpers", () => {
	...
	describe("git-backed worktree helpers", () => {
		...
		beforeAll(async () => {
			repo = await fs.mkdtemp(path.join(os.tmpdir(), "modular-worktree-"));
		...
		it("uses compact isolation paths that do not embed long task ids", async () => {
			const originalWorktreeDir = process.env.MODULAR_WORKTREE_DIR;
			const worktreeBase = await fs.mkdtemp(path.join(os.tmpdir(), "modular-worktree-base-"));
			tempDirs.push(worktreeBase);
			delete process.env.MODULAR_WORKTREE_DIR;
			...
			try {
				...
				if (originalWorktreeDir === undefined) {
					...
					process.env.MODULAR_WORKTREE_DIR = originalWorktreeDir;
				}
		...
		describe("after rewinding the shared fixture", () => {
			...
			it("keeps the index clean when stash pop would conflict with a cherry-picked change", async () => {
				...
				expect(stashList).toContain("modular-task-merge");
			...
			it("commits isolated edits when parent dirt only changes nearby context", async () => {
				...
				const isoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "modular-worktree-iso-"));
				...
				let branchName = `modular/task/${taskId}`;
				try {
...
describe("getRepoRoot", () => {
	...
	it("rejects pure jj workspaces with an actionable Jujutsu message", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-purejj-"));
		tempDirs.push(dir);
	...
	it("preserves the generic git-not-found error for directories without any repo", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-norepo-"));
		tempDirs.push(dir);
	...
	it("returns the nested git root when a git checkout lives under an outer jj workspace", async () => {
		...
		const outer = await fs.mkdtemp(path.join(os.tmpdir(), "modular-outerjj-"));
		tempDirs.push(outer);
...
describe("applyNestedPatches", () => {
	...
	beforeEach(async () => {
		parentRepo = await fs.mkdtemp(path.join(os.tmpdir(), "modular-nested-apply-"));
	...
	it("returns a stash-restore warning when pop conflicts with the agent commit", async () => {
		...
		expect(committedFiles.trim()).toBe("file.txt");
		expect(stashList).toContain("modular-isolation-");
	});
});
...
describe("commitToBranch preserves agent commits", () => {
	...
	beforeEach(async () => {
		parent = await fs.mkdtemp(path.join(os.tmpdir(), "modular-commit-parent-"));
		isolation = await fs.mkdtemp(path.join(os.tmpdir(), "modular-commit-iso-"));
	...
	it("preserves the agent's commit message after merge", async () => {
		...
		expect(result?.branchName).toBe(`modular/task/${taskId}`);
	...
	it("preserves every message when the agent makes multiple commits", async () => {
		...
		expect(result?.branchName).toBe("modular/task/multi");

		const merge = await mergeTaskBranches(parent, [
			{ branchName: result!.branchName!, taskId: "multi", baseSha: result!.baseSha! },
		]);
		expect(merge).toEqual({ failed: [], merged: ["modular/task/multi"] });
	...
	it("appends one trailing commit when the agent leaves uncommitted work after committing", async () => {
		...
		expect(result?.branchName).toBe("modular/task/leftover");
	...
	it("filters baseline WIP when the agent commits with git add -A", async () => {
		...
		expect(result?.branchName).toBe("modular/task/dirty-baseline");
		expect(aiMessage).not.toHaveBeenCalled();
		...
		expect(merge).toEqual({ failed: [], merged: ["modular/task/dirty-baseline"] });
	...
	it("falls back to the AI-generated message when the agent never committed", async () => {
		...
		expect(result?.branchName).toBe("modular/task/nocommit");
		expect(aiMessage).toHaveBeenCalledTimes(1);
	...
	describe("with baseline WIP overlapping the agent's changes (#4136)", () => {
		...
		it("commits an agent-only delta via --3way when WIP and agent modify unrelated hunks of the same tracked file", async () => {
			...
			expect(result?.branchName).toBe("modular/task/wip-tracked-file");
		...
		it("commits an untracked WIP file that the agent modifies inside isolation", async () => {
			...
			expect(result?.branchName).toBe("modular/task/wip-untracked");
		...
		it("commits a staged-new WIP file that the agent modifies inside isolation", async () => {
			...
			expect(result?.branchName).toBe("modular/task/wip-staged-new");
		...
		it("does not leak WIP-only files into the branch commit when the agent leaves them untouched", async () => {
			...
			expect(result?.branchName).toBe("modular/task/wip-filter");
		...
		it("still seeds WIP when the agent commits only baseline WIP and leaves the real edit uncommitted", async () => {
			...
			expect(result?.branchName).toBe("modular/task/wip-only-commit");
