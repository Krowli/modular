...
import { SPINNER_GLYPH_ADVANCE_MS, sharedSpinnerFrame } from "@modular/pi-coding-agent/modes/components/tool-execution";
import { getThemeByName } from "@modular/pi-coding-agent/modes/theme/theme";
import { getConfigRootDir, getCustomThemesDir, removeWithRetries, setAgentDir } from "@modular/pi-utils";
...
describe("theme symbols.spinnerFrames", () => {
	beforeEach(async () => {
		tmpAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-spinner-frames-"));
