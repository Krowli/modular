/**
 * Regression for https://github.com/Krowli/modular/issues/3031
 *
 ...
 * round-trips through `__modular_worker_mnemopi_embed`, and `SIGKILL`s the child
...
} from "@modular/pi-coding-agent/mnemopi/embed-client";
import type {
	MnemopiEmbedWorkerInbound,
	MnemopiEmbedWorkerOutbound,
} from "@modular/pi-coding-agent/mnemopi/embed-protocol";
...
describe("issue #3031 — mnemopi embeddings live in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		...
		// `modular --smoke-test`. Run it in a child Bun process instead of this
		...
		const script =
			'const { smokeTestMnemopiEmbedWorker } = await import("@modular/pi-coding-agent/mnemopi/embed-client"); await smokeTestMnemopiEmbedWorker({ timeoutMs: 15000 });';
