...
import { parseArgs, reportUnrecognizedFlags } from "@modular/pi-coding-agent/cli/args";
import { applyExtensionFlags } from "@modular/pi-coding-agent/cli/extension-flags";

// Regression coverage for issue #2459: `modular --list-models` (a stale flag) was
...
describe("parseArgs — unrecognized flag tracking (#2459)", () => {
	...
	it("treats every token after `--` as a positional, even flag-shaped ones (#2461 review)", () => {
		// `modular -p -- --explain-this` must forward `--explain-this` as the
