...
import { shouldSkipHistory } from "@modular/pi-coding-agent/modes/controllers/input-controller";
...
describe("shouldSkipHistory — security filter for slash command history", () => {
	...
	it("skips /join with a link argument (carries 32-byte room key and write token)", () => {
		expect(shouldSkipHistory("/join modular://share/abc123def456...")).toBe(true);
		expect(shouldSkipHistory("/join modular:abc123def456...")).toBe(true);
	});
