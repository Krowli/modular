...
import { BUILTIN_SLASH_COMMANDS } from "@modular/pi-coding-agent/slash-commands/builtin-registry";
...
describe("/move directory completion", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-move-completion-"));
	...
	it("completes parent directory paths", async () => {
		...
		const siblingName = `modular-move-sibling-${path.basename(tempDir)}`;
