...
import { RawSseDebugBuffer } from "@modular/pi-coding-agent/debug/raw-sse-buffer";
import { createReportBundle } from "@modular/pi-coding-agent/debug/report-bundle";
...
describe("raw SSE report bundle", () => {
	it("includes captured raw SSE text and dropped-record disclosure", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "modular-raw-sse-report-"));
		const xdgStateHome = path.join(cleanupRoot, "state");
		await fs.mkdir(path.join(xdgStateHome, "modular"), { recursive: true });
		...
		expect(rawSseText).toContain(": modular-debug-dropped records=");
