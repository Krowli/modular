...
import { Agent, AgentBusyError, type AgentMessage, type AgentTool } from "@modular/pi-agent-core";
import type { AssistantMessage, Message, ToolCall } from "@modular/pi-ai";
import { createMockModel } from "@modular/pi-ai/providers/mock";
import { AssistantMessageEventStream } from "@modular/pi-ai/utils/event-stream";
import { getBundledModel } from "@modular/pi-catalog/models";
import { AsyncJobManager } from "@modular/pi-coding-agent/async";
import type { Rule } from "@modular/pi-coding-agent/capability/rule";
import { ModelRegistry } from "@modular/pi-coding-agent/config/model-registry";
import { type SettingPath, Settings } from "@modular/pi-coding-agent/config/settings";
import { TtsrManager } from "@modular/pi-coding-agent/export/ttsr";
import type { ExtensionRunner } from "@modular/pi-coding-agent/extensibility/extensions";
...
import { convertToLlm } from "@modular/pi-coding-agent/session/messages";
import { SessionManager } from "@modular/pi-coding-agent/session/session-manager";
import { removeSyncWithRetries, Snowflake } from "@modular/pi-utils";
...
describe("AgentSession concurrent prompt guard", () => {
	...
	// already processing` from modular-rpc clients (robodular triage reminder path).
...
describe("AgentSession TTSR resume gate", () => {
	...
	it("relativizes the rule file path in the TTSR interrupt injection (no absolute leak)", async () => {
		...
		const ruleAbsPath = path.join(cwd, ".modular", "rules", "no-unwrap.md");
