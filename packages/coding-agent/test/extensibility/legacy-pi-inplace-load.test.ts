...
async function writePackage(files: Record<string, string>): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-legacy-inplace-"));
	tempRoots.push(dir);
...
describe("legacy-pi in-place module loading (issue #1674)", () => {
	...
	it("loads the extension's own node_modules deps natively while remapping legacy pi imports", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				...
				// `@modular/*` is a fork alias with no real published package,
	...
	it("honors legacy bash operations overrides", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				'import { createBashToolDefinition } from "@modular/pi-coding-agent";',
				"const updates = [];",
				"let captured;",
	...
	it("preserves relative paths from legacy find operations", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				'import { createFindToolDefinition } from "@modular/pi-coding-agent";',
	...
	it("remaps legacy pi-ai utils/oauth subpaths to registry OAuth exports", async () => {
		const dir = await writePackage({
			...
			"index.ts": [
				'import { registerOAuthProvider } from "@modular/pi-ai/utils/oauth";',
				'import { refreshAnthropicToken } from "@modular/pi-ai/utils/oauth/anthropic";',
	...
	it("rewrites legacy imports in ../src modules reached through relative imports", async () => {
		const dir = await writePackage({
			...
			"src/helper.ts": [
				'import { isCompiledBinary } from "@modular/pi-utils";',
	...
	it("does not rewrite sibling files outside the loaded extension's import graph", async () => {
		const dir = await writePackage({
			...
			// `@modular/*` only resolves via the rewrite, so an un-rewritten
		...
		await expect(import(siblingUrl)).rejects.toThrow(/@modular\/pi-ai/);
	});
});
