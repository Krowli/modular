...
import {
	...
	getBundledProviders,
	modelsAreEqual,
} from "@modular/pi-catalog/models";
import {
	__resetLegacyPiResolutionCache,
	installLegacyPiSpecifierShim,
	loadLegacyPiModule,
} from "@modular/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
import { Type as TypeBoxShimType } from "@modular/pi-coding-agent/extensibility/typebox";
import { removeWithRetries } from "@modular/pi-utils";

// pi-ai 15.1.0 removed the runtime `Type` export from `@modular/pi-ai`'s
// package root. Legacy extensions (and their aliased-scope variants such as
// `@modular/pi-ai`) still author parameter schemas as
// `import { Type } from "@modular/pi-ai"` and then `Type.Object(...)`.
...
async function writeFixtureExtension(source: string): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-pi-ai-type-remap-"));
	tempRoots.push(dir);
...
describe("legacy-pi @(scope)/pi-ai root `Type` remap (issue #1437)", () => {
	it('redirects `import { Type } from "@modular/pi-ai"` to the TypeBox shim', async () => {
		const entry = await writeFixtureExtension(
			[
				'import { Type } from "@modular/pi-ai";',
	...
	it('redirects `import { Type } from "@modular/pi-ai"` for plugins published against the canonical scope', async () => {
		const entry = await writeFixtureExtension(
			['import { Type } from "@modular/pi-ai";', "export const probe = Type;"].join("\n"),
		);
	...
	it("preserves canonical pi-ai exports alongside the shimmed Type (z is still re-exported)", async () => {
		const entry = await writeFixtureExtension(
			[
				'import { Type, z } from "@modular/pi-ai";',
	...
	it("does not redirect subpath imports such as @modular/pi-ai/utils/schema", async () => {
		const entry = await writeFixtureExtension(
			[
				...
				'import { zodToWireSchema } from "@modular/pi-ai/utils/schema";',
	...
	it("exports getModel as getBundledModel", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				'import { getModel } from "@modular/pi-ai"; export const testGetModel = getModel;',
			),
	...
	it("exports getModels as getBundledModels", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				'import { getModels } from "@modular/pi-ai"; export const testGetModels = getModels;',
			),
		)) as { testGetModels: unknown };
		expect(loaded.testGetModels).toBe(getBundledModels);
	});

	it("re-exports calculateCost from @modular/pi-catalog/models (issue #4584)", async () => {
		// `calculateCost` was moved from the `@modular/pi-ai` barrel to
		// `@modular/pi-catalog/models` in the catalog split. Legacy extensions
		...
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				'import { calculateCost } from "@modular/pi-ai"; export const probe = calculateCost;',
			),
		)) as { probe: unknown };
		expect(loaded.probe).toBe(calculateCost);
	});

	it("re-exports modelsAreEqual and getBundledProviders from @modular/pi-catalog/models", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				[
					'import { modelsAreEqual, getBundledProviders } from "@modular/pi-ai";',
	...
	it("re-exports getBundledModel and getBundledModels from @modular/pi-catalog/models", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				[
					'import { getBundledModel, getBundledModels } from "@modular/pi-ai";',
	...
	it("exports StringEnum as a schema builder with options support", async () => {
		const loaded = (await loadLegacyPiModule(
			await writeFixtureExtension(
				[
					'import { StringEnum } from "@modular/pi-ai";',
...
describe("legacy pi package root remaps (issue #1474)", () => {
	it("loads @modular/pi-coding-agent root imports when host package resolution is unavailable", async () => {
		const realResolveSync = Bun.resolveSync.bind(Bun);
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@modular/pi-coding-agent" && from.endsWith(path.join("src", "extensibility", "plugins"))) {
				throw new Error("compiled binary host package resolution unavailable");
			}
			return realResolveSync(specifier, from);
		});
		const entry = await writeFixtureExtension(
			['import { VERSION } from "@modular/pi-coding-agent";', "export const loadedVersion = VERSION;"].join("\n"),
		);
	...
	it("preserves legacy defineTool root imports and usable coding tools", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-legacy-coding-tools-"));
		tempRoots.push(dir);
		...
		await fs.writeFile(
			...
			[
				...
				'import { createCodingTools, defineTool, Type } from "@modular/pi-coding-agent";',
				"const definition = {",
	...
	it("preserves legacy frontmatter helper root imports", async () => {
		const entry = await writeFixtureExtension(
			[
				'import { parseFrontmatter, stripFrontmatter } from "@modular/pi-coding-agent";',
	...
	it("falls back to legacy-scoped subpath peers for direct plugin imports", async () => {
		const realResolveSync = Bun.resolveSync.bind(Bun);
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@modular/pi-ai/oauth") {
				throw new Error(`canonical peer unavailable from ${from}`);
			}
			return realResolveSync(specifier, from);
		});

		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-legacy-direct-subpath-"));
		tempRoots.push(dir);
		const packageDir = path.join(dir, "node_modules", "@modular", "pi-ai");
		...
		await fs.writeFile(
			entry,
			['import { marker } from "@modular/pi-ai/oauth";', "export const loadedMarker = marker;"].join("\n"),
			"utf8",
		);
	...
	it("routes @modular/pi-utils through canonical Bun.resolveSync in non-compiled mode", async () => {
		// Regression: when modular runs from a node_modules install (not the monorepo
		// and not a compiled binary), the bundled packages live at
		// `node_modules/@modular/pi-*`, not next to the source tree. Hardcoding
		...
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@modular/pi-utils") {
				canonicalLookupSeen = true;
			}
			return realResolveSync(specifier, from);
		});
		const entry = await writeFixtureExtension(
			['import { isCompiledBinary } from "@modular/pi-utils";', "export const probe = isCompiledBinary;"].join("\n"),
		);
