...
import { __buildLegacyPiPackageRootOverrides } from "@modular/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
...
// failed to resolve `@modular/pi-ai/oauth` because the override map
...
// the same `modular-legacy-pi-bundled:` virtual namespace as package roots without
// a generated registry or duplicate key list.
describe("legacy pi compat compiled-mode subpath overrides (issue #3442)", () => {
	it("serves @modular/pi-ai/oauth through the bundled virtual namespace in compiled mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@modular/pi-ai/oauth"]).toBe("modular-legacy-pi-bundled:@modular/pi-ai/oauth");
	});
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		// from `@modular/pi-ai/utils/oauth/anthropic`) still hit the bunfs
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@modular/pi-ai/oauth/anthropic"]).toBe(
			"modular-legacy-pi-bundled:@modular/pi-ai/oauth/anthropic",
		);
		// Sanity: the wildcard expansion also reaches deeper subroots so plugins
		// pinned to e.g. `@modular/pi-ai/providers/openai` keep resolving.
		expect(bundledModuleKeys.has("@modular/pi-ai/oauth/anthropic")).toBe(true);
		expect(bundledModuleKeys.has("@modular/pi-ai/oauth/openai-codex")).toBe(true);
	});

	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@modular/pi-coding-agent/web/search/providers/xai",
			"@modular/pi-coding-agent/web/search/providers/tinyfish",
			"@modular/pi-coding-agent/web/search/providers/firecrawl",
			"@modular/pi-coding-agent/web/search/providers/duckduckgo",
		] as const;

		for (const key of providerKeys) {
			expect(bundledModuleKeys.has(key)).toBe(true);
			expect(overrides[key]).toBe(`modular-legacy-pi-bundled:${key}`);
		}
	});
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@modular/pi-coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@modular/pi-coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@modular/pi-coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@modular/pi-coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("maps every bundled key (minus shimmed roots + typebox) to its virtual specifier in compiled mode", () => {
		...
		for (const key of bundledModuleKeys) {
			...
			if (key === "@modular/pi-ai" || key === "@modular/pi-coding-agent" || key === "typebox") continue;
			if (overrides[key] !== `modular-legacy-pi-bundled:${key}`) {
				missing.push(key);
			}
		}
	...
	it("keeps pi-ai/pi-coding-agent roots routed to their compat shims in compiled mode", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@modular/pi-ai"]).toBeDefined();
		expect(overrides["@modular/pi-ai"]).not.toBe("modular-legacy-pi-bundled:@modular/pi-ai/oauth");
		expect(overrides["@modular/pi-coding-agent"]).toBeDefined();
	});

	it("does not register subpath overrides in dev/install mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(false);
		expect(overrides).not.toHaveProperty("@modular/pi-ai/oauth");
		expect(overrides).not.toHaveProperty("@modular/pi-coding-agent/tools");
