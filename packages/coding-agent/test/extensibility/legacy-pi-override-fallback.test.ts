...
// compiled-binary mode now routes through `modular-legacy-pi-bundled:` virtual
...
describe("legacy pi compat package-root override validation (issue #2168)", () => {
	it("keeps overrides whose filesystem targets exist", () => {
		const candidates = {
			"@modular/pi-ai": "/tmp/exists-ai.js",
			"@modular/pi-utils": "/tmp/exists-utils.js",
		};
	...
	it("drops overrides whose filesystem targets are missing on disk", () => {
		const candidates = {
			"@modular/pi-ai": "/tmp/exists-ai.js",
			"@modular/pi-coding-agent": "/tmp/exists-shim.js",
			"@modular/pi-utils": "/$bunfs/root/packages/utils/src/index.js",
			"@modular/pi-tui": "/$bunfs/root/packages/tui/src/index.js",
		};
		...
		expect(result).not.toHaveProperty("@modular/pi-utils");
		expect(result).not.toHaveProperty("@modular/pi-tui");
	});
	...
	it("keeps virtual modular-legacy-pi-bundled: entries without touching the filesystem (issue #3423)", () => {
		...
		let probed = false;
		const candidates = {
			"@modular/pi-ai": "modular-legacy-pi-bundled:@modular/pi-ai",
			"@modular/pi-coding-agent": "modular-legacy-pi-bundled:@modular/pi-coding-agent",
			"@modular/pi-agent-core": "modular-legacy-pi-bundled:@modular/pi-agent-core",
			"@modular/pi-natives": "modular-legacy-pi-bundled:@modular/pi-natives",
			"@modular/pi-tui": "modular-legacy-pi-bundled:@modular/pi-tui",
			"@modular/pi-utils": "modular-legacy-pi-bundled:@modular/pi-utils",
		};
	...
	it("mixes virtual and filesystem entries: virtuals always pass, filesystems gated", () => {
		const candidates = {
			"@modular/pi-ai": "modular-legacy-pi-bundled:@modular/pi-ai",
			"@modular/pi-coding-agent": "/dev/source/legacy-pi-coding-agent-shim.ts",
			"@modular/pi-tui": "/missing/path.ts",
		};
...
describe("legacy pi compat typebox shim path resolution (issues #3414, #3423)", () => {
	it("returns the virtual specifier in compiled-binary mode without touching the filesystem", () => {
		...
		expect(result).toBe("modular-legacy-pi-bundled:typebox");
		expect(probed).toBe(false);
	});
