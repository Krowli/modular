...
import type { ObservableSession } from "@modular/pi-coding-agent/modes/session-observer-registry";
...
import type { TUI } from "@modular/pi-tui";
...
function makeSubagentRegistry(sessions: ObservableSession[]) {
	...
	} as unknown as import("@modular/pi-coding-agent/modes/session-observer-registry").SessionObserverRegistry;
}
...
describe("Agent hub silent-abort regression", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-overlay-test-"));
	});
