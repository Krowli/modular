...
} from "@modular/pi-coding-agent/system-prompt";
...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".modular");
	...
	it("suppresses discovered SYSTEM.md while preserving the project footer", async () => {
		...
		fs.mkdirSync(path.join(projectDir, ".modular"), { recursive: true });
		fs.writeFileSync(path.join(projectDir, ".modular", "SYSTEM.md"), "Discovered project SYSTEM prompt");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".modular"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".modular", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".modular", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".modular", "SYSTEM.md"), "Project SYSTEM prompt");
