/**
 * Routing tests for `modular plugin install <local-path>` (#1945).
 *
...
import { runPluginCommand } from "@modular/pi-coding-agent/cli/plugin-cli";
import { PluginManager } from "@modular/pi-coding-agent/extensibility/plugins/manager";
import { MarketplaceManager } from "@modular/pi-coding-agent/extensibility/plugins/marketplace";
import type { InstalledPlugin } from "@modular/pi-coding-agent/extensibility/plugins/types";
...
async function createLocalPlugin(root: string): Promise<string> {
	...
	await Bun.write(
		...
		JSON.stringify({
			name: "kimi-datasource",
			version: "1.0.0",
			modular: { extensions: ["./src/extension.ts"] },
		}),
	);
	return localPlugin;
}
...
describe("runPluginCommand({ action: 'install', args: [<local>] })", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "modular-plugin-install-local-"));
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "modular-plugins.lock.json"));
	...
	test("real local plugin directory: install symlinks it like link would", async () => {
		...
		// symlink rather than copy-install, matching `modular plugin link`.
	...
	test("doctor --fix preserves linked local plugin state without package dependencies", async () => {
		await Bun.write(
			...
			JSON.stringify({ name: "modular-plugins", private: true, dependencies: {} }),
		);
		const localPlugin = await createLocalPlugin(tmpRoot);
