...
import type { Args } from "@modular/pi-coding-agent/cli/args";
import type { Settings } from "@modular/pi-coding-agent/config/settings";
import { createSessionManager } from "@modular/pi-coding-agent/main";
import type { SessionHeader } from "@modular/pi-coding-agent/session/session-entries";
import type { SessionInfo } from "@modular/pi-coding-agent/session/session-listing";
import * as sessionListingModule from "@modular/pi-coding-agent/session/session-listing";
import { loadEntriesFromFile } from "@modular/pi-coding-agent/session/session-loader";
...
describe("createSessionManager — cross-project --resume cancellation (#1668)", () => {
	...
	beforeEach(async () => {
		existingProject = await fsp.mkdtemp(path.join(os.tmpdir(), "modular-xproj-"));
	});
...
describe("createSessionManager — cross-project --resume relocation (moved worktree)", () => {
	...
	beforeEach(async () => {
		missingRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "modular-moved-xproj-"));
