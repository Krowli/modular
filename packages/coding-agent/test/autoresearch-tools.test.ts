...
import { createSessionRuntime } from "@modular/pi-coding-agent/autoresearch/state";
import {
	type AutoresearchStorage,
	closeAllAutoresearchStorages,
	openAutoresearchStorage,
	type SessionRow,
} from "@modular/pi-coding-agent/autoresearch/storage";
import { createInitExperimentTool } from "@modular/pi-coding-agent/autoresearch/tools/init-experiment";
import { createLogExperimentTool } from "@modular/pi-coding-agent/autoresearch/tools/log-experiment";
import { createRunExperimentTool } from "@modular/pi-coding-agent/autoresearch/tools/run-experiment";
import { createUpdateNotesTool } from "@modular/pi-coding-agent/autoresearch/tools/update-notes";
import type { ASIData, LogDetails, NumericMetricMap, RunDetails } from "@modular/pi-coding-agent/autoresearch/types";
import type { ExtensionAPI, ExtensionContext } from "@modular/pi-coding-agent/extensibility/extensions";
...
describe("init_experiment", () => {
	...
	beforeEach(() => {
		...
		process.env.MODULAR_AUTORESEARCH_DB_DIR = dbOverride.path();
	});

	afterEach(async () => {
		delete process.env.MODULAR_AUTORESEARCH_DB_DIR;
		closeAllAutoresearchStorages();
		await Bun.sleep(0);
