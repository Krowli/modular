...
import { runRootCommand } from "@modular/pi-coding-agent/main";
...
describe("parseArgs — --max-time flag", () => {
	...
	it("reports invalid --max-time values as CLI usage errors", async () => {
		...
		expect(stderr).toContain("Run `mod --help` for available flags.");
	...
	it("converts maxTime to an absolute session deadline", async () => {
		using tempDir = TempDir.createSync("@modular-max-time-");
