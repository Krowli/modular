...
import { type SkillPromptDetails, stripInternalDetailsFields } from "@modular/pi-coding-agent/session/messages";
import type { CustomMessageEntry } from "@modular/pi-coding-agent/session/session-entries";
	...
	it("F1: strips __queueChipText from persisted details while preserving all other SkillPromptDetails fields", () => {
		...
		const id = session.appendCustomMessageEntry<SkillPromptDetails>(
			...
			true,
			{
				...
				args: "bar",
				lineCount: 10,
				__queueChipText: "modular-cmd-1-0",
			},
			"user",
		);
