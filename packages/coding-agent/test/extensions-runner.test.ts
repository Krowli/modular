...
import type { AgentMessage, AgentTool } from "@modular/pi-agent-core";
...
} from "@modular/pi-coding-agent/extensibility/extensions/runner";
import { ExtensionToolWrapper } from "@modular/pi-coding-agent/extensibility/extensions/wrapper";
import { Type } from "@modular/pi-coding-agent/extensibility/typebox";
...
import { getProjectAgentDir, logger, TempDir } from "@modular/pi-utils";
...
describe("ExtensionRunner", () => {
	...
	describe("memory context", () => {
		it("exposes the lazy memory runtime after initialization", async () => {
			const extCode = `
				export default function(pi) {
					pi.on("session_start", async (_event, ctx) => {
						globalThis.__modMemoryStatus = await ctx.memory.status();
					});
				}
			`;
			const explicitExtensionPath = path.join(tempDir.path(), "memory-context.ts");
			fs.writeFileSync(explicitExtensionPath, extCode);
			const globalState = globalThis as typeof globalThis & { __modMemoryStatus?: unknown };
			delete globalState.__modMemoryStatus;
			...
			expect(globalState.__modMemoryStatus).toMatchObject({
				backend: "mnemopi",
				active: true,
				searchable: true,
			});
