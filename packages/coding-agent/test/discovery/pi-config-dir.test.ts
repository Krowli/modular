...
import { getConfigDirs } from "@modular/pi-coding-agent/config";
import { getUserPath } from "@modular/pi-coding-agent/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getUserPath resolves the native user scope via getAgentDir (profile-aware)", () => {
		...
		// The old behavior joined ctx.home + ".modular/agent" and leaked the default
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		process.env.PI_CONFIG_DIR = ".config/modular";
		const result = getConfigDirs("commands", { project: false });
		const expected = path.resolve(path.join(os.homedir(), ".config/modular", "agent", "commands"));
		expect(result[0]).toEqual({ path: expected, source: ".modular", level: "user" });
	});
});
