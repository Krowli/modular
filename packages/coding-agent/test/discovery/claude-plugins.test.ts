...
import { loadCapability } from "@modular/pi-coding-agent/capability";
...
import { loadSlashCommands } from "@modular/pi-coding-agent/extensibility/slash-commands";
import { discoverAgents } from "@modular/pi-coding-agent/task/discovery";
import { removeWithRetries } from "@modular/pi-utils";
import "@modular/pi-coding-agent/discovery/claude-plugins";
import { type MCPServer, mcpCapability } from "@modular/pi-coding-agent/capability/mcp";
import type { Skill } from "@modular/pi-coding-agent/capability/skill";
import type { SlashCommand } from "@modular/pi-coding-agent/capability/slash-command";
...
describe("listClaudePluginRoots", () => {
	...
	test("expands env placeholders in marketplace plugin MCP url and headers", async () => {
		...
		const originalApiKey = process.env.MODULAR_PLUGIN_MCP_API_KEY;
		const originalUrl = process.env.MODULAR_PLUGIN_MCP_URL;
		const envPlaceholder = (name: string): string => ["$", "{", name, ":-}"].join("");
		process.env.MODULAR_PLUGIN_MCP_API_KEY = "ctx7sk-test-key";
		process.env.MODULAR_PLUGIN_MCP_URL = "https://mcp.context7.example";
		...
		try {
			...
			await fs.writeFile(
				...
				JSON.stringify({
					context7: {
						type: "http",
						url: `${envPlaceholder("MODULAR_PLUGIN_MCP_URL")}/mcp`,
						headers: {
							CONTEXT7_API_KEY: envPlaceholder("MODULAR_PLUGIN_MCP_API_KEY"),
						},
					},
				}),
			);
			...
			if (originalApiKey === undefined) delete process.env.MODULAR_PLUGIN_MCP_API_KEY;
			else process.env.MODULAR_PLUGIN_MCP_API_KEY = originalApiKey;
			if (originalUrl === undefined) delete process.env.MODULAR_PLUGIN_MCP_URL;
			else process.env.MODULAR_PLUGIN_MCP_URL = originalUrl;
		}
	});
