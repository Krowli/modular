/**
 * Regression: MODULAR-native user-level config discovery must follow the active
 * profile. A profile relocates the agent directory to ~/.modular/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.modular/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".modular/agent") points at the tempdir decoys below; without the fix
...
import { type Skill, skillCapability } from "@modular/pi-coding-agent/capability/skill";
import { type SlashCommand, slashCommandCapability } from "@modular/pi-coding-agent/capability/slash-command";
...
describe("native user-level config discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "modular-profile-iso-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-profile-iso-project-"));
		profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-profile-iso-agent-"));
