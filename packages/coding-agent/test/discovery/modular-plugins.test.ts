 ...
 *
 * The native `modular` discovery provider only walks `.modular/` and `~/.modular/agent/`.
 * Extension packages registered via `extensions:` in settings or
 * `--extension` on the CLI ship their own `skills/`, `hooks/`, `tools/`,
 * `commands/`, `rules/`, `prompts/`, and `.mcp.json`. The `modular-plugins`
 * provider (`src/discovery/modular-plugins.ts`) is what wires those sub-trees
...
import { getCapability } from "@modular/pi-coding-agent/capability";
import { clearCache } from "@modular/pi-coding-agent/capability/fs";
import { hookCapability } from "@modular/pi-coding-agent/capability/hook";
import { mcpCapability } from "@modular/pi-coding-agent/capability/mcp";
import { promptCapability } from "@modular/pi-coding-agent/capability/prompt";
import { ruleCapability } from "@modular/pi-coding-agent/capability/rule";
import { skillCapability } from "@modular/pi-coding-agent/capability/skill";
import { slashCommandCapability } from "@modular/pi-coding-agent/capability/slash-command";
import { toolCapability } from "@modular/pi-coding-agent/capability/tool";
import type { LoadContext, Provider } from "@modular/pi-coding-agent/capability/types";
// Register all discovery providers as a side effect.
import "@modular/pi-coding-agent/discovery";
import {
	clearOmpExtensionCliRoots,
	injectOmpExtensionCliRoots,
} from "@modular/pi-coding-agent/discovery/modular-extension-roots";
import { getConfigRootDir, removeSyncWithRetries, setAgentDir } from "@modular/pi-utils";

const PROVIDER_ID = "modular-plugins";
...
function buildExtensionPackage(packageDir: string): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), modular: { extensions: ["./src/main.ts"] } }),
	);
...
beforeEach(() => {
	clearCache();
	clearOmpExtensionCliRoots();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-plugins-"));
	...
	setAgentDir(path.join(home, ".modular", "agent"));
});
...
test("project settings.json#extensions surfaces every sub-directory", async () => {
	writeFile(path.join(project, ".modular", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("user settings.json#extensions also feeds sub-discovery", async () => {
	writeFile(path.join(home, ".modular", "agent", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("file-extension entrypoints contribute zero sub-surface (the file has no siblings to scan)", async () => {
	...
	writeFile(path.join(project, ".modular", "settings.json"), JSON.stringify({ extensions: [standaloneFile] }));
...
test("relative paths in settings resolve against the project cwd", async () => {
	...
	writeFile(path.join(project, ".modular", "settings.json"), JSON.stringify({ extensions: [`./${relative}`] }));
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `modular plugin link`/`install`)", async () => {
	...
	const pluginsDir = path.join(home, ".modular", "plugins");
	...
	writeFile(
		...
		JSON.stringify({ name: "modular-plugins", dependencies: { "my-installed-ext": "1.0.0" } }),
	);
	// Plugin's own package.json must carry an `modular`/`pi` manifest for the
	// loader to recognise it; the buildExtensionPackage fixture already wrote
	// one with `modular.extensions`, which is sufficient.
...
test("project-scoped installed plugins surface project-level sub-discovery", async () => {
	const pluginsDir = path.join(project, ".modular", "plugins");
	const installed = path.join(pluginsDir, "node_modules", "my-project-ext");
	fs.mkdirSync(installed, { recursive: true });
	fs.cpSync(ext, installed, { recursive: true });
	writeFile(
		path.join(pluginsDir, "modular-plugins.lock.json"),
		JSON.stringify({
...
test("disabled installed plugins do not contribute sub-discovery", async () => {
	...
	writeFile(
		path.join(pluginsDir, "package.json"),
		JSON.stringify({ name: "modular-plugins", dependencies: { "my-disabled-ext": "1.0.0" } }),
	);
	writeFile(
		path.join(pluginsDir, "modular-plugins.lock.json"),
...
test("linked plugins (only in lockfile, not in package.json#dependencies) are surfaced", async () => {
	// `modular plugin link ./local-ext` creates a symlink under
	...
	// still find the package — otherwise the documented `modular install
