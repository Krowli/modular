...
import type { SessionManager } from "@modular/pi-coding-agent/session/session-manager";
import { executeAcpBuiltinSlashCommand } from "@modular/pi-coding-agent/slash-commands/acp-builtins";
import { removeWithRetries, setProjectDir } from "@modular/pi-utils";
...
describe("ACP builtin slash commands", () => {
	...
	it("dump: outputs transcript with LLM request JSON path when sidecar succeeds", async () => {
		...
		runtime.session.dumpLlmRequestToTmpDir = async () => "/tmp/modular-llm-request-test.json";

		const result = await executeAcpBuiltinSlashCommand("/dump", runtime);

		expect(result).toEqual({ consumed: true });
		expect(output[0]).toContain("Session content here");
		expect(output[0]).toContain("LLM request JSON: /tmp/modular-llm-request-test.json");
...
describe("wave 3 commands", () => {
	...
	it("/move: relocates the current session instead of switching to an empty target session", async () => {
		...
		const targetDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-move-target-"));
...
describe("wave 5 — adapters and polish", () => {
	...
	it("/mcp add foo --url https://example.com --token X --scope project: outputs success or propagates write error", async () => {
		// Uses project scope so it writes to /tmp/project/.modular/mcp.json which test infra controls.
		// We verify the command either reports success or a meaningful error (not a parse error).
		const mcpModule = await import("@modular/pi-coding-agent/mcp/config-writer");
	...
	it("/ssh add foo --host x --user y --scope user: calls addSSHHost", async () => {
		const sshModule = await import("@modular/pi-coding-agent/ssh/config-writer");
	...
	it("/marketplace discover: output is bulleted with '  - ' token", async () => {
		const { MarketplaceManager } = await import("@modular/pi-coding-agent/extensibility/plugins/marketplace");
