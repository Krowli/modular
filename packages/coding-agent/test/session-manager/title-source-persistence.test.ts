...
import { FileSessionStorage, type WriteTextAtomicOptions } from "@modular/pi-coding-agent/session/session-storage";
import type { SessionTitleUpdate } from "@modular/pi-coding-agent/session/session-title-slot";
...
describe("session title source persistence", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-title-source-"));
