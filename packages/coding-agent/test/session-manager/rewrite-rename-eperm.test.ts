...
import { recoverOrphanedBackups } from "@modular/pi-coding-agent/session/session-listing";
import { SessionManager } from "@modular/pi-coding-agent/session/session-manager";
import { FileSessionStorage, MemorySessionStorage } from "@modular/pi-coding-agent/session/session-storage";
...
describe("SessionManager rewrite EPERM replacement fallback", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "modular-eperm-"));
	});
...
describe("FileSessionStorage.writeTextAtomic commitGuard cleanup", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "modular-guard-cleanup-"));
	});
