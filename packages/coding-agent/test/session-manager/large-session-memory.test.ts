...
import { listSessions } from "@modular/pi-coding-agent/session/session-listing";
...
import { MemorySessionStorage } from "@modular/pi-coding-agent/session/session-storage";
...
describe("large session memory guards", () => {
	...
	it("streams large session files and keeps only the latest compaction summary", async () => {
		const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "modular-large-session-"));
	...
	it("only elides loaded compactions on the active branch", async () => {
		const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), "modular-branch-load-"));
		tempDirs.push(tempDir);
