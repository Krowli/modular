...
import type { FileEntry, SessionHeader } from "@modular/pi-coding-agent/session/session-entries";
import { findMostRecentSession, resolveResumableSession } from "@modular/pi-coding-agent/session/session-listing";
...
import { getConfigRootDir, getSessionsDir, removeSyncWithRetries, Snowflake, setAgentDir } from "@modular/pi-utils";
...
describe("SessionManager temp cwd session dirs", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-session-dir-test-"));
...
describe("SessionManager legacy session migration persistence", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-session-manager-legacy-"));
	});
