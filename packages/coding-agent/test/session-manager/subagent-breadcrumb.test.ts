...
import { readTerminalBreadcrumbEntry } from "@modular/pi-coding-agent/session/session-paths";
...
describe("SessionManager subagent breadcrumb isolation", () => {
	...
	beforeEach(async () => {
		...
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "modular-subagent-crumb-"));
