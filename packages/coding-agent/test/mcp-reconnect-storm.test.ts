...
import type { MCPStdioServerConfig } from "@modular/pi-coding-agent/mcp/types";
...
describe("MCP reconnect storm (issue #1592)", () => {
	...
	beforeEach(() => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-mcp-storm-"));
	...
	it("stops respawning after a burst of immediate exits", async () => {
		...
		const config: MCPStdioServerConfig = {
			...
			command: BUN_EXEC,
			args: [FIXTURE_PATH],
			env: { MODULAR_TEST_SPAWN_LOG: spawnLog },
		};
