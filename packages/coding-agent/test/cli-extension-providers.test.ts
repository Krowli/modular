 ...
 *
 * One-shot CLIs (`modular bench`, dry-balance) build a bare `ModelRegistry` that
 ...
 * `~/.modular/agent/extensions/`) was invisible to model resolution and
 * `modular bench <provider>/<model>` failed with "Model not found".
 *
 ...
 * passed explicitly so the test never touches the developer's real `~/.modular`.
 */

import { afterAll, beforeAll, expect, test } from "bun:test";
import * as fs from "node:fs/promises";
import { AuthStorage } from "@modular/pi-ai";
import { ModelRegistry } from "@modular/pi-coding-agent/config/model-registry";
import { getModelMatchPreferences, resolveCliModel } from "@modular/pi-coding-agent/config/model-resolver";
import { resetSettingsForTest, Settings } from "@modular/pi-coding-agent/config/settings";
import { loadCliExtensionProviders } from "@modular/pi-coding-agent/sdk";
