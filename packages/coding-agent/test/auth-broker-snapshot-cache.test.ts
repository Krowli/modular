...
} from "@modular/pi-ai/auth-broker";
import { discoverAuthStorage } from "@modular/pi-coding-agent/sdk";
import { removeWithRetries } from "@modular/pi-utils";

const ENV_KEYS = [
	"MODULAR_AUTH_BROKER_URL",
	"MODULAR_AUTH_BROKER_TOKEN",
	"MODULAR_AUTH_BROKER_SNAPSHOT_CACHE",
	"MODULAR_AUTH_BROKER_SNAPSHOT_TTL_MS",
] as const;
...
describe("discoverAuthStorage auth-broker snapshot cache", () => {
	...
	test("boots from a fresh encrypted cache when the broker is down", async () => {
		...
		process.env.MODULAR_AUTH_BROKER_URL = downUrl;
		process.env.MODULAR_AUTH_BROKER_TOKEN = TOKEN;
		process.env.MODULAR_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
		process.env.MODULAR_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
