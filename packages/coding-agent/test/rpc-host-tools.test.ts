...
import type { AgentEvent } from "@modular/pi-agent-core";
import { defineRpcClientTool, RpcClient } from "@modular/pi-coding-agent/modes";
import { RpcHostToolBridge } from "@modular/pi-coding-agent/modes/rpc/host-tools";
import type {
	RpcHostToolCallRequest,
	RpcHostToolCancelRequest,
	RpcHostToolUpdate,
} from "@modular/pi-coding-agent/modes/rpc/rpc-types";
...
describe("RpcClient custom tools", () => {
	it("registers host custom tools and serves tool calls over the RPC transport", async () => {
		const scriptPath = path.join(os.tmpdir(), `modular-rpc-host-tools-${Date.now()}.js`);
