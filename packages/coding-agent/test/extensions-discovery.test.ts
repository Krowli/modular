...
import { type ExtensionModule, extensionModuleCapability } from "@modular/pi-coding-agent/capability/extension-module";
import { resetSettingsForTest, Settings } from "@modular/pi-coding-agent/config/settings";
import { getCapability, initializeWithSettings } from "@modular/pi-coding-agent/discovery";
import {
	discoverAndLoadExtensions,
	discoverExtensionPaths,
	loadExtensions,
} from "@modular/pi-coding-agent/extensibility/extensions/loader";
...
describe("extensions discovery", () => {
	...
	it("discovers a symlinked extension directory with a package.json manifest", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ name: "ctk", modular: { extensions: ["./index.ts"] } }),
		);
