...
import { type Skill as CapabilitySkill, skillCapability } from "@modular/pi-coding-agent/capability/skill";
import { getCapability } from "@modular/pi-coding-agent/discovery";
import {
	loadSkills,
	loadSkillsFromDir,
	parseSkillInvocation,
	type Skill,
} from "@modular/pi-coding-agent/extensibility/skills";
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		// canonical MODULAR-native `~/.agent[s]/skills` (the `agents` provider).
		...
		// MUST NOT consider the MODULAR-native `enableAgentsUser`/`...Project`
		...
		it("should skip skills disabled via frontmatter", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-disabled-skill-"));
...
`,
		...
		it("should hide skills with disable-model-invocation frontmatter (Agent Skills spec)", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-dmi-skill-"));
