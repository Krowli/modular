 ...
 * `process.exit()` at the top level must not terminate the host MODULAR process.
 *
 * The harness intercepts the load via `withExitGuard`; this test pins that the
 * intercepted error surfaces as a per-module load failure (so MODULAR keeps going)
...
import { ExtensionExitError, withExitGuard } from "@modular/pi-coding-agent/extensibility/utils";
...
describe("extension/hook loader process.exit guard (#3680)", () => {
	...
	beforeEach(() => {
		project = TempDir.createSync("@modular-exit-guard-");
	});
