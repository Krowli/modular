...
import {
	...
	type TtsrScanArgs,
	type TtsrTestArgs,
} from "@modular/pi-coding-agent/cli/ttsr-cli";
import { resetSettingsForTest } from "@modular/pi-coding-agent/config/settings";
import { getProjectAgentDir, getProjectDir, removeSyncWithRetries, setProjectDir } from "@modular/pi-utils";

let testTmpDir: string;

beforeAll(() => {
	testTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-ttsr-tests-"));
});
...
describe("modular ttsr", () => {
	...
	describe("test — context inference and matching", () => {
		it("infers tool/edit context when a positional resolves to a .ts file and --source is omitted", async () => {
			...
			// Simulate `modular ttsr test --rule <rule> src/foo.ts`: the command layer
