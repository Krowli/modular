...
import {
	...
	parseDefaultBranch,
	parseGitHubRepo,
} from "@modular/pi-coding-agent/modes/components/status-line/git-utils";

describe("parseGitHubRepo", () => {
	test("parses HTTPS URL", () => {
		expect(parseGitHubRepo("https://github.com/Krowli/modular.git")).toBe("Krowli/modular");
	});

	test("parses HTTPS URL without .git suffix", () => {
		expect(parseGitHubRepo("https://github.com/Krowli/modular")).toBe("Krowli/modular");
	});

	test("parses SSH scp-style URL", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/modular.git")).toBe("loftiskg/modular");
	});

	test("parses SSH scp-style URL without .git suffix", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/modular")).toBe("loftiskg/modular");
	});
