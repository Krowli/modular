/**
 * ACP stdout-hygiene smoke: launching `modular acp` must not leak any banner,
 ...
 * unwind, but force-kill quickly if it hasn't reaped — `modular acp` blocks on
...
describe("ACP stdout hygiene", () => {
	it("emits a JSON-RPC initialize response as the first bytes on stdout", async () => {
		const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "modular-acp-stdout-"));
