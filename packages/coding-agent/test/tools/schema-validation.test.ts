...
import { normalizeSchemaForGoogle, toolWireSchema } from "@modular/pi-ai";
import { Settings } from "@modular/pi-coding-agent/config/settings";
import { createTools, HIDDEN_TOOLS, type ToolSession } from "@modular/pi-coding-agent/tools";
...
describe("tool schema validation (post-sanitization)", () => {
	...
	it("read path schema advertises local, URL, and internal URI targets", async () => {
		...
		expect(description).toContain("modular://");
