...
import {
	...
	flushGrievances,
	isAutoQaEnabled,
} from "@modular/pi-coding-agent/tools/report-tool-issue";
...
describe("flushGrievances", () => {
	...
	it("posts pending rows with bearer header and marks them pushed=1 on 200", async () => {
		...
		expect(body.agent?.name).toBe("modular");
		...
		// stays so users can browse what they've shipped via `modular grievances`.
