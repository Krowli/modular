...
import type { AgentToolResult, RenderResultOptions } from "@modular/pi-agent-core";
import { preloadPluginRoots } from "@modular/pi-coding-agent/discovery/helpers";
import { LspTool } from "@modular/pi-coding-agent/lsp";
import * as lspClient from "@modular/pi-coding-agent/lsp/client";
import * as lspConfig from "@modular/pi-coding-agent/lsp/config";
import { getServersForFile, type LspConfig, loadConfig } from "@modular/pi-coding-agent/lsp/config";
import {
	applyTextEditsToString,
	applyWorkspaceEdit,
	sortAndValidateTextEdits,
} from "@modular/pi-coding-agent/lsp/edits";
import { renderCall, renderResult } from "@modular/pi-coding-agent/lsp/render";
import type {
	CodeAction,
	CreateFile,
	DeleteFile,
	...
	TextDocumentEdit,
	WorkspaceEdit,
} from "@modular/pi-coding-agent/lsp/types";
import {
	applyCodeAction,
	collectGlobMatches,
	...
	resolveSymbolColumn,
	uriToFile,
} from "@modular/pi-coding-agent/lsp/utils";
import { getThemeByName } from "@modular/pi-coding-agent/modes/theme/theme";
import type { ToolSession } from "@modular/pi-coding-agent/tools";
import { clampTimeout } from "@modular/pi-coding-agent/tools/tool-timeouts";
import * as piUtils from "@modular/pi-utils";
import { sanitizeText, TempDir } from "@modular/pi-utils";
...
describe("lsp regressions", () => {
	...
	it("sends the LSP exit notification after shutdown completes", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-shutdown-");
		try {
	...
	it("advertises workspace folder support during LSP initialization", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-workspace-folders-");
		try {
			const server = installFakeLsp((message, srv) => {
				if (message.method === "initialize") {
					srv.send({ jsonrpc: "2.0", id: message.id, result: { capabilities: {} } });
	...
	it("answers workspace/workspaceFolders requests with the current folder set", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-workspace-folders-request-");
		try {
			const server = installFakeLsp((message, srv) => {
				if (message.method === "initialize") {
					srv.send({ jsonrpc: "2.0", id: message.id, result: { capabilities: {} } });
	...
	it("sends initial workspace configuration after initialized before semantic requests (#5276)", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-initial-config-");
	...
	it("accepts dynamic capability registration before semantic requests", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-dynamic-registration-");
		try {
	...
	it("drains every workspace/configuration pull during lazy cold start when a pull id collides with an in-flight request", async () => {
		...
		const tempDir = TempDir.createSync("@modular-lsp-lazy-config-drain-");
		try {
			const symbols = [
				{
					name: "main",
	...
	it("answers defined server→client requests with spec no-op results", async () => {
		...
		// must receive a spec-shaped reply, not a -32601. Headless modular can't
		// surface UI prompts but still owes a defined no-op.
		const tempDir = TempDir.createSync("@modular-lsp-server-requests-");
		try {
			const server = installFakeLsp((message, srv) => {
				if (message.method === "initialize") {
					srv.send({ jsonrpc: "2.0", id: message.id, result: { capabilities: {} } });
	...
	it("opens rust-analyzer Cargo workspace files before polling workspace readiness", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-rust-workspace-");
		try {
	...
	it("skips rust-analyzer workspace polling for standalone Rust files", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-rust-standalone-");
		try {
	...
	it("limits glob collection to avoid large diagnostic stalls", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-glob-");
		try {
			await Promise.all([
	...
	it("treats existing bracket paths as literal diagnostic targets", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-bracket-path-");
		try {
	...
	it("resolves the requested symbol occurrence on a line", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-regression-");
		try {
	...
	it("throws when symbol does not exist on the target line", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-missing-symbol-");
		try {
			const filePath = path.join(tempDir.path(), "symbol.ts");
	...
	it("throws when occurrence is out of bounds", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-occurrence-");
		try {
			const filePath = path.join(tempDir.path(), "symbol.ts");
	...
	it("does not reuse stale file diagnostics after another URI publishes", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-stale-diags-");
		try {
	...
	it("treats a go.work-only root as a Go workspace for workspace diagnostics", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-go-work-only-");
	...
	it("builds every go.work use module when go.work and go.mod coexist", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-go-work-before-mod-");
		const spawnCalls: BunSpawnCall[] = [];
		recordBunSpawn(spawnCalls, cmd => {
			if (cmd.join("\0") === "go\0work\0edit\0-json") {
				return {
	...
	it("detects Windows local .exe LSP shims in node_modules/.bin", async () => {
		...
		const tempDir = TempDir.createSync("@modular-lsp-win32-bin-");
	...
	it("detects Ruff in Windows virtualenv Scripts directories", async () => {
		...
		const tempDir = TempDir.createSync("@modular-lsp-win32-ruff-");
		const whichSpy = vi.spyOn(Bun, "which").mockReturnValue(null);
	...
	it("detects Ruff in Windows virtualenv Scripts directories for Ruff-only roots", async () => {
		...
		try {
			for (const marker of ["ruff.toml", ".ruff.toml"] as const) {
				const tempDir = TempDir.createSync("@modular-lsp-win32-ruff-marker-");
				try {
	...
	it("detects pyright and pylsp in Windows virtualenv Scripts for Python-only roots", async () => {
		...
		try {
			...
			for (const { marker, server, binary } of cases) {
				const tempDir = TempDir.createSync("@modular-lsp-win32-py-marker-");
				try {
					await Bun.write(path.join(tempDir.path(), marker), "");
					const scriptsDir = path.join(tempDir.path(), ".venv", "Scripts");
					await fs.promises.mkdir(scriptsDir, { recursive: true });
	...
	it("detects tlaplus files for LSP startup and language ids", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-tlaplus-");
	...
	it("loads config-only marketplace LSP servers from Claude plugin cache", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-marketplace-config-");
	...
	it("rename_file applies LSP willRenameFiles edits and renames the file", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-rename-file-");
		try {
	...
	it("rename_file with apply:false previews edits without filesystem changes", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-rename-file-preview-");
		try {
	...
	it("rename_file enumerates every file inside a directory rename", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-rename-dir-");
		try {
	...
	it("request action sends raw LSP method with auto-built textDocument/position params", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-request-");
		try {
	...
	it("request action forwards explicit JSON payload verbatim", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-request-payload-");
		try {
			const server: ServerConfig = { command: "test-lsp", fileTypes: ["ts"], rootMarkers: [] };
			const client: LspClient = {
				name: "test-lsp",
	...
	it("capabilities action dumps server capabilities", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-caps-");
		try {
			const server: ServerConfig = { command: "test-lsp", fileTypes: ["ts"], rootMarkers: [] };
			const client: LspClient = {
				name: "test-lsp",
	...
	it("flushes pending descendant text edits before a folder rename", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-folder-rename-");
		try {
	...
	it("flushes pending edits queued against a rename target before performing the rename", async () => {
		...
		const tempDir = TempDir.createSync("@modular-lsp-rename-target-prefill-");
		try {
	...
	it("validates every file's edits before writing any workspace-edit file", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-atomic-validate-");
		try {
	...
	it("round-trips file URIs containing percent and hash characters", () => {
		const tricky = path.resolve(os.tmpdir(), "modular uri", "100% #1.ts");
		...
		const plain = path.resolve(os.tmpdir(), "modular uri", "plain.ts");
	...
	it("resolves $-prefixed identifiers past compound matches", async () => {
		...
		const tempDir = TempDir.createSync("@modular-lsp-dollar-identifier-");
		try {
	...
	it("applies a create op followed by a text edit for the same URI in declared order", async () => {
		...
		const tempDir = TempDir.createSync("@modular-lsp-create-then-edit-");
		try {
	...
	it("flushes pending descendant text edits before a folder delete", async () => {
		...
		const tempDir = TempDir.createSync("@modular-lsp-folder-delete-");
		try {
			const srcDir = path.join(tempDir.path(), "src");
			fs.mkdirSync(srcDir, { recursive: true });
			const childPath = path.join(srcDir, "a.ts");
	...
	it("rename_file skips the LSP loop when no configured server handles the file extension", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-rename-irrelevant-");
		try {
	...
	it("workspace reload rediscovers LSP servers after an empty config was cached", async () => {
		const tempDir = TempDir.createSync("@modular-lsp-reload-redetect-");
		try {
			const server: ServerConfig = {
				command: "test-lsp",
				fileTypes: [".ts"],
	...
	it("reload * invalidates the per-cwd config cache so newly written .modular/lsp.json is observed", async () => {
		// #3546: `getConfig` caches the first `loadConfig` result per cwd
		// permanently. Creating `.modular/lsp.json` after the first LSP call left
		...
		const tempDir = TempDir.createSync("@modular-lsp-config-cache-reload-");
		try {
	...
	describe("lsp cold-start and notification writes honor caller signal (#3962)", () => {
		it("aborts a wedged cold-start initialize on the caller signal instead of the 30s internal fallback", async () => {
			...
			const tempDir = TempDir.createSync("@modular-lsp-init-abort-");
			try {
		...
		it("does not negative-cache caller-aborted initialize attempts", async () => {
			...
			const tempDir = TempDir.createSync("@modular-lsp-init-abort-cache-");
			try {
				const controller = new AbortController();
				const timer = setTimeout(() => controller.abort(), 100);
				const config: ServerConfig = {
		...
		it("bounds a wedged notification flush on the caller signal and tears down the client", async () => {
			...
			const tempDir = TempDir.createSync("@modular-lsp-flush-wedge-");
			try {
				const config: ServerConfig = {
