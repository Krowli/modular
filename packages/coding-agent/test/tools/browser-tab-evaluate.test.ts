...
import { BrowserTool } from "@modular/pi-coding-agent/tools/browser";
import { ensureChromiumExecutable } from "@modular/pi-coding-agent/tools/browser/launch";
	...
	it("runs tab.evaluate in the page's main JavaScript world", async () => {
		...
		try {
			await tool.execute("open", {
				action: "open",
				name,
				url: "data:text/html,<script>globalThis.__modMainWorld = 42</script>",
			});
			const result = await tool.execute("run", {
				action: "run",
				name,
				code: "return await tab.evaluate(() => globalThis.__modMainWorld);",
			});
