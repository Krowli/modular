...
import { AuthStorage, type completeSimple, type ImageContent, type Model } from "@modular/pi-ai";
...
import { createAgentSession } from "@modular/pi-coding-agent/sdk";
...
import { InspectImageTool } from "@modular/pi-coding-agent/tools/inspect-image";
import { inspectImageToolRenderer } from "@modular/pi-coding-agent/tools/inspect-image-renderer";
import { toolRenderers } from "@modular/pi-coding-agent/tools/renderers";
import { removeSyncWithRetries, sanitizeText } from "@modular/pi-utils";
...
describe("InspectImageTool", () => {
	...
	beforeEach(() => {
		testDir = fs.mkdtempSync(path.join(os.tmpdir(), "modular-inspect-image-"));
	});
