...
import type { CustomToolContext } from "@modular/pi-coding-agent/extensibility/custom-tools";
import type { ReadonlySessionManager } from "@modular/pi-coding-agent/session/session-manager";
import {
	getImageGenTools,
	getImageGenToolsWithRegistry,
	imageGenTool,
	setPreferredImageProvider,
} from "@modular/pi-coding-agent/tools/image-gen";
...
describe("imageGenTool", () => {
	...
	it("routes xAI image generation with xAI-only aspect ratios", async () => {
		...
		expect(captured.userAgent).toBe("modular/xai");
		expect(requestBody).toMatchObject({
