...
} from "@modular/pi-coding-agent/internal-urls";
import { AgentRegistry } from "@modular/pi-coding-agent/registry/agent-registry";
import * as sshFileTransfer from "@modular/pi-coding-agent/ssh/file-transfer";
...
describe("GrepTool internal URL resolution", () => {
	...
	it("expands modular:// root to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# modular://tools/grep.md");
		expect(text).toContain("Grep file contents with a regex across files");
	});

	it("expands modular://docs to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# modular://tools/read.md");
