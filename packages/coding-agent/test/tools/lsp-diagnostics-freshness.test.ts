...
import { createLspWritethrough, type FileDiagnosticsResult } from "@modular/pi-coding-agent/lsp";
import * as lspClient from "@modular/pi-coding-agent/lsp/client";
import * as lspConfig from "@modular/pi-coding-agent/lsp/config";
import type { Diagnostic, LspClient, ServerConfig } from "@modular/pi-coding-agent/lsp/types";
import { fileToUri } from "@modular/pi-coding-agent/lsp/utils";
import type { DeferredDiagnosticsEntry, ToolSession } from "@modular/pi-coding-agent/tools";
import { WriteTool } from "@modular/pi-coding-agent/tools/write";
import { type ptree, TempDir } from "@modular/pi-utils";
...
describe("LSP diagnostics freshness", () => {
	...
	beforeEach(() => {
		tempDir = TempDir.createSync("@modular-lsp-freshness-");
	});
	...
	it("suppresses TypeScript project diagnostics for orphan files but keeps syntax errors", async () => {
		...
		const orphanDir = TempDir.createSync("@modular-lsp-orphan-");
		try {
