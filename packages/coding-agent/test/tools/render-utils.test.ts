...
import { getThemeByName, initTheme, type Theme, theme } from "@modular/pi-coding-agent/modes/theme/theme";
import {
	dedupeParseErrors,
	expandKeyHint,
	formatCodeFrameLine,
	...
	shortenPath,
	truncateDiffByHunk,
} from "@modular/pi-coding-agent/tools/render-utils";
import { getKeybindings, setKeybindings, type KeybindingsManager as TuiKeybindingsManager } from "@modular/pi-tui";
...
describe("formatScreenshot", () => {
	...
	it("formats non-home path without tilde", () => {
		const filePath = path.join(path.parse(os.homedir()).root, "modular-render-utils", "capture.png");
		const resized = fakeResized({ mimeType: "image/webp", buffer: new Uint8Array(1024) });
	...
	it("formats temp-only screenshot without save line", () => {
		...
		expect(
			formatScreenshot({
				...
				dest: path.join(os.tmpdir(), "modular-sshots-123.png"),
				resized,
			}),
