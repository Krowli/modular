...
import * as capability from "@modular/pi-coding-agent/capability";
import type { SSHHost } from "@modular/pi-coding-agent/capability/ssh";
import type { CapabilityResult } from "@modular/pi-coding-agent/capability/types";
import { Settings } from "@modular/pi-coding-agent/config/settings";
import { parseInternalUrl } from "@modular/pi-coding-agent/internal-urls/parse";
import { InternalUrlRouter } from "@modular/pi-coding-agent/internal-urls/router";
import { SshProtocolHandler } from "@modular/pi-coding-agent/internal-urls/ssh-protocol";
import type { ToolSession } from "@modular/pi-coding-agent/tools";
import { GrepTool } from "@modular/pi-coding-agent/tools/grep";
import { ReadTool } from "@modular/pi-coding-agent/tools/read";
import { WriteTool } from "@modular/pi-coding-agent/tools/write";
...
describe.skipIf(!SSH_OK)("ssh:// handler against a real localhost ssh", () => {
	...
	const TMP = `/tmp/modular-ssh-e2e-${process.pid}`;
	...
	it("writes byte-exact, leaves no temp, and the read path round-trips", async () => {
		...
		const leftovers = await Bun.$`ssh -o BatchMode=yes localhost ls ${TMP} | grep -c modular-tmp || true`.text();
	...
	it("refuses to write to a directory and cleans up its temp", async () => {
		...
		const leftovers =
			await Bun.$`ssh -o BatchMode=yes localhost ls -A ${TMP} | grep -c "wdir.modular-tmp" || true`.text();
		expect(leftovers.trim()).toBe("0");
	});
...
describe.skipIf(!SSH_OK)("ssh:// through the real read/grep/write tools (localhost)", () => {
	const TMP = `/tmp/modular-ssh-tools-e2e-${process.pid}`;
	...
	it("GrepTool reports matches under the ssh:// URL with no scratch-temp leak", async () => {
		...
		expect(out).not.toContain("modular-search-probe");
		expect(detailsJson).not.toContain("modular-search-probe");
	});
