...
import type { CmuxKind } from "@modular/pi-coding-agent/tools/browser/cmux/rpc";
import { CmuxSocketClient } from "@modular/pi-coding-agent/tools/browser/cmux/socket-client";
import { acquireBrowser, getBrowsersMapForTest } from "@modular/pi-coding-agent/tools/browser/registry";
import {
	acquireTab,
	getTabsMapForTest,
	releaseTab,
	releaseTabsForOwner,
} from "@modular/pi-coding-agent/tools/browser/tab-supervisor";
import { ToolAbortError } from "@modular/pi-coding-agent/tools/tool-errors";

function makeKind(socketSuffix: string): CmuxKind {
	return { kind: "cmux", socketPath: `/tmp/modular-test-${socketSuffix}.sock`, surface: `surface-${socketSuffix}` };
}
