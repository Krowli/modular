...
describe("daemon broker", () => {
	it("shares PTY output and input across project clients", async () => {
		const projectDir = await tempDir("modular-daemon-project-");
		const runtimeDir = await tempDir("modular-daemon-runtime-");
...
`,
	...
	it("stops non-persistent daemons after the last project modular exits", async () => {
		const projectDir = await tempDir("modular-daemon-exit-project-");
		const runtimeDir = await tempDir("modular-daemon-exit-runtime-");
		const scriptPath = path.join(projectDir, "service.ts");
	...
	it("keeps detached daemons alive through broker replacement", async () => {
		const projectDir = await tempDir("modular-daemon-detached-project-");
		const runtimeDir = await tempDir("modular-daemon-detached-runtime-");
		const scriptPath = path.join(projectDir, "service.ts");
		await Bun.write(scriptPath, `process.stdout.write("READY\\n"); setInterval(() => {}, 1000);\n`);
		const first = await createDaemonBrokerClient(projectDir, { runtimeDir, idleGraceMs: 5_000 });
	...
	it("names the unmet readiness condition when start times out", async () => {
		const projectDir = await tempDir("modular-daemon-ready-project-");
		const runtimeDir = await tempDir("modular-daemon-ready-runtime-");
		const scriptPath = path.join(projectDir, "service.ts");
