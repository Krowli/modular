...
} from "@modular/pi-coding-agent/tools/gh";
import * as git from "@modular/pi-coding-agent/utils/git";
import { getAgentDir, hashPath, removeWithRetries, setAgentDir } from "@modular/pi-utils";
 ...
 * user's real `~/.modular/wt`. Returns the temp home and a cleanup hook.
 */
interface TempHome {
	home: string;
...
async function setupTempHome(): Promise<{ home: string; cleanup: () => Promise<void> }> {
	...
	// through the spied homedir instead of `$XDG_DATA_HOME/modular/wt` (CI sets these).
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	...
	return fs.realpath(path.join(home, ".modular", "wt", segment));
}
...
describe("github tool", () => {
	...
	describe("git.remote.add idempotency", () => {
		...
		it("does not depend on localized git remote-add stderr for existing remotes", async () => {
			...
			const fakeBin = await fs.mkdtemp(path.join(os.tmpdir(), "modular-fake-git-"));
...
`,
	...
	describe("pr_checkout (array of pull requests)", () => {
		...
		it("checks out multiple pull requests in a single call when pr is an array", async () => {
			...
			// `--get-regexp` echoes variable names in git's canonical lowercase.
			const prUrls = runGit(fixture.repoRoot, ["config", "--get-regexp", "^branch\\.pr-.*\\.modprurl$"]);
			expect(prUrls).toContain("branch.pr-100.modprurl https://github.com/owner/repo/pull/100");
			expect(prUrls).toContain("branch.pr-200.modprurl https://github.com/owner/repo/pull/200");
