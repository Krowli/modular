...
import { Effort, type FetchImpl, type Model } from "@modular/pi-ai";
import type { OAuthCredentials } from "@modular/pi-ai/oauth/types";
import { buildModel } from "@modular/pi-catalog/build";
import { writeModelCache } from "@modular/pi-catalog/model-cache";
import type { OpenAICompat } from "@modular/pi-catalog/types";
import { kNoAuth, ModelRegistry } from "@modular/pi-coding-agent/config/model-registry";
import { resetSettingsForTest } from "@modular/pi-coding-agent/config/settings";
...
describe("ModelRegistry runtime discovery", () => {
	...
	test("keeps OLLAMA_BASE_URL precedence over OLLAMA_HOST", async () => {
		using _baseUrl = withEnv("OLLAMA_BASE_URL", "http://modular-ollama.example:2222");
		using _host = withEnv("OLLAMA_HOST", "ollama-host.example:3333");
		const fetchMock = mockOllamaDiscovery(["phi4-mini"], "http://modular-ollama.example:2222");
		const registry = new ModelRegistry(authStorage, modelsJsonPath, { fetch: fetchMock });
		await registry.refresh();

		const model = registry.find("ollama", "phi4-mini");
		expect(model?.baseUrl).toBe("http://modular-ollama.example:2222/v1");
	});
