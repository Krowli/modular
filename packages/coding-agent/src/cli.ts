...
import { declareWorkerHostEntry, installWorkerInbox } from "@modular/pi-utils/worker-host";
...
// `@modular/pi-utils/env` eagerly loads `.env` from the agent directory at
// import time, so it must not be imported before `setProfile` runs.

async function showHelp(config: CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@modular/pi-utils/cli");
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker, startServer } = await import("@modular/modular-stats");
...
const TINY_WORKER_ARG = "__modular_worker_tiny_inference";
const STATS_SYNC_WORKER_ARG = "__modular_worker_stats_sync";
const TAB_WORKER_ARG = "__modular_worker_tab";
const JS_EVAL_WORKER_ARG = "__modular_worker_js_eval";
const JS_EVAL_PROCESS_ARG = "__modular_worker_js_eval_process";
const STT_WORKER_ARG = "__modular_worker_stt";
const TTS_WORKER_ARG = "__modular_worker_tts";
const MNEMOPI_EMBED_WORKER_ARG = "__modular_worker_mnemopi_embed";
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG) {
		...
		await import("@modular/modular-stats/sync-worker");
...
export async function runCli(argv: string[]): Promise<void> {
	...
	try {
		...
		if (extracted.profile !== undefined) {
			...
			// No explicit --profile: activate any MODULAR_PROFILE/PI_PROFILE inherited
			...
			// MODULAR_PROFILE after importing this module (profile aliases/tests). Surfacing
			// validation here turns `MODULAR_PROFILE=.. modular --version` into a clean error;
			// calling setProfile keeps every later path helper on the env-selected
			// profile instead of the default agent directory.
			setProfile(resolveProfileEnv(process.env.MODULAR_PROFILE, process.env.PI_PROFILE));
		}
		if (extracted.aliasName !== undefined) {
			const profile = extracted.profile ?? getActiveProfile();
			if (!profile) {
				throw new Error("--alias requires --profile <name> or MODULAR_PROFILE");
			}
	...
	if (resolvedArgv[0]?.startsWith("__modular_worker_")) {
	...
	// `@modular/pi-utils/env` here would snapshot the wrong agent `.env`.
	...
	// `__modular_worker_` dispatch, and importers (`runCli` in profile-CLI tests,
