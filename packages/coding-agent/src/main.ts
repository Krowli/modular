...
import { EventLoopKeepalive } from "@modular/pi-agent-core";
import type { ImageContent } from "@modular/pi-ai";
import {
	$env,
	directoryExists,
	getLogPath,
...
import { injectOmpExtensionCliRoots } from "./discovery/modular-extension-roots";
...
async function checkForNewVersion(currentVersion: string): Promise<string | undefined> {
	...
	try {
		const response = await fetch("https://registry.npmjs.org/@modular/pi-coding-agent/latest", {
...
// re-apply at startup so embedders inherit MODULAR's neutral defaults instead of
...
async function runInteractiveMode(
	...
	// `modular join <link>`: dispatch through the same builtin path as a typed
	...
	if (parsed.fork) {
		...
		if (!match) {
			throw new SessionResolutionError(
				...
				"Run `modular --resume` without an argument to pick from recent sessions, or `modular` to start a new one.",
			);
		}
...
function discoverSystemPromptFile(): string | undefined {
	// Check project-local first (.modular/SYSTEM.md, .pi/SYSTEM.md legacy)
...
export async function runRootCommand(
	...
	// the `modular-plugins` discovery provider can surface their `skills/`, `hooks/`,
	...
	if (mode === "acp") {
		...
		// Fail fast on stale/typo flags (e.g. `modular --list-models`) now that we
