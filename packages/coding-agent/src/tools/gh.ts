...
import { getWorktreeDir, hashPath, isEnoent, prompt, untilAborted } from "@modular/pi-utils";
...
async function resolvePrBranchPushTarget(
	...
	const headRef = await git.config.getBranch(repoRoot, localBranch, "modPrHeadRef", signal);
	if (!headRef) {
	...
	const prUrl = await git.config.getBranch(repoRoot, localBranch, "modPrUrl", signal);
	const maintainerCanModifyValue = await git.config.getBranch(
		repoRoot,
		localBranch,
		"modPrMaintainerCanModify",
		signal,
	);
	const isCrossRepositoryValue = await git.config.getBranch(repoRoot, localBranch, "modPrIsCrossRepository", signal);
...
async function checkoutPullRequest(
	...
	return git.withRepoLock(
		...
		async () => {
			...
			await git.config.setBranch(repoRoot, localBranch, "modPrHeadRef", headRefName, signal);
			await git.config.setBranch(repoRoot, localBranch, "modPrUrl", data.url ?? "", signal);
			await git.config.setBranch(
				repoRoot,
				localBranch,
				"modPrIsCrossRepository",
