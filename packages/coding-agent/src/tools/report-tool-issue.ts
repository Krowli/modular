 ...
 *
 * When the user grants consent AND a push endpoint is configured
 * (`dev.autoqaPush.endpoint` or `PI_AUTO_QA_PUSH_URL`; no default — push is off
 * unless set), push becomes active. Each
...
import type { AgentTool } from "@modular/pi-agent-core";
import type { FetchImpl } from "@modular/pi-ai";
import { $env, $flag, getAutoQaDbDir, getInstallId, logger, VERSION } from "@modular/pi-utils";
 ...
 * `~/.modular/agent/autoqa.db`. Idempotently runs schema creation, the
 * `pushed`-column migration, and index setup so every consumer — tool
 * execute path, manual `modular grievances push`, future debug scripts —
 ...
 *
 * Exported because the `modular grievances` CLI handlers need the migrated
...
/**
 * Optional per-flush controls. Used by `modular grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`modular grievances push`) where the
...
async function performFlush(db: Database, config: PushConfig, options: FlushOptions = {}): Promise<FlushResult> {
	...
	for (;;) {
		...
		const body = JSON.stringify({
			agent: { name: "modular", version: VERSION },
...
export function createReportToolIssueTool(session: ToolSession, activeBuiltinNames: readonly string[] = []): AgentTool {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			// at ~/.modular/agent/autoqa.db regardless of consent — they always
			// own their local data and can inspect or wipe it via `modular grievances`.
			...
			try {
				...
				if (db) {
					...
					//      `modular grievances push` or a future consent change.
