...
import { prompt, sanitizeText } from "@modular/pi-utils";
...
const launchSchema = type({
	...
	"persist?": type("boolean").describe("start: survive the last modular client exiting; default false"),
	"detached?": type("boolean").describe(
		"start: survive every modular and broker exit; implies persist and disables PTY input",
	),
