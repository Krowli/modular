...
import { type ApiKey, type FetchImpl, getEnvApiKey, type Model, withAuth } from "@modular/pi-ai";
...
import { getAntigravityUserAgent } from "@modular/pi-catalog/wire/gemini-headers";
import {
	$env,
	isEnoent,
	parseImageMetadata,
...
import { modularXAIUserAgent, resolveXAIHttpCredentials } from "../lib/xai-http";
...
async function saveImageToTemp(image: InlineImageData): Promise<string> {
	...
	const filename = `modular-image-${Snowflake.next()}.${ext}`;
					...
					async key => {
						const resp = await fetchImpl(`${xaiCreds.baseURL}${xaiEndpoint}`, {
							...
							headers: {
								...
								"Content-Type": "application/json",
								"User-Agent": modularXAIUserAgent(),
							},
					...
					async key => {
						const resp = await fetchImpl("https://openrouter.ai/api/v1/chat/completions", {
							...
							headers: {
								"Content-Type": "application/json",
								Authorization: `Bearer ${key}`,
								"HTTP-Referer": "https://modularcli.com/",
								"X-OpenRouter-Title": "Modular",
