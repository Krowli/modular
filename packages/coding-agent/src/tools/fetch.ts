...
import type { FetchImpl, ImageContent, TextContent } from "@modular/pi-ai";
import { htmlToMarkdown } from "@modular/pi-natives";
import { type Component, Text } from "@modular/pi-tui";
import { $which, ptree, truncate } from "@modular/pi-utils";
...
async function renderNotebookPayload(bytes: Uint8Array, displayUrl: string): Promise<string> {
	return withTempBinaryFile("modular-url-notebook-", ".ipynb", bytes, tempPath =>
		readEditableNotebookText(tempPath, displayUrl),
	);
}

async function renderSqlitePayload(bytes: Uint8Array): Promise<string> {
	return withTempBinaryFile("modular-url-sqlite-", ".sqlite", bytes, async tempPath => {
