...
import { formatHashlineHeader } from "@modular/hashline";
import type {
	AgentTool,
	AgentToolContext,
	AgentToolResult,
...
import { type GrepMatch, GrepOutputMode, type GrepResult, grep } from "@modular/pi-natives";
import type { Component } from "@modular/pi-tui";
import { Text } from "@modular/pi-tui";
import { prompt, untilAborted } from "@modular/pi-utils";
...
async function resolveArchiveSearchPaths(
	...
	for (let idx = 0; idx < pathSpecs.length; idx++) {
		...
		if (!tempDir) {
			tempDir = await mkdtemp(path.join(tmpdir(), "modular-search-archive-"));
		}
...
const MODULAR_ROOT_URL_RE = /^modular:\/\/(?:\/?|docs\/?)$/i;
...
async function searchVirtualResources(
	...
	const dir = await mkdtemp(path.join(tmpdir(), "modular-search-virtual-"));
	try {
...
async function expandVirtualInternalResource(
	...
	if (MODULAR_ROOT_URL_RE.test(rawPath)) {
		const completions = await internalRouter.complete("modular", "");
		if (completions && completions.length > 0) {
			...
			for (const completion of completions) {
				...
				const docUrl = `modular://${completion.value}`;
