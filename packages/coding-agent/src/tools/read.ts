...
import { formatHashlineHeader, formatNumberedLine, formatNumberedLines } from "@modular/hashline";
import type {
	AgentTool,
	AgentToolContext,
	AgentToolResult,
...
import { glob, type SummaryResult, summarizeCode } from "@modular/pi-natives";
...
const readSchema = type({
	path: type("string").describe(
		'Local path, internal URI (e.g. "modular://", "issue://123", "pr://123"), or URL. Inline :<sel> is still accepted for compatibility.',
	),
...
export class ReadTool implements AgentTool<typeof readSchema, ReadToolDetails> {
	...
	#pdfImageCacheDir(absolutePdfPath: string): string {
		...
		if (root === undefined) {
			...
			root = sessionFile?.endsWith(".jsonl")
				...
				: path.join(os.tmpdir(), "modular-read-pdf-images");
		}
	...
	async execute(
		...
		// Handle internal URLs (agent://, artifact://, memory://, skill://, rule://, local://, mcp://, modular://, issue://, pr://).
