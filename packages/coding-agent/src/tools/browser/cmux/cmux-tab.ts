...
import { logger, postmortem, Snowflake, untilAborted } from "@modular/pi-utils";
...
export class CmuxTab {
	...
	async screenshot(opts: ScreenshotOptions = {}): Promise<ScreenshotResult> {
		...
		const dest =
			...
			(context.session.browserScreenshotDir
				...
				: (returnedPath ?? path.join(os.tmpdir(), `modular-sshots-${Snowflake.next()}.${ext}`)));
	...
	async #responseCursor(): Promise<number> {
		const value = await this.#evalScript<unknown>(
			"(() => Math.max(0, ((globalThis.__modCmuxResponses && globalThis.__modCmuxResponses.nextId) || 1) - 1))()",
		);
	...
	async #responseRecordsAfter(id: number): Promise<CmuxResponseRecord[]> {
		const value = await this.#evalScript<unknown>(
			`(() => ((globalThis.__modCmuxResponses && globalThis.__modCmuxResponses.records) || []).filter(record => record.id > ${JSON.stringify(id)}))()`,
		);
