...
export function serializeEvalWithEnvelope(fn: string | ((...args: unknown[]) => unknown), args: unknown[]): string {
	...
	return `(() => {
		try {
			const __v = (${expr});
			if (__v && typeof __v.then === "function") return { __modPromise: true };
			return { __modOk: __v === undefined ? null : __v };
		} catch (e) {
			return { __modErr: (e && (e.stack || e.message)) || String(e) };
		}
	})()`;
}
...
export function unwrapEvalEnvelope<TResult>(value: unknown, label: string): TResult {
	if (value && typeof value === "object") {
		if ("__modErr" in value && typeof value.__modErr === "string") {
			throw new ToolError(`${label} threw a JavaScript exception:\n${value.__modErr}`);
		}
		if ("__modPromise" in value && value.__modPromise === true) {
			throw new ToolError(
		...
		if ("__modOk" in value) {
			return value.__modOk as TResult;
		}
	}
