...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@modular/pi-utils";
 ...
 *
 * Used to key per-project autoresearch state under `~/.modular/autoresearch/`.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.MODULAR_AUTORESEARCH_DB_DIR;
