Launches and controls project-scoped long-running processes shared by every modular instance in the same directory.
...
- `persist: true` opts out of last-modular teardown. Otherwise the broker stops every non-persistent supervised process after the last modular in this directory exits.
- `detached: true` survives broker shutdown and all modular exits. It implies `persist` and disables PTY/stdin.
</instruction>
