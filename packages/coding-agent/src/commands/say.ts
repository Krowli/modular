...
export default class Say extends Command {
	...
	static examples = [
		'modular say "hello world"',
		"modular say --file notes.md --voice bm_fable",
		'modular say "hello world" --out /tmp/hello.wav',
	];
	...
	#synthesisFailed(model: string): void {
		process.stderr.write(
			chalk.red(
				`error: could not synthesize with local TTS model "${model}". Run \`modular setup speech\` to install it.\n`,
			),
		);
	}
}
