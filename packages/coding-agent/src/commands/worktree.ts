/**
 * List and clean up agent-managed git worktrees under `~/.modular/wt`.
 */
import { getProjectDir } from "@modular/pi-utils";
...
export default class Worktree extends Command {
	static description = "List or clear agent-managed git worktrees (~/.modular/wt)";

	static aliases = ["wt"];

	static args = {
		// `list` (default) inspects the worktree dir; `clear` removes entries.
		// A positional action keeps `modular worktree` (the no-arg form) useful.
		action: Args.string({
	...
	static examples = [
		"modular worktree",
		"modular worktree list --json",
		"modular worktree clear",
		"modular worktree clear --dry-run",
		"modular worktree clear --all",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Worktree);
		// Load settings so the `worktree.base` override is applied before we scan
		// — otherwise this command would inspect ~/.modular/wt while the agent created
