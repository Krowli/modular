...
/**
 * `modular ttsr` — inspect and test Time-Traveling Stream Rules.
 *
 * `modular ttsr test` feeds a snippet (inline, --file, or stdin) through the real
 * TTSR matching pipeline and reports which rules would trigger. `modular ttsr list`
 * shows every TTSR-registered rule the current project/user config would load.
 */
import { Args, Command, Flags } from "@modular/pi-utils/cli";
import {
	runTtsrCommand,
	TTSR_ACTIONS,
	TTSR_SOURCES,
...
export default class Ttsr extends Command {
	...
	static examples = [
		"modular ttsr list",
		"modular ttsr test 'const x: any = 1'",
		"modular ttsr test src/foo.ts",
		"modular ttsr test --file src/foo.ts",
		"modular ttsr test --file src/foo.ts --source text",
		"modular ttsr test --rule .modular/rules/no-any.md --source tool --path src/foo.ts 'const x: any = 1'",
		"echo 'Box::leak(&mut v)' | modular ttsr test --file - --path src/lib.rs",
		"modular ttsr test --source tool --tool edit --path src/foo.ts 'const x: any = 1'",
		"modular ttsr scan",
		"modular ttsr scan src/",
		"modular ttsr scan -r .modular/rules/no-any.md src/",
	];
	...
	async run(): Promise<void> {
		...
		// inline text — so `modular ttsr test src/foo.ts` works without --file.
