/**
 * `modular install <target>` — top-level convenience over `modular plugin install` /
 * `modular plugin link`.
 *
 * The docs (modularcli.com/docs/extension-authoring) advertise
 *
 *   modular install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `modular install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: modular install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
