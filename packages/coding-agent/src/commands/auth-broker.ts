/**
 * `modular auth-broker` — manage the modular credential vault.
 */
import { Args, Command, Flags, renderCommandHelp } from "@modular/pi-utils/cli";
import {
	AUTH_BROKER_ACTIONS,
...
export default class AuthBroker extends Command {
	static description = "Manage the modular auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  modular auth-broker serve",
		"# Boot on a non-default port\n  modular auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  modular auth-broker token",
		"# Rotate the bearer token\n  modular auth-broker token --regenerate",
		"# List supported OAuth providers\n  modular auth-broker list",
		"# Local login (run on the broker host)\n  modular auth-broker login anthropic",
		"# Interactive provider selection\n  modular auth-broker login",
		"# Remote login over SSH tunnel\n  modular auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  modular auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  modular auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  modular auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  modular auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  modular auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  modular auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("modular", "auth-broker", AuthBroker);
			return;
		}
