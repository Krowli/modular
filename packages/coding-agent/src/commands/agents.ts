...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.modular/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.modular/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  modular agents unpack",
		"# Export bundled agents into project config\n  modular agents unpack --project",
		"# Overwrite existing local agent files\n  modular agents unpack --project --force",
		"# Export into a custom directory\n  modular agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("modular", "agents", Agents);
			return;
		}
