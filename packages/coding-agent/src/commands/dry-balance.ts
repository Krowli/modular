...
export default class DryBalance extends Command {
	...
	static flags = {
		model: Flags.string({ description: "Model selector (same syntax as --model on modular)" }),
	...
	static examples = [
		...
		"# Dry-run a specific model\n  modular dry-balance anthropic/claude-sonnet-4-5",
		"# Larger run with bounded concurrency\n  modular dry-balance --model openai-codex/gpt-5-codex --count 1000 --concurrency 64",
		"# Benchmark every OAuth account in parallel\n  modular dry-balance --bench",
		"# Machine-readable output\n  modular dry-balance --json",
	];
