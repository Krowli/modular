...
import type { Api, AssistantMessage, Model } from "@modular/pi-ai";
...
describe("completion() through eval runtimes", () => {
	...
	it("exposes completion() in the JavaScript runtime", async () => {
		using tempDir = TempDir.createSync("@modular-eval-completion-js-");
	...
	it("parses structured completion() output in the JavaScript runtime", async () => {
		using tempDir = TempDir.createSync("@modular-eval-completion-js-struct-");
		const sessionFile = path.join(tempDir.path(), "session.jsonl");
	...
	it("exposes completion() in the Python runtime", async () => {
		const tempDir = TempDir.createSync("@modular-eval-completion-py-");
		try {
	...
	it("parses structured completion() output in the Python runtime", async () => {
		const tempDir = TempDir.createSync("@modular-eval-completion-py-struct-");
		try {
