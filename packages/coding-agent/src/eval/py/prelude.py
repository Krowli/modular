...
# MODULAR prelude helpers (loaded once into the runner namespace)
if "__mod_prelude_loaded__" not in globals():
    __mod_prelude_loaded__ = True
    from pathlib import Path
    import os, json, math, re
    from urllib.parse import unquote
    INTENT_FIELD = "i"

    # __mod_display is injected by runner.py before the prelude executes; it
    # mirrors IPython's display() semantics with the same MIME bundle output.
    _mod_display = __mod_display  # type: ignore[name-defined]
    ...
    def _emit_status(op: str, **data):
        ...
        _mod_display({"application/x-modular-status": {"op": op, **data}}, raw=True)
    ...
    def _resolve_mod_path(path: str | Path) -> Path:
    ...
    def _bridge_call(name: str, args: dict):
        ...
        _run_id_getter = globals().get("__mod_current_run_id__")
        _run_id = _run_id_getter() if callable(_run_id_getter) else globals().get("__mod_run_id__")
        payload = json.dumps(
    ...
    def phase(title):
        ...
        globals()["__mod_current_phase__"] = str(title)
