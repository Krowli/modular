"""MODULAR Python runner — subprocess wrapper used by the coding-agent host.
...
_CURRENT_RID: contextvars.ContextVar[str | None] = contextvars.ContextVar("modular_current_rid", default=None)
_CURRENT_DISPLAYED_MATPLOTLIB_FIGURE_IDS: contextvars.ContextVar[set[int] | None] = contextvars.ContextVar(
    "modular_displayed_matplotlib_figure_ids",
    default=None,
)
...
def _start_capture_drain() -> None:
    if _CAPTURE_READ_FD is None:
        return
    thread = threading.Thread(target=_drain_captured_stdout, name="modular-fd1-capture", daemon=True)
    thread.start()
...
def transform_cell(source: str) -> str:
    ...
    Rules
    -----
    * ``%name args``              -> ``__mod_magic("name", "args")``
    * ``var = %name args``        -> ``var = __mod_magic("name", "args")``
    * ``!cmd``                    -> ``__mod_shell("cmd")``
    * ``var = !cmd``              -> ``var = __mod_shell("cmd")``
    * ``%%name args\\n<body>``    -> ``__mod_magic_cell("name", "args", "<body>")``
    ...
    while i < len(lines):
        ...
        if stripped.startswith("%%"):
            ...
            body = "\n".join(body_lines)
            out.append(
                f"{indent}__mod_magic_cell({_quote_arg(name)}, {_quote_arg(args)}, {_quote_arg(body)})"
            )
            return "\n".join(out)
        ...
        if stripped.startswith("%") and not stripped.startswith("%%"):
            ...
            name, args = head
            out.append(f"{indent}__mod_magic({_quote_arg(name)}, {_quote_arg(args)})")
            i += consumed
            continue
        ...
        if stripped.startswith("!"):
            ...
            out.append(f"{indent}__mod_shell({_quote_arg(cmd)})")
            i += consumed
            continue
        ...
        if m:
            ...
            if rhs.startswith("!"):
                cmd = rhs[1:].strip()
                out.append(f"{m.group('indent')}{m.group('lhs').rstrip()} = __mod_shell({_quote_arg(cmd)})")
                i += 1
                continue
            if rhs.startswith("%") and not rhs.startswith("%%"):
                ...
                name, args = head
                out.append(
                    f"{m.group('indent')}{m.group('lhs').rstrip()} = __mod_magic({_quote_arg(name)}, {_quote_arg(args)})"
                )
                i += 1
                continue
...
def _emit_status(op: str, **data: Any) -> None:
    bundle = {"application/x-modular-status": {"op": op, **data}}
    rid = _CURRENT_RID.get()
    if rid is None:
        return
...
def __mod_magic(name: str, args: str) -> Any:
...
def __mod_magic_cell(name: str, args: str, body: str) -> Any:
...
def __mod_shell(cmd: str) -> _ShellResult:
    proc = subprocess.Popen(
        cmd,
        shell=True,
        stdout=subprocess.PIPE,
...
def __mod_display(value: Any, *, raw: bool = False, kind: str = "display") -> None:
    if raw:
...
def _install_builtins(ns: dict) -> None:
    ns["display"] = __mod_display
    ns["__mod_display"] = __mod_display
    ns["__mod_magic"] = __mod_magic
    ns["__mod_magic_cell"] = __mod_magic_cell
    ns["__mod_shell"] = __mod_shell
    ns["__mod_current_run_id__"] = lambda: _CURRENT_RID.get()
...
def _start_parent_watchdog() -> None:
    ...
    thread = threading.Thread(target=watch, name="modular-parent-watchdog", daemon=True)
    thread.start()
...
async def _handle_request_async(req: dict) -> None:
    ...
    _STATE.user_ns["__mod_run_id__"] = rid
...
async def _main_async() -> None:
    ...
    reader = threading.Thread(target=_read_stdin, args=(loop, queue, stdin), name="modular-stdin-reader", daemon=True)
    reader.start()
