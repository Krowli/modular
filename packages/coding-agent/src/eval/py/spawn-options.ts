 ...
 * kernel shares MODULAR's session, that shell can replace MODULAR as the controlling
 ...
 * redirection cases (`modular -p > out.txt`, `< in.txt`, `2> err.log`) where at
 ...
 * long-lived MODULAR host never changes for the lifetime of the process, and
 ...
 *   of how the standard streams are redirected — so an `modular -p ... < in.txt
