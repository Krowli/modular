...
import { $flag, isBunTestRuntime, logger, Snowflake } from "@modular/pi-utils";
import { $ } from "bun";
...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "modular-python-runner");
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	return [
		"import os, sys",
		`__modular_cwd = ${JSON.stringify(cwd)}`,
		"os.chdir(__modular_cwd)",
		`__modular_env = ${JSON.stringify(envPayload)}`,
		"for __modular_key, __modular_val in __modular_env.items():\n    os.environ[__modular_key] = __modular_val",
		"if __modular_cwd not in sys.path:\n    sys.path.insert(0, __modular_cwd)",
	].join("\n");
}
