import { logger, postmortem, Snowflake, workerHostEntry } from "@modular/pi-utils";
import {
	createWorkerHandle,
 ...
 * `modular --smoke-test` so binary / source / tarball installs all exercise it.
 */
...
function spawnBunWorker(): WorkerHandle {
	try {
		...
		const worker = hostEntry
			? new Worker(hostEntry, { type: "module", argv: ["__modular_worker_js_eval"] })
