...
export interface RuntimeOptions {
	...
	sessionId: string;
	/**
	 * Extra globals installed alongside `__modular_helpers__` / prelude. Use for stable, lifetime-
...
export class JsRuntime {
	...
	#install(extraGlobals: Record<string, unknown> | undefined): void {
		...
		const injected: Record<string, unknown> = {
			__modular_session__: this.#session,
			__modular_helpers__: this.helpers,
			__modular_call_tool__: async (name: string, args: unknown) => {
				const hooks = this.#activeHooks("tool");
				if (!hooks) return undefined;
				return await hooks.callTool(name, args);
			},
			__modular_import__: async (source: string, options?: ImportCallOptions) => {
			...
			__modular_import_from__: async (moduleUrl: string, source: string, options?: ImportCallOptions) => {
				const resolved = await this.#moduleLoader.resolveForModule(moduleUrl, source, this.#activeCwd());
				if (resolved.mode === "local") return resolved.value;
				const target = resolved.target;
				return options !== undefined ? await import(target, options) : await import(target);
			},
			__modular_get_require__: (moduleUrl?: string) => this.#activeRequire(moduleUrl),
			__modular_get_filename__: (moduleUrl?: string) => this.#moduleFilename(moduleUrl),
			__modular_get_dirname__: (moduleUrl?: string) => this.#moduleDirname(moduleUrl),
			__modular_emit_status__: (op: string, data: Record<string, unknown> = {}) => {
				const event: JsStatusEvent = { op, ...data };
				this.#activeHooks("emitStatus")?.onDisplay({ type: "status", event });
			},
			__modular_log__: (level: string, ...args: unknown[]) => {
				const prefix = level === "error" ? "[error] " : level === "warn" ? "[warn] " : "";
				const text = `${prefix}${formatConsoleArgs(args)}`;
				this.#activeHooks("log")?.onText(text.endsWith("\n") ? text : `${text}\n`);
			},
			__modular_table__: (...args: unknown[]) => {
			...
			__modular_display__: (value: unknown) => this.displayValue(value),
			__modular_set_final_expr__: (value: unknown) => {
