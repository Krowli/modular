...
export class LocalModuleLoader {
	...
	async #buildLocalModule(modulePath: string): Promise<LocalModuleEntry> {
		...
		const identifier = `${fileUrl}?modular-session=${this.#sessionTag}&v=${version}`;
...
function buildModuleSource(source: string, modulePath: string): string {
	const moduleDir = path.dirname(modulePath);
	return [
		`const require = globalThis.__modular_get_require__(${JSON.stringify(pathToFileURL(modulePath).href)});`,
