...
import { $flag, Snowflake } from "@modular/pi-utils";
import { $ } from "bun";
...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "modular-julia-runner");
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	const lines = [
		`__modular_init_cwd = String(Base64.base64decode("${Buffer.from(cwd).toString("base64")}"))`,
		"try cd(__modular_init_cwd) catch; end",
	];
	...
	lines.push("if !(__modular_init_cwd in LOAD_PATH); pushfirst!(LOAD_PATH, __modular_init_cwd); end");
	return lines.join("\n");
}
