...
const RUNNER_CACHE_DIR = path.join(os.tmpdir(), "modular-ruby-runner");
...
function buildInitScript(cwd: string, env?: Record<string, string | undefined>): string {
	...
	const lines = [`__modular_init_cwd = ${JSON.stringify(cwd)}`, "Dir.chdir(__modular_init_cwd) rescue nil"];
	for (const key in envPayload) {
		lines.push(`ENV[${JSON.stringify(key)}] = ${JSON.stringify(envPayload[key])}`);
	}
	lines.push("$LOAD_PATH.delete(__modular_init_cwd)", "$LOAD_PATH.unshift(__modular_init_cwd)");
	return lines.join("\n");
}
