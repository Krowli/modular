...
# MODULAR Ruby prelude helpers (loaded once into the runner's TOPLEVEL_BINDING).
#
...
#
# `__mod_*` primitives (emit/present/status/scrub/run_id) are provided by
# runner.rb; this file only consumes them.

unless defined?($__mod_prelude_loaded) && $__mod_prelude_loaded
  $__mod_prelude_loaded = true
  ...
  def __mod_resolve_path(path)
    ...
    relative = __mod_url_decode(m[2].tr("\\", "/"))
  ...
  def display_image(base64, mime_type: "image/png")
    __mod_emit_display({ mime_type.to_s => base64.to_s })
    nil
  end
  ...
  def env(key = nil, value = nil)
    if key.nil?
      ...
      __mod_emit_status("env", "count" => items.size, "keys" => items.keys.first(20))
      return items
    end
    unless value.nil?
      ENV[key.to_s] = value.to_s
      __mod_emit_status("env", "key" => key.to_s, "value" => value.to_s, "action" => "set")
      return value
    end
    val = ENV[key.to_s]
    __mod_emit_status("env", "key" => key.to_s, "value" => val, "action" => "get")
    val
  end
  ...
  def read(path, offset = 1, limit = nil)
    resolved = __mod_resolve_path(path)
    ...
    __mod_emit_status("read", "path" => resolved.to_s, "chars" => data.length, "preview" => __mod_scrub(data[0, 500].to_s))
    data
  end
  ...
  def write(path, content)
    ...
    __mod_emit_status("write", "path" => resolved.to_s, "chars" => content.to_s.length)
    resolved.to_s
  end
  ...
  def __mod_apply_query(data, query)
  ...
  def output(*ids, format: "raw", query: nil, offset: nil, limit: nil)
    ...
    if artifacts_dir.nil? || artifacts_dir.empty?
      ...
      if session_file.nil? || session_file.empty?
        __mod_emit_status("output", "error" => "No session file available")
        raise "No session - output artifacts unavailable"
      end
      artifacts_dir = session_file.sub(/\.[^.]*\z/, "")
    end
    unless File.directory?(artifacts_dir)
      __mod_emit_status("output", "error" => "Artifacts directory not found", "path" => artifacts_dir)
    ...
    if query && (!offset.nil? || !limit.nil?)
      __mod_emit_status("output", "error" => "query cannot be combined with offset/limit")
      ...
      if query
        json_value =
          begin
            ...
            __mod_emit_status("output", "id" => output_id, "error" => "Not valid JSON: #{e.message}")
        ...
        if start_line > total_lines
          __mod_emit_status("output", "id" => output_id, "error" => "Offset #{start_line} beyond end (#{total_lines} lines)")
    ...
    unless not_found.empty?
      ...
      __mod_emit_status("output", "not_found" => not_found, "available_count" => available.length)
      raise msg
    end

    if ids.length == 1
      if format == "json"
        __mod_emit_status("output", "id" => ids[0], "chars" => results[0]["char_count"])
        return results[0]
      end
      __mod_emit_status("output", "id" => ids[0], "chars" => results[0]["content"].length)
      return results[0]["content"]
    end

    if format == "json"
      __mod_emit_status("output", "count" => results.length, "total_chars" => results.sum { |r| r["char_count"] })
      return results
    end
    combined = results.map { |r| { "id" => r["id"], "content" => r["content"] } }
    __mod_emit_status("output", "count" => combined.length, "total_chars" => combined.sum { |r| r["content"].length })
    combined
  end
  ...
  module ModularBridge
    ...
    def call(name, args)
      ...
      payload = JSON.generate("session" => session, "run" => $__mod_current_rid, "name" => name, "args" => args)
  ...
  class ModularToolCallable
    def initialize(name)
      @name = name
    end

    def call(args = nil, **kwargs)
      ModularBridge.tool_call(@name, args, kwargs)
    end
  ...
  class ModularToolProxy < BasicObject
    def method_missing(name, args = nil, **kwargs)
      ::ModularBridge.tool_call(name.to_s, args, kwargs)
    end

    def [](name)
      ::ModularToolCallable.new(name.to_s)
    end
  ...
  def tool
    $__mod_tool_proxy ||= ModularToolProxy.new
  end
  ...
  def completion(prompt, model: "default", system: nil, schema: nil)
    ...
    res = ModularBridge.call("__completion__", args)
  ...
  def agent(prompt, agent: "task", model: nil, label: nil, schema: nil, isolated: nil, apply: nil, merge: nil, handle: false)
    ...
    res = ModularBridge.call("__agent__", args)
    text = res.is_a?(Hash) ? res["text"] : res
  ...
  def __mod_concurrency_limit
    snap = (ModularBridge.call("__concurrency__", {}) rescue nil) || {}
    n = (snap["limit"] || 0).to_i
    n > 0 ? n : 0
  rescue StandardError
    0
  ...
  def __mod_pool_map(items)
    arr = items.to_a
    return [] if arr.empty?
    limit = __mod_concurrency_limit
  ...
  def pipeline(items, *stages)
    ...
    stages.each do |stage|
      ...
      current = __mod_pool_map(current) { |item| stage.call(item) }
    end
    current
  end
  ...
  def log(message)
    __mod_emit_status("log", "message" => message.to_s)
    nil
  end

  def phase(title)
    $__mod_current_phase = title.to_s
    __mod_emit_status("phase", "title" => title.to_s)
    nil
  end
  ...
  class ModularBudget
    def total
      snap = (ModularBridge.call("__budget__", {}) || {})
      snap["total"]
    end
    ...
    def inspect
      snap = ((ModularBridge.call("__budget__", {}) rescue nil) || {})
  ...
  def budget
    $__mod_budget ||= ModularBudget.new
  end
end
