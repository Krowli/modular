...
# MODULAR Ruby runner — subprocess wrapper used by the coding-agent host.
#
...
$__mod_out_mutex = Mutex.new
$__mod_raw_stderr = (STDERR.dup rescue STDERR)
$__mod_current_rid = nil
$__mod_capture_rid = nil
$__mod_exec_count = 0
$__mod_active_exec = 0
$__mod_silent = false

begin
  $__mod_frame_io = STDOUT.dup
  $__mod_frame_io.sync = true
  __mod_stdout_cap_r, __mod_stdout_cap_w = IO.pipe
  STDOUT.reopen(__mod_stdout_cap_w)
  STDOUT.sync = true
  __mod_stdout_cap_w.close
  $__mod_stdout_capture_read = __mod_stdout_cap_r
rescue StandardError
  $__mod_frame_io = STDOUT
  ($__mod_frame_io.sync = true) rescue nil
  $__mod_stdout_capture_read = nil
end

begin
  __mod_stderr_cap_r, __mod_stderr_cap_w = IO.pipe
  STDERR.reopen(__mod_stderr_cap_w)
  STDERR.sync = true
  __mod_stderr_cap_w.close
  $__mod_stderr_capture_read = __mod_stderr_cap_r
rescue StandardError
  $__mod_stderr_capture_read = nil
end
...
begin
  $__mod_proto_stdin = STDIN.dup
  STDIN.reopen(File.open(File::NULL, "r"))
rescue StandardError
  $__mod_proto_stdin = STDIN
end
...
def __mod_emit(frame)
  ...
  $__mod_out_mutex.synchronize do
    $__mod_frame_io.write(line)
    $__mod_frame_io.write("\n")
    $__mod_frame_io.flush
  end
rescue StandardError
  nil
end
...
def __mod_emit_stream(kind, text)
  rid = $__mod_current_rid
  if rid.nil?
    ($__mod_raw_stderr.write(text) rescue nil)
    return
  end
  __mod_emit("type" => kind, "id" => rid, "data" => __mod_scrub(text))
end

def __mod_emit_display(bundle, kind = "display")
  rid = $__mod_current_rid
  return if rid.nil?
  __mod_emit("type" => kind, "id" => rid, "bundle" => bundle)
end

def __mod_emit_status(op, data = {})
  status = { "op" => op.to_s }
  data.each { |k, v| status[k.to_s] = v }
  __mod_emit_display({ "application/x-modular-status" => status }, "display")
end

MODULAR_IMAGE_MIMES = %w[image/png image/jpeg].freeze
...
def __mod_image_payload(content)
  require "base64"
  s = content.to_s
  return s.gsub(/\s+/, "") if __mod_base64?(s)
...
def __mod_sniff_image_mime(bytes)
  b = bytes.to_s.b
...
# bundle is always JSON-safe before it reaches __mod_emit.
def __mod_normalize_bundle(hash)
  bundle = {}
  hash.each do |key, val|
    k = key.to_s
    bundle[k] =
      if MODULAR_IMAGE_MIMES.include?(k)
        __mod_image_payload(val)
...
def __mod_finalize_bundle(bundle, value)
  bundle["text/plain"] ||= __mod_scrub((value.inspect rescue value.class.name))
  bundle
end

# Rich-display resolution for non-collection objects. Honors the repo
# `to_mod_mime` convention first, then the IRuby protocol
...
def __mod_rich_mime_bundle(value)
  if value.respond_to?(:to_mod_mime)
    mime = (value.to_mod_mime rescue nil)
    return __mod_finalize_bundle(__mod_normalize_bundle(mime), value) if mime.is_a?(Hash) && !mime.empty?
  end
  if value.respond_to?(:to_iruby_mimebundle)
    ...
    return __mod_finalize_bundle(__mod_normalize_bundle(data), value) if data.is_a?(Hash) && !data.empty?
  end
  if value.respond_to?(:to_iruby)
    pair = (value.to_iruby rescue nil)
    if pair.is_a?(Array) && pair.size == 2 && !pair[0].nil?
      return __mod_finalize_bundle(__mod_normalize_bundle({ pair[0].to_s => pair[1] }), value)
    end
  end
  ...
  if value.respond_to?(:to_png)
    ...
    return __mod_finalize_bundle({ "image/png" => __mod_image_payload(png) }, value) if png
  end
  jpeg_method = %i[to_jpeg to_jpg].find { |m| value.respond_to?(m) }
  if jpeg_method
    jpg = (value.public_send(jpeg_method) rescue nil)
    return __mod_finalize_bundle({ "image/jpeg" => __mod_image_payload(jpg) }, value) if jpg
  end
  if value.respond_to?(:to_blob)
    blob = (value.to_blob rescue nil)
    if blob.is_a?(String) && (mime = __mod_sniff_image_mime(blob))
      return __mod_finalize_bundle({ mime => __mod_image_payload(blob) }, value)
    end
  end
...
# Other objects may expose a rich representation via `to_mod_mime` or the IRuby
# protocol (`to_iruby`/`to_iruby_mimebundle`); otherwise they fall back to inspect.
def __mod_mime_bundle(value)
  case value
  when String
    { "text/plain" => __mod_scrub(value) }
  when Hash, Array
    safe = begin
    ...
    if safe.nil?
      { "text/plain" => __mod_scrub(value.inspect) }
    else
      { "application/json" => safe, "text/plain" => __mod_scrub(value.inspect) }
    end
  when nil
    { "text/plain" => "nil" }
  else
    __mod_rich_mime_bundle(value) || { "text/plain" => __mod_scrub(value.inspect) }
  end
end

def __mod_present(value, kind = "display")
  __mod_emit_display(__mod_mime_bundle(value), kind)
end
...
def __mod_start_capture_drain(io, kind)
  return if io.nil?
  Thread.new do
    loop do
      chunk =
      ...
      rid = $__mod_capture_rid
      if rid.nil?
        ($__mod_raw_stderr.write(chunk) rescue nil)
      else
        __mod_emit("type" => kind, "id" => rid, "data" => __mod_scrub(chunk))
      end
    end
  end
end

def __mod_start_parent_watchdog
...
def __mod_install_idle_sigint
  Signal.trap("INT", "IGNORE") rescue nil
end

def __mod_install_exec_sigint
  Signal.trap("INT", "DEFAULT") rescue nil
end

def __mod_begin_exec
  $__mod_active_exec += 1
  __mod_install_exec_sigint
end

def __mod_end_exec
  $__mod_active_exec -= 1 if $__mod_active_exec > 0
  __mod_install_idle_sigint if $__mod_active_exec.zero?
end
...
def __mod_apply_request_runtime(req)
  cwd = req["cwd"]
  ...
  env = req["env"]
  if env.is_a?(Hash)
    MODULAR_MANAGED_ENV_KEYS.each do |key|
...
def __mod_should_display_result?(src)
  ...
  last = __mod_ast_last(node)
  return true if last.nil?
  !MODULAR_NON_DISPLAY_NODES.include?(last.type)
end
...
def __mod_emit_error(rid, exc, name_override = nil)
  ...
  __mod_emit(
    ...
    "id" => rid,
    "ename" => ename,
    "evalue" => __mod_scrub(evalue),
    "traceback" => traceback.map { |l| __mod_scrub(l) },
  )
end

def __mod_handle_request(req)
  rid = req["id"].to_s
  $__mod_current_rid = rid
  $__mod_capture_rid = rid
  $__mod_silent = req["silent"] == true
  $__mod_exec_count += 1
  count = $__mod_exec_count
  __mod_emit("type" => "started", "id" => rid)

  status = "ok"
  cancelled = false
  begin
    begin
      __mod_apply_request_runtime(req)
      src = req["code"].to_s
    rescue Exception => e # rubocop:disable Lint/RescueException
      __mod_emit_error(rid, e)
      __mod_emit("type" => "done", "id" => rid, "status" => "error", "executionCount" => count, "cancelled" => false)
      return
    end
    ...
    begin
      ...
      unless $__mod_silent || value.nil? || !__mod_should_display_result?(src)
        __mod_present(value, "result")
      end
    rescue Interrupt => e
      cancelled = true
      status = "error"
      __mod_emit_error(rid, e, "Interrupt")
    rescue SystemExit => e
      status = "error"
      __mod_emit_error(rid, e)
    rescue Exception => e # rubocop:disable Lint/RescueException
    ...
    __mod_emit("type" => "done", "id" => rid, "status" => status, "executionCount" => count, "cancelled" => cancelled)
  ensure
    $__mod_capture_rid = nil if $__mod_capture_rid == rid
    $__mod_current_rid = nil
  end
end
...
def __mod_main
  $stdout = ModularStreamProxy.new("stdout", STDOUT, 1)
  $stderr = ModularStreamProxy.new("stderr", STDERR, 2)
  __mod_install_idle_sigint
  __mod_start_parent_watchdog
  __mod_start_capture_drain($__mod_stdout_capture_read, "stdout")
  __mod_start_capture_drain($__mod_stderr_capture_read, "stderr")

  $__mod_proto_stdin.each_line do |raw|
    line = raw.strip
    next if line.empty?
    req =
      begin
    ...
    __mod_handle_request(req) if req.is_a?(Hash)
  end
end
