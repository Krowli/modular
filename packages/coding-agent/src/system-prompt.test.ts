...
async function runProbeScenario(options: {
	...
	const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "modular-gpu-probe-"));
	try {
		const binDir = path.join(tempRoot, "bin");
		const cacheRoot = path.join(tempRoot, "cache");
		const probeCountPath = path.join(tempRoot, "probe-count");
		await fs.mkdir(binDir, { recursive: true });
		await fs.mkdir(path.join(cacheRoot, "modular"), { recursive: true });
		const lspciPath = path.join(binDir, "lspci");
		await Bun.write(
			lspciPath,
			'#!/usr/bin/env sh\nprintf x >> "$MODULAR_GPU_PROBE_COUNT"\nif [ -n "$MODULAR_GPU_PROBE_VALID_OUTPUT" ]; then printf "%s\\n" "$MODULAR_GPU_PROBE_VALID_OUTPUT"; fi\nif [ "$MODULAR_GPU_PROBE_DESCENDANT_HOLDS_STDOUT" = "true" ]; then sleep "$MODULAR_GPU_PROBE_SLEEP" & exit 0; fi\nif [ "$MODULAR_GPU_PROBE_HOLD_STDOUT_OPEN" = "true" ]; then sleep "$MODULAR_GPU_PROBE_SLEEP" & wait "$!"; fi\nif [ -n "$MODULAR_GPU_PROBE_SLEEP" ]; then exec sleep "$MODULAR_GPU_PROBE_SLEEP"; fi\nexit 0\n',
		);
...
for (let index = 0; index < Number(process.env.MODULAR_GPU_PROBE_RUNS ?? "1"); index += 1) {
...
const countFile = Bun.file(process.env.MODULAR_GPU_PROBE_COUNT ?? "");
...
`,
		...
		const env: Record<string, string | undefined> = {
			...
			MODULAR_GPU_PROBE_COUNT: probeCountPath,
			MODULAR_GPU_PROBE_RUNS: String(options.runs),
		};
		...
		for (const key of ["PI_CODING_AGENT_DIR", "MODULAR_PROFILE", "PI_PROFILE", "PI_CONFIG_DIR"]) {
			delete env[key];
		}
		if (options.sleepSeconds === undefined) {
			delete env.MODULAR_GPU_PROBE_SLEEP;
		} else {
			env.MODULAR_GPU_PROBE_SLEEP = String(options.sleepSeconds);
		}
		if (options.holdStdoutOpen) {
			env.MODULAR_GPU_PROBE_HOLD_STDOUT_OPEN = "true";
		} else {
			delete env.MODULAR_GPU_PROBE_HOLD_STDOUT_OPEN;
		}
		if (options.descendantHoldsStdout) {
			env.MODULAR_GPU_PROBE_DESCENDANT_HOLDS_STDOUT = "true";
		} else {
			delete env.MODULAR_GPU_PROBE_DESCENDANT_HOLDS_STDOUT;
		}
		if (options.validOutput !== undefined) {
			env.MODULAR_GPU_PROBE_VALID_OUTPUT = options.validOutput;
		} else {
			delete env.MODULAR_GPU_PROBE_VALID_OUTPUT;
		}
