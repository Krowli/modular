    (function() {
      ...
      // ============================================================
      //
      // Served by the modular relay at /s/<id>; the AES-256-GCM key rides in the
      ...
      async function fetchGistBlob(id) {
        ...
        var file = files.find(function(f) { return /\.modularshare\.txt$/.test(f.filename); }) || files[0];
      ...
      async function load() {
        ...
        if (data && data.header && data.header.title) {
          document.title = data.header.title + ' — modular session';
        }
        return data;
      }
