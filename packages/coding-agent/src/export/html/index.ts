...
import type { AgentState } from "@modular/pi-agent-core";
import { APP_NAME, isEnoent } from "@modular/pi-utils";
...
export interface ExportOptions {
	 ...
	 * - `"web"` (default) — the modular brand identity (collab-web pink/purple),
	 *   so public HTML exports and the `/s/<id>` share viewer match the live
	 *   `my.modularcli.com` client. See `web-palette.ts`.
 ...
 *     `"web"` (the default for public artifacts) returns the fixed modular brand
 *     palette from `web-palette.ts` — collab-web pink/purple identity, shared
 *     with the live `my.modularcli.com` client, so exports and the share viewer render
