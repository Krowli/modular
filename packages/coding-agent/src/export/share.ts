...
import type { AgentMessage, AgentState } from "@modular/pi-agent-core";
import type { AssistantMessage, ImageContent, TextContent } from "@modular/pi-ai";
import { $which, logger } from "@modular/pi-utils";
...
const GIST_FILENAME = "session.modularshare.txt";
...
async function tryCreateGist(sealed: Uint8Array): Promise<{ id: string; url: string } | null> {
	...
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-share-"));
	try {
