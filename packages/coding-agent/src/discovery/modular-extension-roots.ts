/**
 * MODULAR extension package roots.
 *
 ...
 * `prompts/`, `.mcp.json`) are wired into discovery by `modular-plugins.ts`.
 *
 ...
 *
 * @see ./modular-plugins.ts
 * @see ./builtin.ts `loadExtensionModules`
 */
import * as fs from "node:fs/promises";
import * as path from "node:path";
import { getAgentDir, isEnoent, logger, tryParseJson } from "@modular/pi-utils";
...
export interface ModularExtensionRoot {
...
export function getInjectedOmpExtensionCliRoots(): readonly ModularExtensionRoot[] {
...
function scopeDirs(ctx: LoadContext): ScopeDirs {
	return {
		project: path.join(ctx.cwd, ".modular"),
		user: getAgentDir(),
	};
}
 ...
 * 2. Project `<cwd>/.modular/settings.json#extensions`
 * 3. User `~/.modular/agent/settings.json#extensions`
 * 4. Enabled npm/link plugins installed under `<plugins>/node_modules/` (for
 *    `modular install <pkg>` / `modular plugin install` / `modular plugin link`). Marketplace
 ...
 */
export async function listOmpExtensionRoots(ctx: LoadContext): Promise<ModularExtensionRoot[]> {
	...
	const roots: ModularExtensionRoot[] = [];
 ...
 * "Claude Code Marketplace" and "MODULAR Extension Packages".
 */
