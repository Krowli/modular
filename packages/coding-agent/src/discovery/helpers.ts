...
import { FileType, glob } from "@modular/pi-natives";
import {
	CONFIG_DIR_NAME,
	getAgentDir,
	getConfigDirName,
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ modular?: ExtensionModuleManifest; pi?: ExtensionModuleManifest }>(content);
	const manifest = pkg?.modular ?? pkg?.pi;
 ...
 * 3. Subdirectory with package.json: `extensions/<ext>/package.json` with "modular"/"pi" field → load declared paths
 *
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.modular/`.
 *    The first match returns `<dir>/.modular/plugins/installed_plugins.json`.
 * 2. If no `.modular/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.modular/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .modular/ directory (nearest wins).
	// Stop before os.homedir() — ~/.modular/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.modular/plugins/installed_plugins.json`
 * when no project anchor (.modular/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .modular/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.modular/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	// ── MODULAR installed plugins registry ───────────────────────────────────────
	// MODULAR registry is authoritative: its entries replace Claude's entries for the same plugin ID.
	...
	const modRegistryPath = path.join(getPluginsDir(home), "installed_plugins.json");
	const modContent = await readFile(modRegistryPath);
	if (modContent) {
		const modRegistry = parseClaudePluginsRegistry(modContent);
		if (modRegistry) {
			for (const [pluginId, entries] of Object.entries(modRegistry.plugins)) {
				if (!Array.isArray(entries) || entries.length === 0) continue;
				...
				// MODULAR is authoritative: drop all Claude-sourced entries for this plugin ID
			...
			warnings.push(`Failed to parse MODULAR plugin registry: ${modRegistryPath}`);
		}
	}

	// ── Project-scoped MODULAR registry ────────────────────────────────────────
	// Loaded from the nearest .modular/plugins/installed_plugins.json relative to cwd.
 ...
 * These are prepended to the cache with highest precedence (before MODULAR/Claude entries).
