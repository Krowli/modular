...
import { logger, parseFrontmatter } from "@modular/pi-utils";
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	...
	// MODULAR hooks must be named `pre-<tool>.<ts|js>` or `post-<tool>.<ts|js>`.
	// Files without that prefix are not MODULAR hooks (e.g. the standalone Codex
