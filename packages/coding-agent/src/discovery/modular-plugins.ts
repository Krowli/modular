/**
 * MODULAR extension-package sub-discovery provider.
 *
 ...
 * `rules/`, `prompts/`, and `.mcp.json` — are picked up by modular's standard
 * discovery surfaces. The native `modular` provider in `builtin.ts` only walks
 * `.modular/` and `~/.modular/agent/`, so without this provider those sub-trees are
 * silently ignored.
 *
 * Provider priority is set below the native `modular` provider (100) so an
 * extension package never shadows the user's own `.modular/` configuration on
 * dedup.
 *
 * @see ./modular-extension-roots.ts
 * @see ../../docs/extension-loading.md
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@modular/pi-utils";
...
import { listOmpExtensionRoots, type ModularExtensionRoot } from "./modular-extension-roots";

const PROVIDER_ID = "modular-plugins";
const DISPLAY_NAME = "MODULAR Extension Packages";
const DESCRIPTION =
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	const roots = await listOmpExtensionRoots(ctx);
	const tasks: Array<{ root: ModularExtensionRoot; hookType: "pre" | "post" }> = [];
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	const tasks: Array<{ root: ModularExtensionRoot; mcpPath: string }> = [];
	for (const root of roots) {
	...
	for (let i = 0; i < tasks.length; i++) {
		...
		if (!parsed) {
			warnings.push(`[modular-plugins] Invalid JSON in ${mcpPath}`);
			logger.warn(`[modular-plugins] Invalid JSON in ${mcpPath}`);
			continue;
		}
		...
		for (const [serverName, serverCfg] of Object.entries(servers)) {
			...
			if (typeof cfg.command !== "string" && typeof cfg.url !== "string") {
				warnings.push(
					`[modular-plugins] Skipping MCP server "${serverName}" in ${mcpPath}: missing command or url`,
				);
				continue;
			}
			items.push({
