...
import { $which, logger, Snowflake } from "@modular/pi-utils";
...
$r = Mci "open new type waveaudio alias modular_rec"
if ($r -ne 0) { exit 1 }

Mci "set modular_rec channels 1 samplespersec 16000 bitspersample 16"

$r = Mci "record modular_rec"
if ($r -ne 0) {
    Mci "close modular_rec"
    exit 1
}
...
# Stop and save
Mci "stop modular_rec"
$saveCmd = 'save modular_rec "' + $outPath + '"'
$r = Mci $saveCmd
if ($r -ne 0) {
...
async function startPowerShellRecording(outputPath: string): Promise<RecordingHandle> {
	...
	const scriptPath = path.join(os.tmpdir(), `modular-stt-record-${Snowflake.next()}.ps1`);
...
export async function startRecording(outputPath: string): Promise<RecordingHandle> {
	...
	if (recorders.length === 0) {
		throw new Error("No audio recorder available — run `modular setup speech`");
	}
	...
	throw new Error(`No audio recorder could start — run \`modular setup speech\`.\n${failures.join("\n")}`);
}
