...
import { getProjectDir, readJsonl, Snowflake } from "@modular/pi-utils";
...
export interface StdioSpawnCommand {
	 ...
	 *
	 * Windows uses this only when the MODULAR host has no console to share. When
	 ...
	 * wrapper chains must stay in the MODULAR console session so nested console
...
export async function resolveStdioSpawnCommand(
	...
	// same console session. Only hide the child when MODULAR itself has no console
