...
import type { OAuthCallbackFlowOptions } from "@modular/pi-ai/oauth/callback-server";
import { OAuthCallbackFlow } from "@modular/pi-ai/oauth/callback-server";
import type { OAuthController, OAuthCredentials } from "@modular/pi-ai/oauth/types";
import type { FetchImpl } from "@modular/pi-ai/types";
import { getActiveProfile } from "@modular/pi-utils/dirs";
import type { OAuthCredential } from "../session/auth-storage";

/** Credential-id prefix for MODULAR-managed MCP OAuth credentials keyed by profile and server URL. */
...
/**
 * Deterministic credential id for an MCP server URL scoped to an MODULAR profile.
 *
...
/** Whether a credential id was minted by MODULAR's MCP OAuth flows (either era). */
 ...
 *
 * Plane is stricter for MODULAR-synthesized fallback resources (e.g. using the
...
export class MCPOAuthFlow extends OAuthCallbackFlow {
	...
	async generateAuthUrl(state: string, redirectUri: string): Promise<{ url: string; instructions?: string }> {
		...
		if (existingResource) {
			...
			// provider-authored, not MODULAR's server-URL fallback. Preserve same-host
