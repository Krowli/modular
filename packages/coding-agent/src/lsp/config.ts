...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@modular/pi-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.modular/, .pi/, .claude/)
	...
	// User config directories (~/.modular/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .modular/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.modular/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
