...
import type * as MnemopiNs from "@modular/pi-mnemopi";
import type { Mnemopi, RecallResult } from "@modular/pi-mnemopi";
import type * as MnemopiCoreNs from "@modular/pi-mnemopi/core";
import type { LocalModelInitializer } from "@modular/pi-mnemopi/core";
...
/**
 * Lazily load `@modular/pi-mnemopi` (memoized) and route fastembed loads
...
export async function loadMnemopi(): Promise<typeof MnemopiNs> {
	if (!mnemopiMod) {
		mnemopiMod = await import("@modular/pi-mnemopi");
		installLocalModelInitializer(mnemopiMod.setLocalModelInitializer);
	}
	return mnemopiMod;
}

/** Lazily load `@modular/pi-mnemopi/core` (memoized). */
export async function loadMnemopiCore(): Promise<typeof MnemopiCoreNs> {
	if (!mnemopiCoreMod) {
		mnemopiCoreMod = await import("@modular/pi-mnemopi/core");
