import type { Model, ProviderResponseMetadata, RawSseEvent } from "@modular/pi-ai";
...
function trimRawLines(raw: string[]): TrimResult {
	...
	const tail = `: modular-debug-truncated originalChars=${originalChars}`;
	trimmed.push(tail);
	chars += tail.length + 1;
...
export class RawSseDebugBuffer {
	...
	toRawText(): string {
		...
		const dropped = `: modular-debug-dropped records=${this.#droppedRecords} chars=${this.#droppedChars}\n\n`;
