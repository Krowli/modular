...
import type { WorkProfile } from "@modular/pi-natives";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@modular/pi-utils";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `modular-report-${timestamp}.tar.gz`);
