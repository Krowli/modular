 ...
 *
 * modular's agent core (`@modular/pi-agent-core`) emits OpenTelemetry GenAI
 ...
 * spawns `modular` as a child process) had no way to collect modular's internal traces.
 *
 ...
 * consuming process configures the destination entirely through env; modular stays
...
/**
 * Periodic flush interval. A long-lived `modular` process (the ACP server is
...
async function registerProvider(): Promise<void> {
	...
	const tracerProvider = new NodeTracerProvider({
		resource: resourceFromAttributes({
			"service.name": process.env.OTEL_SERVICE_NAME ?? "modular",
		}),
