...
import type * as natives from "@modular/pi-natives";
 ...
 *
 * Branch mode: on success, commits the diff onto `modular/task/${agentId}` and
	...
	try {
		...
		if (opts.mergeMode === "branch" && result.exitCode === 0) {
			try {
				...
				const branchName = `modular/task/${opts.agentId}`;
