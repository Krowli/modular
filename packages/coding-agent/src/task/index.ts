 ...
 *   - Bundled agents (shipped with modular-coding-agent)
...
import type { AgentTool, AgentToolResult, AgentToolUpdateCallback } from "@modular/pi-agent-core";
import type { Usage } from "@modular/pi-ai";
import { $env, logger, prompt, Snowflake } from "@modular/pi-utils";
...
export class TaskTool implements AgentTool<TaskToolSchemaInstance, TaskToolDetails, Theme> {
	...
	async #runSpawn(
		...
		const tempArtifactsDir = artifactsDir ? null : path.join(os.tmpdir(), `modular-task-${Snowflake.next()}`);
