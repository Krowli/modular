 ...
 *
 * Discovers agent definitions from MODULAR-native task-agent roots:
 *   - ~/.modular/agent/agents/*.md (user-level)
 *   - .modular/agents/*.md (project-level)
 *   - <ext>/agents/*.md for every MODULAR extension package wired through
 ...
 *     `hooks/`, `tools/`, etc. by `discovery/modular-plugins.ts`.
 *
 ...
 * are intentionally skipped because their frontmatter schema is not the MODULAR
 * task-agent contract.
 *
...
import { listOmpExtensionRoots } from "../discovery/modular-extension-roots";
import { loadBundledAgents, parseAgent } from "./agents";
import type { AgentDefinition, AgentSource } from "./types";

const TASK_AGENT_CONFIG_SOURCE = ".modular";
 ...
 * Precedence (highest wins): project `.modular/agents`, user `.modular/agents`,
 * MODULAR extension-package agents in `listOmpExtensionRoots` source order
...
export async function discoverAgents(cwd: string, home: string = os.homedir()): Promise<DiscoveryResult> {
	...
	// MODULAR extension-package agents/ dirs. `listOmpExtensionRoots` returns roots in
	...
	// surface in `discovery/modular-plugins.ts`. Gate on `modular-plugins` so
	// disabledProviders suppresses the whole extension-package surface.
	const extensionRoots = isProviderEnabled("modular-plugins")
