...
import { CompactionCancelledError, type CompactionOutcome } from "@modular/pi-agent-core/compaction";
import {
	getEnvApiKey,
	getProviderDetails,
...
import { Loader, Markdown, padding, Spacer, Text, visibleWidth } from "@modular/pi-tui";
import { formatDuration, Snowflake, sanitizeText } from "@modular/pi-utils";
...
export function renderUsageReports(
	...
	for (const { provider, providerReports } of providerEntries) {
		...
		// Provider-wide disclaimers (e.g. "MODULAR-observed spend only") render once
