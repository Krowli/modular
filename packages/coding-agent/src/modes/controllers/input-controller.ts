...
import { type AutocompleteProvider, matchesKey, type SlashCommand } from "@modular/pi-tui";
import { $env, isEnoent, logger, sanitizeText } from "@modular/pi-utils";
...
const MODULAR_STATUS_LINE_RE = /^\s*in:\s+\d+\s+out:\s+\d+(?:\s+cache\s+\S+)?\s+t:\s+\S+\s+tok\/s:\s+\S+/m;
...
function looksLikePastedShellPrompt(code: string): boolean {
	...
	return (
		...
		MODULAR_STATUS_LINE_RE.test(code)
	);
}
...
export class InputController {
	...
	handleCtrlZ(): void {
		...
		try {
			...
			// longer stops modular: tokio swallows it and the TUI ends up torn down
			...
			//
			// pid=0 (foreground process group, not just our PID): modular is not
			// always the shell's direct child. Package-manager launchers (`npx`,
			// `pnpm exec`, `bunx`, …) wait on the real CLI from a parent shim
			// that shares modular's process group, and a `modular … | tee log` style
	...
	async openExternalEditor(): Promise<void> {
		...
		try {
			...
			const result = await openInEditor(editorCmd, currentText, { extension: ".modular.md", stdio });
			if (result !== null) {
