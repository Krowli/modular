...
import { getBlobsDir, isEnoent, logger, VERSION } from "@modular/pi-utils";
	...
	async initialize(params: InitializeRequest): Promise<InitializeResponse> {
		...
		const authMethods: AuthMethod[] = [
			{
				...
				description: "Authenticate via the provider keys/OAuth state already configured under ~/.modular.",
			},
		];
		if (params.clientCapabilities?.auth?.terminal === true) {
			authMethods.push({
				type: "terminal",
				id: "terminal",
				name: "Set up Modular in terminal",
				description: "Launch the modular TUI to add provider keys and select models.",
