...
import type { CompactionOutcome } from "@modular/pi-agent-core/compaction";
import type { AssistantMessage, ImageContent, Message, Model, Usage, UsageReport } from "@modular/pi-ai";
...
import { isInsideTerminalMultiplexer } from "@modular/pi-tui/terminal-capabilities";
import {
	APP_NAME,
	adjustHsv,
	formatNumber,
...
export class InteractiveMode implements InteractiveModeContext {
	...
	async init(options: InteractiveModeInitOptions = {}): Promise<void> {
		...
		// Start the UI. Cold `modular` launch opts into clearing on the first paint so
		...
		// ignored. This way `modular --continue` (or auto-resume) that finds no recent
	...
	async shutdown(): Promise<void> {
		...
		// hands Bash the cursor immediately after visible MODULAR content.
