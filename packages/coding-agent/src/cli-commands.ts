 ...
 */
import type { CommandEntry } from "@modular/pi-utils/cli";
...
// commands. Without a guard `resolveCliArgv` rewrites e.g. `modular list` to
// `modular launch list`, silently forwarding the bare verb to the model as a prompt
...
// `modular plugin <action>` command; multi-word invocations still fall through to
...
const RESERVED_TOP_LEVEL_WORDS = new Map<string, string>([
	[
		"extensions",
		'`modular extensions` is not a management command. Use `modular plugin list` / `modular plugin install`, or run `modular launch extensions` if you meant to send "extensions" as a prompt.',
	],
	[
		"list",
		'`modular list` is not a top-level command. Use `modular plugin list` to list installed plugins, or run `modular launch list` if you meant to send "list" as a prompt.',
	],
	[
		"remove",
		'`modular remove` is not a top-level command. Use `modular plugin uninstall <name>` to remove a plugin, or run `modular launch remove` if you meant to send "remove" as a prompt.',
	],
]);
...
export function resolveCliArgv(argv: string[]): ResolvedCliArgv {
	...
	// (`modular --approval-mode=yolo acp`). `run` dispatches strictly on argv[0], so
