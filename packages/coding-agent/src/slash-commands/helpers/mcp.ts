...
import { getMCPConfigPath, logger } from "@modular/pi-utils";
...
async function handleSmitherySearchCommand(rest: string, runtime: SlashCommandRuntime): Promise<SlashCommandResult> {
	...
	try {
		...
		if (AIError.is(AIError.classify(err), AIError.Flag.AuthFailed)) {
			return usage(
				"Smithery authentication required. Run /mcp smithery-login in the TUI client or add an API key to ~/.modular/agent/smithery.json.",
				runtime,
			);
		}
