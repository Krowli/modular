...
import { getAgentDir, isEnoent, logger, prompt } from "@modular/pi-utils";
 ...
 * `.modular/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.modular/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (both standalone and native config .modular/): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
			candidates.add(path.resolve(current, ".modular", filename));
	...
	for (const candidate of candidates) {
		try {
			...
			const ownerDir = baseName === ".modular" ? path.dirname(parent) : parent;
			const ownerBaseName = ownerDir.split(path.sep).pop() ?? "";
			if (isUser || !ownerBaseName.startsWith(".") || baseName === ".modular") {
...
/**
 * Discover and load WATCHDOG.md files walking up from cwd, project .modular folder, and user agent dir.
