...
import type { Api, Effort, KnownProvider, Model, ModelSpec } from "@modular/pi-ai";
import { buildModel } from "@modular/pi-catalog/build";
import { modelMatchesHost } from "@modular/pi-catalog/hosts";
import { buildModelProviderPriorityRank } from "@modular/pi-catalog/identity";
import { stripThinkingVariantToken } from "@modular/pi-catalog/identity/family";
import { clampThinkingLevelForModel } from "@modular/pi-catalog/model-thinking";
import { modelsAreEqual } from "@modular/pi-catalog/models";
import { DEFAULT_MODEL_PER_PROVIDER } from "@modular/pi-catalog/provider-models";
import { resolveBareVariantAlias, resolveVariantAlias } from "@modular/pi-catalog/variant-collapse";
import { fuzzyMatch } from "@modular/pi-tui";
...
export function resolveCliModel(options: {
	...
	if (cliProvider && !provider) {
		return {
			...
			selector: undefined,
			warning: undefined,
			error: `Unknown provider "${cliProvider}". Run "modular models" to see available providers/models.`,
		};
	}
	...
	if (!model) {
		...
		return {
			...
			thinkingLevel: undefined,
			warning,
			error: `Model "${display}" not found. Run "modular models" to see available models.`,
		};
	}
