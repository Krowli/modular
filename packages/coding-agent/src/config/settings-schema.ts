import { THINKING_EFFORTS } from "@modular/pi-ai";
import { DEFAULT_SHARE_URL } from "@modular/pi-wire";
import { SHAPE_VARIANT_NAMES } from "@modular/snapcompact";
...
export const DEFAULT_BASH_INTERCEPTOR_RULES: BashInterceptorRule[] = [
	...
	},
	{
		...
		tool: "launch",
		message:
			"Use the `launch` tool for services, watchers, and debuggers so other modular instances can observe and control them.",
	},
	{
		pattern:
...
export const SETTINGS_SCHEMA = {
	...
	// Auth broker — credentials proxied through a remote `modular auth-broker serve`
	// host. Hidden from the UI; populate via env vars or hand-edited config.yml.
	// Env (`MODULAR_AUTH_BROKER_URL` / `MODULAR_AUTH_BROKER_TOKEN`) takes precedence so
	...
	"providers.maxInFlightRequests": {
		...
		ui: {
			...
			description:
				'Maximum concurrent LLM requests per provider id (for example "openai" or "anthropic"), shared across local MODULAR processes with this config root. Omitted providers are unlimited.',
		},
	},
	...
	"startup.checkUpdate": {
		...
		ui: {
			...
			description: "Check for modular updates on startup",
		},
	},
	...
	// and mint/enhance isolated managed skills under ~/.modular/agent/managed-skills.
	...
	"hindsight.retainContext": { type: "string", default: "modular" },
	...
	"github.cache.enabled": {
		...
		ui: {
			...
			description:
				"Cache rendered issue/PR view output in ~/.modular/cache/github-cache.db so repeated reads are free",
		},
	},
	...
	"worktree.base": {
		...
		ui: {
			...
			description:
				"Base directory for agent-managed worktrees — task-isolation copies, `github` PR checkouts, and `modular worktree` cleanup all live here. Unset uses ~/.modular/wt. Must be an absolute or ~-relative path; relative paths are ignored. The MODULAR_WORKTREE_DIR env var overrides this.",
		},
	},
	...
	"dev.autoqaPush.endpoint": {
		...
		ui: {
			...
			description: "Full URL receiving Auto QA JSON reports. Empty = local-only, nothing is sent anywhere.",
		},
	},
