...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@modular/pi-utils";
 ...
 * project with modular's own assets and was the source of issue #1423 (the host
 * project's `CHANGELOG.md` rendered as modular's startup changelog).
 */
...
/**
 * Path to modular's own `CHANGELOG.md`, or `undefined` when the package directory
 ...
 * User-level: ~/.modular/agent, ~/.claude, ~/.codex, ~/.gemini
 * Project-level: .modular, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".modular", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.modular/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.modular/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.modular/agent/commands", source: ".modular", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.modular, .claude) - the nearest one found.
