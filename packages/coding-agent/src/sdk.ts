...
import type { Context, CredentialDisabledEvent, Message, Model, SimpleStreamOptions } from "@modular/pi-ai";
import type { Dialect } from "@modular/pi-ai/dialect";
import {
	getOpenAICodexTransportDetails,
	prewarmOpenAICodexResponses,
} from "@modular/pi-ai/providers/openai-codex-responses";
import { FALLBACK_DIALECT, preferredDialect } from "@modular/pi-catalog/identity";
import type { Component } from "@modular/pi-tui";
import { $env, $flag, getAgentDir, getProjectDir, logger, postmortem, prompt, Snowflake } from "@modular/pi-utils";
import { INTENT_FIELD } from "@modular/pi-wire";
import {
...
export interface CreateAgentSessionOptions {
	...
	cwd?: string;
	/** Global config directory. Default: ~/.modular/agent */
	agentDir?: string;
	...
	/**
	 * Pre-discovered custom-tool source paths from `.modular/tools/`, `.claude/tools/`,
	...
	/** Prompt templates. Default: discovered from cwd/.modular/prompts/ + agentDir/prompts/ */
 ...
 *
 * Broker mode: when `MODULAR_AUTH_BROKER_URL` is set, credentials are pulled from
 ...
 * (`modular bench`, dry-balance) build a bare {@link ModelRegistry} that only knows
 ...
 * `~/.modular/agent/extensions/`) never reach model resolution. Mirrors the
 * session / `modular models` path: drain the queued provider registrations, then
...
function isLegacyBuiltinToolDefinition(tool: CustomTool | ToolDefinition): boolean {
	return !isCustomTool(tool) && "__modLegacyBuiltinTool" in tool && tool.__modLegacyBuiltinTool === true;
}
 ...
 *
 * // With explicit model
 * import { getModel } from '@modular/pi-ai';
 * const { session } = await createAgentSession({
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		// Discover custom tools from `.modular/tools/`, `.claude/tools/`, plugins, etc.
		...
		// cache that `modular models find` uses before the online refresh continues in
		// the background.
