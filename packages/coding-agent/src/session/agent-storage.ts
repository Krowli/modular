...
import { AsyncDrain, getAgentDbPath, getStatsDbPath, isRecord, logger } from "@modular/pi-utils";
...
/** Row shape read from an `modular stats` messages table during backfill. */
 ...
 * Delegates auth credential operations to AuthCredentialStore from @modular/pi-ai.
...
ON CONFLICT(model_key) DO UPDATE SET
	...
	#initializeSchema(): void {
		...
		// recovery). Without this, concurrent modular startups can crash here with
...
`);
	 ...
	 * `modular stats` database (`~/.modular/stats.db`) into model_perf. Fire-and-forget:
	 ...
	 * A missing stats.db leaves the marker unset so a later `modular stats` run
	...
	/**
	 * Imports recent measurable request rows from an `modular stats` database
