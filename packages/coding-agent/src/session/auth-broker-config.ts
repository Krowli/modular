/**
 * Resolve auth-broker connection configuration for the local modular client.
 *
 * This is a thin coding-agent wrapper around the shared resolver in
 * `@modular/pi-ai/auth-broker/discover` that preserves the process-lifetime
 ...
 *   1. `MODULAR_AUTH_BROKER_URL` / `MODULAR_AUTH_BROKER_TOKEN` env vars.
 *   2. `auth.broker.url` / `auth.broker.token` in `~/.modular/agent/config.yml`
 *      (hidden from the settings UI; `!command` resolution supported).
 *   3. Token file `~/.modular/auth-broker.token` (paired with URL from env or config).
 *
...
} from "@modular/pi-ai/auth-broker/discover";
import { getAgentDir } from "@modular/pi-utils";
 ...
 * that flip `MODULAR_AUTH_BROKER_*` between cases still observe the change, while
...
export function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const key = `${process.env.MODULAR_AUTH_BROKER_URL ?? ""}\u0000${process.env.MODULAR_AUTH_BROKER_TOKEN ?? ""}\u0000${getAgentDir()}`;
