...
import type { InMemorySnapshotStore } from "@modular/hashline";
import { Patch } from "@modular/hashline";
import {
...
} from "@modular/pi-agent-core/compaction";
import {
	DEFAULT_PRUNE_CONFIG,
	pruneSupersededToolResults,
	pruneToolOutputs,
	readToolSupersedeKey,
} from "@modular/pi-agent-core/compaction/pruning";
import type { ProtectedToolMatcher } from "@modular/pi-agent-core/compaction/tool-protection";
import type {
	AssistantMessage,
...
import * as AIError from "@modular/pi-ai/error";
import { resetOpenAICodexHistoryAfterCompaction } from "@modular/pi-ai/providers/openai-codex-responses";
import { toolWireSchema } from "@modular/pi-ai/utils/schema";
import { GeminiHeaderRunDetector, isGeminiThinkingModel } from "@modular/pi-ai/utils/thinking-loop";
import { type RepeatedToolCallDetection, ToolCallLoopGuard } from "@modular/pi-ai/utils/tool-call-loop-guard";
import { isFireworksFastModelId, toFireworksBaseModelId } from "@modular/pi-catalog/fireworks-model-id";
import { getSupportedEfforts } from "@modular/pi-catalog/model-thinking";
import { modelsAreEqual } from "@modular/pi-catalog/models";
import { MacOSPowerAssertion } from "@modular/pi-natives";
import {
	escapeXmlText,
...
import * as snapcompact from "@modular/snapcompact";
import {
...
function buildSessionMetadata(
	...
	if (provider === "anthropic") {
		...
		if (typeof accountUuid === "string" && accountUuid.length > 0) {
			...
			// identifier. Include both modular's persistent install id and the Claude
...
export class AgentSession {
	 ...
	 * `device_id` is derived from both the persistent modular install id and that
	...
	async #doDispose(options: AgentSessionDisposeOptions = {}): Promise<void> {
		...
		// unbounded when MODULAR_MCP_TIMEOUT_MS=0), so awaiting `disconnectAll()`
	...
	async exportToHtml(outputPath?: string): Promise<string> {
		// Public HTML export ships in the modular brand palette (collab-web
		// pink/purple), matching my.modularcli.com — not the host's terminal theme.
	...
	async dumpLlmRequestToTmpDir(): Promise<string | undefined> {
		...
		const filePath = path.join(os.tmpdir(), `modular-llm-request-${Snowflake.next()}.json`);
