/**
 * Protocol handler for modular:// URLs.
 *
 * Serves statically embedded documentation files bundled at build time.
 *
 * URL forms:
 * - modular:// - Lists all available documentation files
 * - modular://<file>.md - Reads a specific documentation file
 */
...
/**
 * Handler for modular:// URLs.
 *
 * Resolves documentation file names to their content, or lists available docs.
 */
export class ModularProtocolHandler implements ProtocolHandler {
	readonly scheme = "modular";
	...
	async #listDocs(url: InternalUrl): Promise<InternalResource> {
		...
		const listing = filenames.map(f => `- [${f}](modular://${f})`).join("\n");
	...
	async #readDoc(filename: string, url: InternalUrl): Promise<InternalResource> {
		...
		if (path.isAbsolute(filename)) {
			throw new Error("Absolute paths are not allowed in modular:// URLs");
		}

		const normalized = path.posix.normalize(filename.replaceAll("\\", "/"));
		if (normalized === ".." || normalized.startsWith("../") || normalized.includes("/../")) {
			throw new Error("Path traversal (..) is not allowed in modular:// URLs");
		}
		...
		if (content === undefined) {
			...
			const suffix =
				suggestions.length > 0
					...
					: "\nUse modular:// to list available files.";
