...
import { logger, postmortem } from "@modular/pi-utils";
...
echo "# Shell snapshot - generated by modular agent" >| "$SNAPSHOT_FILE"
...
${fnEnvHelper}
printf '%s\\n' "$__modular_funcs" | __modular_emit_referenced_exports >> "$SNAPSHOT_FILE"
unset -f __modular_sq_quote __modular_emit_export_for __modular_emit_referenced_exports 2>/dev/null
unset __modular_funcs __modular_qbuf __modular_qout __modular_sq __modular_xv __modular_name 2>/dev/null
...
export async function getOrCreateSnapshot(
	...
	const snapshotDir = path.join(os.tmpdir(), "modular-shell-snapshots");
