...
#
# `__modular_emit_referenced_exports` reads captured function bodies from stdin,
...
__modular_sq_quote() {
	...
	__modular_qout=
	__modular_sq=\'
	while case "$__modular_qbuf" in *$__modular_sq*) true ;; *) false ;; esac; do
		__modular_qout=$__modular_qout${__modular_qbuf%%$__modular_sq*}"'\\''"
		__modular_qbuf=${__modular_qbuf#*$__modular_sq}
	done
	__modular_qout=$__modular_qout$__modular_qbuf
	printf "'%s'" "$__modular_qout"
}
...
__modular_emit_export_for() {
	...
	eval "__modular_xv=\"\${$1}\"" 2>/dev/null || return
	printf 'export %s=%s\n' "$1" "$(__modular_sq_quote "$__modular_xv")"
}

# Read function bodies on stdin, emit dedup'd export lines on stdout.
__modular_emit_referenced_exports() {
	grep -oE '\$\{?[A-Za-z_][A-Za-z0-9_]*' \
		| sed -E 's/^\$\{?//' \
		| sort -u \
		| while IFS= read -r __modular_name; do
			__modular_emit_export_for "$__modular_name"
		done
}
