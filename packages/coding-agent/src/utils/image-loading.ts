...
import type { ImageContent, Model } from "@modular/pi-ai";
import { formatBytes, readImageMetadata, SUPPORTED_IMAGE_MIME_TYPES } from "@modular/pi-utils";
 ...
 * to PNG/JPEG instead — the automatic equivalent of `MODULAR_NO_WEBP=1`.
 */
 ...
 * `MODULAR_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor `MODULAR_NO_WEBP`. */
	excludeWebP?: boolean;
}
