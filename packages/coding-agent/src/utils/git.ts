...
import { $which, hasFsCode, isEisdir, isEnoent, isEnotdir, Snowflake } from "@modular/pi-utils";
...
async function writeTempPatch(content: string): Promise<string> {
	const tempPath = path.join(os.tmpdir(), `modular-git-patch-${Snowflake.next()}.patch`);
