...
export const miscFixtures: Record<string, GalleryFixture> = {
	...
	ssh: {
		...
		args: {
			host: "deploy@web-01",
			command: "systemctl status modular-api --no-pager | head -n 12",
			cwd: "/srv/modular",
			timeout: 60,
		},
		result: {
			content: [
				{
					type: "text",
					text: [
						"● modular-api.service - Modular API",
						"     Loaded: loaded (/etc/systemd/system/modular-api.service; enabled)",
						"     Active: active (running) since Sat 2026-06-06 09:14:02 UTC; 3h 21min ago",
						"   Main PID: 4812 (bun)",
						"      Tasks: 17 (limit: 4915)",
						"     Memory: 142.6M",
						"        CPU: 38.214s",
						"     CGroup: /system.slice/modular-api.service",
	...
	github: {
		...
		errorResult: {
			content: [
				{
					type: "text",
					text: "gh: Could not resolve to a Repository with the name 'modular/pi'. (HTTP 404)",
				},
			],
			isError: true,
		},
