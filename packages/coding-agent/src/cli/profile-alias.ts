...
import { normalizeProfileName } from "@modular/pi-utils/dirs";
...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "modular") {
		throw new Error('Invalid alias "modular". Refusing to shadow the base modular command.');
	}
...
function resolveShellConfigPath(
	...
	switch (shell) {
		...
		case "fish": {
			...
			return posixJoinUnc(configHome, "fish", "conf.d", "modular-profiles.fish");
		}
		case "pwsh":
...
function renderAliasBlock(
	...
	const start = `# >>> modular profile alias: ${aliasName} >>>`;
	const end = `# <<< modular profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps modular --description 'MODULAR profile ${profile}'`,
