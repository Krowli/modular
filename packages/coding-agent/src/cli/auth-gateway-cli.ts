/**
 * `modular auth-gateway` command handlers.
 *
 * Boots a forward-proxy server that lets less-trusted clients (the macOS
 * usage widget, robodular containers, …) make provider API calls without ever
 ...
 * `MODULAR_AUTH_BROKER_URL` / `auth.broker.url` precedence used elsewhere).
 *
 * Sub-verbs:
...
import { AuthBrokerClient, RemoteAuthCredentialStore, type SnapshotResponse } from "@modular/pi-ai/auth-broker";
import { DEFAULT_AUTH_GATEWAY_BIND, startAuthGateway } from "@modular/pi-ai/auth-gateway";
import { type GeneratedProvider, getBundledModels, getBundledProviders } from "@modular/pi-catalog/models";
import { getConfigRootDir, isEnoent, VERSION } from "@modular/pi-utils";
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`modular auth-gateway serve` requires MODULAR_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). The gateway is itself a broker client.",
		);
	}
...
async function runStatus(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		...
		if (flags.json) {
			...
			process.stdout.write(`${chalk.yellow("No broker configured.")} Set MODULAR_AUTH_BROKER_URL.\n`);
			process.stdout.write(
	...
	try {
		...
		if (flags.json) {
			...
			if (!tokenPresent) {
				process.stdout.write(
					"Run `modular auth-gateway token` or `modular auth-gateway serve` to create a bearer token.\n",
				);
			}
		}
...
/**
 * `modular auth-gateway check` — probe each broker-supplied credential and print
...
async function runCheck(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`modular auth-gateway check` requires MODULAR_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). It probes the same credentials the gateway would serve.",
		);
	}
