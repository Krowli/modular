 ...
 *
 * Handles `modular stats` subcommand for viewing AI usage statistics.
 */

import { truncateToWidth } from "@modular/pi-tui/utils";
import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@modular/pi-utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@modular/modular-stats");
	const stats = await getDashboardStats();
