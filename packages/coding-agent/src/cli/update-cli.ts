 ...
 *
 * Handles `modular update` to check for and install updates.
 * Uses the installer that owns the active modular executable when it can be detected.
 */
import * as fs from "node:fs";
import * as os from "node:os";
import * as path from "node:path";
import { pipeline } from "node:stream/promises";
import { $which, APP_NAME, isEnoent, VERSION } from "@modular/pi-utils";
import { $ } from "bun";
import chalk from "chalk";
import { theme } from "../modes/theme/theme";
import { isTimeoutError, withTimeoutSignal } from "../utils/fetch-timeout";

const REPO = "Krowli/modular";
const PACKAGE = "@modular/pi-coding-agent";
const HOMEBREW_FORMULA = "modular/tap/modular";
const MISE_TOOL = "github:Krowli/modular";
/**
 ...
 */
const NATIVES_PACKAGE = "@modular/pi-natives";

/**
 * Platform tags the release pipeline publishes as
 * `@modular/pi-natives-<tag>` leaves. Mirrors `SUPPORTED_PLATFORMS` in
...
function isPathInDirectory(filePath: string, directoryPath: string): boolean {
	...
	// PATH-resolved modular path can refer to the same directory through different
	...
	// links like Homebrew's `bin/modular -> Cellar/.../bin/modular`; the parent fallback
...
function resolveUpdateMethod(
	...
	const launcherExtension = path.extname(modPath).toLowerCase();
	const isWindowsScriptLauncher =
		launcherExtension === ".cmd" || launcherExtension === ".ps1" || launcherExtension === ".bat";
	if (homebrewPrefix && isPathInDirectory(modPath, path.join(homebrewPrefix, "bin"))) return "brew";
	if (miseBinDirs.some(dir => isPathInDirectory(modPath, dir))) return "mise";
	if (miseDataDir && isPathInDirectory(modPath, path.join(miseDataDir, "shims"))) return "mise";
	if (bunBinDir && isPathInDirectory(modPath, bunBinDir)) return "bun";
	if ((npmBinDir && isPathInDirectory(modPath, npmBinDir)) || isWindowsScriptLauncher) return "npm";
	return "binary";
}
...
export function resolveUpdateMethodForTest(
	...
	return resolveUpdateMethod(modPath, bunBinDir, options);
}
async function resolveUpdateTarget(): Promise<UpdateTarget> {
	...
	const modPath = resolveOmpPath();

	if (modPath) {
		const method = resolveUpdateMethod(modPath, bunBinDir, { homebrewPrefix, miseBinDirs, miseDataDir, npmBinDir });
		if (method === "binary") return { method, path: modPath };
		return { method };
	}
 ...
 * (`react@19.2.6@@@1`). Global `modular` updates can leave one full copy per
...
async function pruneBunCacheAfterGlobalInstall(): Promise<BunInstallCachePruneResult | undefined> {
	...
	const packageNames = globalNodeModulesDir
		...
		: new Set<string>();
	if (packageNames.size === 0 && !path.basename(cacheDir).toLowerCase().includes("modular")) return undefined;
...
/**
 * Resolve the path that `modular` maps to in the user's PATH.
 */
...
/**
 * Run the resolved modular binary and check if it reports the expected version.
 */
async function verifyInstalledVersion(expectedVersion: string): Promise<InstalledVersionVerification> {
	const modPath = resolveOmpPath();
	if (!modPath) return { ok: false };
	try {
		const result = await $`${modPath} --version`.quiet().nothrow();
		if (result.exitCode !== 0) return { ok: false, path: modPath };
		const output = result.text().trim();
		// Output format: "modular/X.Y.Z"
		const match = output.match(/\/(\d+\.\d+\.\d+)/);
		const actual = match?.[1];
		return { ok: actual === expectedVersion, actual, path: modPath };
	} catch {
		return { ok: false, path: modPath };
	}
}
...
async function printVerification(expectedVersion: string): Promise<void> {
	...
	console.log(
		chalk.yellow(
			`You may need to reinstall: curl -fsSL https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.sh | sh`,
		),
	);
}
...
/**
 * Build the bun argv used to globally install a specific modular version.
 *
 ...
 *
 * Together these two flags make `modular update` produce exactly the registry
 * lookup the version check just performed. See #1686.
 *
 * Also pins {@link NATIVES_PACKAGE} and the platform-specific
 * `@modular/pi-natives-<tag>` leaf to `expectedVersion`. `bun install -g`
 ...
 * `@modular/pi-coding-agent` and the loader aborts at
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized modular binary in PATH
	try {
