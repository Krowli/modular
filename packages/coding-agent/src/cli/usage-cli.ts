 ...
 *
 * Handles `modular usage` — fetches provider usage reports for every
...
import { formatDuration, formatNumber, sanitizeText } from "@modular/pi-utils";
 ...
 * part (`can.modular@*`) when a couple of mid-string characters suffice.
 ...
 * `modular usage --provider xai` can still confirm the stored credential has no
 * usage endpoint.
 */
		...
		if (cmd.history) {
			...
			if (entries.length === 0) {
				...
				process.stderr.write(
					chalk.yellow(
						`No usage history recorded${scope} yet. Snapshots accumulate whenever usage is fetched (TUI footer, /usage, modular usage).\n`,
					),
				);
				process.exitCode = 1;
				return;
		...
		if (filteredReports.length === 0 && accounts.length === 0) {
			...
			const message =
				storedAccounts.length > 0
					...
					: `No credentials found${scope}. Run \`modular\` and use /login to add accounts.\n`;
