/**
 * CLI handler for `modular worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.modular/wt/`:
 *
...
import { getWorktreesDir, isEnoent } from "@modular/pi-utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.modular/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `modular worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.modular/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
