import type { ResolvedThinkingLevel } from "@modular/pi-agent-core";
import type {
	Api,
	ApiKeyResolver,
	AssistantMessage,
...
import { resolveModelServiceTier, streamSimple } from "@modular/pi-ai";
import { buildModelProviderPriorityRank } from "@modular/pi-catalog/identity";
import { replaceTabs, truncateToWidth } from "@modular/pi-tui";
import { formatDuration, getProjectDir } from "@modular/pi-utils";
...
export async function runBenchCommand(command: BenchCommandArgs, deps: BenchDependencies = {}): Promise<BenchSummary> {
	...
	if (command.models.length === 0) {
		throw new Error("Pass at least one model selector, e.g. `modular bench opus gpt-5.2`");
	}
		...
		for (const { selector, model, thinking } of targets) {
			...
			if (!preflightKey) {
				const failure: BenchRunFailure = {
					ok: false,
					error: `No credentials for provider "${model.provider}". Run \`modular\` and use /login, or set the provider API key.`,
				};
