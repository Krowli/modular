/**
 * `modular models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.modular/models.db`
 ...
 */
import type { Api, Effort, Model } from "@modular/pi-ai";
import { getSupportedEfforts } from "@modular/pi-catalog/model-thinking";
import { formatNumber, getProjectDir } from "@modular/pi-utils";
 ...
 * name doubles as an `modular models <provider>` shortcut.
 */
...
/** `modular models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `modular models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`modular models find` requires a search substring, e.g. `modular models find minimax`\n");
		process.exitCode = 1;
		return;
	}
