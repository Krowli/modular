 ...
 * `@modular/pi-coding-agent` (or one of its aliased scopes like
 * `@modular/pi-coding-agent` or `@modular/pi-coding-agent`).
 *
 ...
 * the same module identity as a direct `@modular/pi-coding-agent` import.
 */

import * as fs from "node:fs/promises";
import * as path from "node:path";
import type { AgentToolResult, AgentToolUpdateCallback } from "@modular/pi-agent-core";
import type { TSchema } from "@modular/pi-ai";
import { Text } from "@modular/pi-tui";
import { getAgentDir, getProjectDir, parseFrontmatter as parseOmpFrontmatter } from "@modular/pi-utils";
...
import {
	...
	discoverSkills,
	createAgentSession as modCreateAgentSession,
} from "../sdk";
import {
	DEFAULT_MAX_BYTES,
	DEFAULT_MAX_LINES,
...
const LEGACY_BUILTIN_TOOL_MARKER = "__modLegacyBuiltinTool";
 ...
 *
 * Upstream `@modular/pi-coding-agent` centralizes extension / skill /
 ...
 *
 * MODULAR does the same discovery inline inside `createAgentSession()`, so this
 ...
 * override below translates them into MODULAR's native session options
 ...
 * extensions in the wild — themes are silently dropped (MODULAR has no
...
export interface ResourceLoader {
	...
	readonly __modLegacyPiLoader?: true;
}
...
export class DefaultResourceLoader implements ResourceLoader {
	readonly __modLegacyPiLoader = true as const;
...
/**
 * Legacy pi extensions call `createAgentSession({ resourceLoader })`. MODULAR's
 ...
 * `noExtensions`/`noSkills` opt-outs), re-run MODULAR's own discovery, and
 ...
 *
 * Translate the loader's captured state into MODULAR's option fields, then
...
export async function createAgentSession(
	...
	if (!loader) {
		return modCreateAgentSession(options);
	}
	...
	return modCreateAgentSession(forwarded);
}
