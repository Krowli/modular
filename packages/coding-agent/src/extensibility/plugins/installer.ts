...
import { getAgentDir, getProjectDir, isEnoent } from "@modular/pi-utils";
...
export async function installPlugin(packageName: string): Promise<InstalledPlugin> {
	...
	if (!(await pkgJson.exists())) {
		await pkgJson.write(JSON.stringify({ name: "modular-plugins", private: true, dependencies: {} }, null, 2));
	}
	...
	return {
		...
		manifest: pkg.modular || pkg.pi || { version: pkg.version },
