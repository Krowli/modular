...
import { getPluginsDir, getPluginsLockfile, isEnoent } from "@modular/pi-utils";
 ...
 *
 * `home` controls which `<plugins>/modular-plugins.lock.json` is read — pass it
...
/**
 * Load project-local plugin overrides (checks .modular and .pi directories).
 */
 ...
 * `<root>/package.json#dependencies`, and `<root>/modular-plugins.lock.json#plugins`.
...
async function collectPluginsAtRoot(
	...
	const lockPath = path.join(root, "modular-plugins.lock.json");
	...
	for (const name of names) {
		...
		let pluginPkg: { version: string; modular?: PluginManifest; pi?: PluginManifest };
		try {
		...
		const manifest: PluginManifest | undefined = pluginPkg.modular || pluginPkg.pi;
		if (!manifest) {
			// Not an modular plugin, skip
			continue;
		}
 ...
 * (`getPluginsDir(home)`) and, when a project anchor (`.modular/` or `.git/`)
 * exists at or above `cwd`, the project root
 * (`<projectAnchor>/.modular/plugins`). Each root contributes the union of its
 * `package.json#dependencies` and `modular-plugins.lock.json#plugins`. Project
...
interface DeclaredManifestEntries {
	/** True when the directory's package.json declares a non-empty `modular`/`pi` `extensions` array. */
	declared: boolean;
...
/**
 * Read the extension entries declared by `dir`'s own package.json `modular`/`pi`
...
function readDeclaredManifestEntries(dir: string): DeclaredManifestEntries {
	...
	let pkg: { modular?: { extensions?: unknown }; pi?: { extensions?: unknown } };
	try {
		pkg = JSON.parse(raw) as { modular?: { extensions?: unknown }; pi?: { extensions?: unknown } };
	} catch {
		return { declared: false, files: [] };
	}
	const declared = (pkg.modular ?? pkg.pi)?.extensions;
 ...
 *   1. the directory's own package.json `modular`/`pi` `extensions` entries —
 ...
 *     {@link resolveDirectoryEntries} — its own package.json `modular`/`pi`
 ...
 *     and MODULAR's configured-directory (`-e`) extension loader
 *   - otherwise (tools/hooks/commands) only a direct index.{ts,js,mjs,cjs}.
 *     The sub-extension scan and the `modular`/`pi` `extensions` manifest are
