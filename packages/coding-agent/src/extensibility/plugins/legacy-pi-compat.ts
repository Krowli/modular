...
import { isCompiledBinary, stripWindowsExtendedLengthPathPrefix } from "@modular/pi-utils";
...
// and serve requested surfaces through `modular-legacy-pi-bundled:<key>` synthetic
...
const BUNDLED_VIRTUAL_SCHEME = "modular-legacy-pi-bundled:";
const BUNDLED_VIRTUAL_NAMESPACE = "modular-legacy-pi-bundled";
const BUNDLED_MODULES_GLOBAL = "__modLegacyPiBundledModules";
 ...
 *
 * `globalThis` is the bridge: each `modular-legacy-pi-bundled:<key>` source string
...
function ensureBundledModulesLoaded(): Promise<BundledModules> {
	if (!IS_COMPILED_BINARY) {
		return Promise.reject(new Error("modular:legacy-pi-shim: bundled modules are only available in compiled mode"));
	}
	if (!bundledModulesPromise) {
		bundledModulesPromise = import("modular-legacy-pi-modules").then(module => {
...
function synthesizeBundledModuleSourceFromModules(moduleKey: string, modules: BundledModules): string {
	...
	if (!mod) {
		throw new Error(`modular:legacy-pi-shim: no bundled module registered for ${moduleKey}`);
	}
	const lines: string[] = [
		`const __modular_bundled = globalThis[${JSON.stringify(BUNDLED_MODULES_GLOBAL)}][${JSON.stringify(moduleKey)}];`,
	];
	...
	for (const exportName in mod) {
		...
		lines.push(`export const ${exportName} = __modular_bundled[${JSON.stringify(exportName)}];`);
	}
	if (hasDefault) {
		lines.push("export default __modular_bundled.default;");
	}
	lines.push("");
	return lines.join("\n");
}
 ...
 * `modular-legacy-pi-bundled:<key>` import.
 */
...
// the aliased scopes below (modular's original publish, modular'
// fork, or the canonical @modular scope itself) are remapped to this scope and
// resolved against the bundled copy that ships inside the modular binary. This
...
const CANONICAL_PI_SCOPE = "@modular";

// Scopes that have historically been used to publish (or alias) the same set
// of internal pi-* packages. `@modular` is intentionally included so direct
...
const PI_SCOPE_ALIASES = ["modular"] as const;

// Internal pi-* package basenames bundled inside the modular binary.
...
// Upstream `@modular/*` packages exposed a few subpaths at the package
...
// is served through the `modular-legacy-pi-bundled:` virtual namespace (see the
 ...
 * bare `@modular/pi-coding-agent` here: from a global install Bun can pick an
 ...
 * `modular-legacy-pi-bundled:` virtual namespace (issue #3423) — bunfs paths are
...
// imports such as `@modular/pi-ai/oauth` continue to resolve directly
...
// miss installs where the bundled packages live at `node_modules/@modular/pi-*`.
//
// Compiled-binary entries are `modular-legacy-pi-bundled:<key>` specifiers handed
 ...
 * through to the canonical-resolution path. Virtual `modular-legacy-pi-bundled:`
...
/**
 * Resolve a canonical `@modular/*` specifier to a filesystem path, preferring
...
function toImportSpecifier(resolvedPath: string): string {
	// Virtual `modular-legacy-pi-bundled:` specifiers are served by the synthetic
...
/**
 * Rewrite the extension-owned specifiers MODULAR must host-resolve — legacy
 ...
 * realpath of every reachable source module MODULAR must rewrite at load time.
...
function installExtensionGraphHook(
	...
	if (asyncModules.size > 0) {
		...
		Bun.plugin({
			name: `modular:legacy-pi-ext:${hookId}`,
			setup(build) {
...
function resolveLegacyPiSpecifier(args: { path: string; importer: string }): { path: string } | undefined {
	...
	// Primary: resolve the canonical @modular/* specifier from the host binary
	...
	try {
		...
		// plugin installed @modular peer deps, then try the original legacy
		// specifier for plugins that still vendor only @modular or
		// @modular peer deps.
...
export function installLegacyPiSpecifierShim(): void {
	...
	Bun.plugin({
		...
		setup(build) {
			...
			// Compiled mode serves `modular-legacy-pi-bundled:<key>` imports from
