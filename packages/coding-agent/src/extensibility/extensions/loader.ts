...
import type { ThinkingLevel } from "@modular/pi-agent-core";
import type { ImageContent, Model, TextContent, TSchema } from "@modular/pi-ai";
import type { KeyId } from "@modular/pi-tui";
import { hasFsCode, isEacces, isEnoent, logger } from "@modular/pi-utils";
...
async function readExtensionManifest(packageJsonPath: string): Promise<ExtensionManifest | null> {
	try {
		const pkg = (await Bun.file(packageJsonPath).json()) as { modular?: ExtensionManifest; pi?: ExtensionManifest };
 ...
 * `.modular`/`.pi` extension capabilities, JS/TS hook factories, the
...
export async function discoverExtensionPaths(
	...
	// 1. Discover extension modules via capability API (native .modular/.pi only).
