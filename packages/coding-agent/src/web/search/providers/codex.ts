...
import { type AuthStorage, type FetchImpl, type Model, type OAuthAccess, withOAuthAccess } from "@modular/pi-ai";
import { decodeJwt } from "@modular/pi-ai/oauth/openai-codex";
import { applyCodexResponsesLiteShape } from "@modular/pi-ai/providers/openai-codex/request-transformer";
import { createOpenAICodexCompatibilityMetadata } from "@modular/pi-ai/providers/openai-codex-responses";
import { getBundledModels } from "@modular/pi-catalog/models";
import { CODEX_CLIENT_VERSION, OPENAI_HEADER_VALUES, OPENAI_HEADERS } from "@modular/pi-catalog/wire/codex";
import { $env, readSseJson } from "@modular/pi-utils";
...
export async function searchCodex(params: SearchParams): Promise<SearchResponse> {
	...
	if (!seed) {
		throw new Error(
			"No Codex OAuth credentials found. Login with 'modular /login openai-codex' to enable Codex web search.",
		);
	}
