...
import { isEexist, isEnoent, postmortem } from "@modular/pi-utils";
...
/** Close every project broker connection held by this modular process. */
...
export async function smokeTestDaemonBroker(): Promise<void> {
	const projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-daemon-smoke-project-"));
	const runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), "modular-daemon-smoke-run-"));
