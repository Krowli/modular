...
import { Process, type PtyRunResult, PtySession } from "@modular/pi-natives";
import { isEexist, isEnoent, logger, postmortem, sanitizeText } from "@modular/pi-utils";
...
export async function startDaemonBrokerFromEnvironment(): Promise<void> {
	...
	if (!lease) return;
	process.title = "modular daemon broker";
