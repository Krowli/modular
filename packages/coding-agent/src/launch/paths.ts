...
import { getConfigRootDir } from "@modular/pi-utils";

/** Resolve the private runtime directory shared by modular processes in one project directory. */
...
export function daemonBrokerEndpoint(projectDir: string, runtimeDir: string): string {
	if (process.platform === "win32") {
		const key = Bun.hash.wyhash(path.resolve(projectDir)).toString(16).padStart(16, "0");
		return `\\\\.\\pipe\\modular-daemon-${key}`;
	}
