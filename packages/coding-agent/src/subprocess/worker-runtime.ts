...
export async function installSharpStubResolver(runtimeDir: string): Promise<string> {
	...
	const sharpStub = path.join(runtimeDir, "modular-sharp-stub.cjs");
...
export interface TransformersRuntimeMetadata {
	__modRuntimeNodeModules?: string;
	__modTransformersEntry?: string;
	__modCudaRepairError?: string;
}
...
function attachTransformersRuntimeMetadata<T extends ConfigurableTransformers>(
	...
	runtime.__modRuntimeNodeModules = metadata.__modRuntimeNodeModules;
	runtime.__modTransformersEntry = metadata.__modTransformersEntry;
	runtime.__modCudaRepairError = metadata.__modCudaRepairError;
	return runtime;
}
...
function cudaFailureCause(
	...
	if (metadata.__modCudaRepairError) {
		return `ONNX Runtime CUDA provider install failed: ${metadata.__modCudaRepairError}`;
	}
...
function resolveOnnxRuntimePackageDir(metadata: TransformersRuntimeMetadata): string | null {
	const entry = metadata.__modTransformersEntry;
	if (entry) {
		try {
	...
	const nodeModules = metadata.__modRuntimeNodeModules;
...
export async function formatOnnxRuntimeCudaDiagnostics(
	...
	const sideRuntime = metadata.__modRuntimeNodeModules;
	const lines = [
		"ONNX Runtime CUDA diagnostics:",
		`  PI_TINY_DEVICE=${requestedDevice} requested CUDAExecutionProvider`,
...
export function loadTransformersRuntime<T extends ConfigurableTransformers, K>(
	...
	return holder.load(async () => {
		...
		return attachTransformersRuntimeMetadata(configureTransformers(require_(entry) as T), {
			__modRuntimeNodeModules: path.join(installedDir, "node_modules"),
			__modTransformersEntry: entry,
			__modCudaRepairError: cudaRepairError,
		});
	});
}
