...
# MODULAR Coding Agent Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.sh | sh
#
# Options:
...
install_via_bun() {
    ...
    if [ -n "$REF" ]; then
        ...
        if ! has_git; then
            echo "git is required to install modular from source"
            exit 1
        fi
        TMP_DIR="$(mktemp -d)"
        trap 'rm -rf "$TMP_DIR"' EXIT
