...
describe("Windows release binary target", () => {
	it("builds the generic Windows release asset with the default x64 runtime", async () => {
		...
		expect(output).toContain("Building packages/coding-agent/binaries/modular-windows-x64.exe...");
		expect(output).toContain(
			"DRY RUN Bun.build target=bun-windows-x64 outfile=packages/coding-agent/binaries/modular-windows-x64.exe",
		);
		expect(output).toContain("external=fastembed,onnxruntime-node");
		expect(output).not.toContain("bun-windows-x64-baseline");
	});

	it("uses the default x64 runtime for local Windows cross-build aliases", () => {
