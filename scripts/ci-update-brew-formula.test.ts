...
const SUMS = {
	"modular-darwin-arm64": "darwin_arm64_sha",
	"modular-darwin-x64": "darwin_x64_sha",
	"modular-linux-arm64": "linux_arm64_sha",
	"modular-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["modular-*"].first` returns `nil`, and
	// `bin.install nil => "modular"` raises (issue #2398).
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		const matches = formula.match(/using: :nounzip/g) ?? [];
		expect(matches).toHaveLength(4);
		for (const arch of ["modular-darwin-arm64", "modular-darwin-x64", "modular-linux-arm64", "modular-linux-x64"]) {
	...
	// popened binary doesn't touch the real `~/.modular` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"modular", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
