#!/usr/bin/env bun
//
// Render the Homebrew formula for `modular` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (modular-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/modular.rb>
//   bun scripts/ci-update-brew-formula.ts v15.10.3        # prints to stdout

import { $ } from "bun";

const REPO = process.env.MODULAR_REPO ?? "Krowli/modular";
const HOMEPAGE = "https://modularcli.com";
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["modular-*"].first` then
	// returns `nil` and `bin.install nil => "modular"` raises.
	//
	...
	// not touch the real `/Users/<user>/.modular` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class Modular < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  license "MIT"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/modular-darwin-arm64",
          using: :nounzip
      sha256 "${sums["modular-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/modular-darwin-x64",
          using: :nounzip
      sha256 "${sums["modular-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/modular-linux-arm64",
          using: :nounzip
      sha256 "${sums["modular-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/modular-linux-x64",
          using: :nounzip
      sha256 "${sums["modular-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["modular-*"].first => "modular"
    (bin/"modular").chmod 0555
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"modular", "completions", shells: [:bash, :zsh, :fish])
    end
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/modular --version")
  end
end
`;
}
...
async function main(): Promise<void> {
	...
	const targets = ["modular-darwin-arm64", "modular-darwin-x64", "modular-linux-arm64", "modular-linux-x64"];
