...
(`~/.modular/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.modular/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.modular/agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@modular/pi-ai`,
credentials resolved through modular's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.modular/stats-audit-cache.json` (keyed by session id +
