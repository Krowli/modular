#!/usr/bin/env bun
/**
 * Token-usage audit over the local modular session corpus (~/.modular/agent/sessions/).
 *
 ...
 * asks a small model (default: anthropic/claude-sonnet-4-6 via @modular/pi-ai)
 * to judge:
 ...
 *
 * Auth: resolves an API key for the classifier provider through modular's auth
 * storage (~/.modular/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
import { getAgentDbPath, isEnoent } from "@modular/pi-utils";
import SYSTEM_PROMPT from "./audit-prompt.md" with { type: "text" };

const SESSIONS_ROOT = path.join(os.homedir(), ".modular", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".modular", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.modular/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.modular/stats-audit-cache.json)\n` +
...
async function openClassifier(modelSpec: string): Promise<Classifier> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (modular login or env var required)`);
	}
	return { model, apiKey };
}
