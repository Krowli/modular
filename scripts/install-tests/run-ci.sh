...
smoke_cli() {
   ...
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$modular_bin" --version
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$modular_bin" --help >/dev/null
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$modular_bin" stats --summary >/dev/null
   ...
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$modular_bin" --smoke-test
}
...
mkdir -p "$BINARY_DIR"
cp packages/coding-agent/dist/modular "$BINARY_DIR/modular"
smoke_cli "$BINARY_DIR/modular"
...
(
   ...
   smoke_cli "$BUN_INSTALL/bin/modular"
)
...
#    `bin.modular` from `src/cli.ts` to the prepack bundle `dist/cli.js`. The repo
...
utils_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-utils-*.tgz)"
wire_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-wire-*.tgz)"
natives_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-natives-[0-9]*.tgz)"
natives_leaf_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-natives-"$host_tag"-*.tgz)"
hashline_tgz="$(find_tarball "$TARBALL_DIR"/modular-hashline-*.tgz)"
catalog_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-catalog-*.tgz)"
ai_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-ai-*.tgz)"
mnemopi_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-mnemopi-*.tgz)"
snapcompact_tgz="$(find_tarball "$TARBALL_DIR"/modular-snapcompact-*.tgz)"
agent_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-agent-core-*.tgz)"
tui_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-tui-*.tgz)"
stats_tgz="$(find_tarball "$TARBALL_DIR"/modular-modular-stats-*.tgz)"
coding_agent_tgz="$(find_tarball "$TARBALL_DIR"/modular-pi-coding-agent-*.tgz)"
collab_web_tgz="$(find_tarball "$TARBALL_DIR"/modular-collab-web-*.tgz)"
...
(
   ...
   node -e "
		...
		pkg.overrides = {
			'@modular/pi-utils': '$utils_tgz',
			'@modular/pi-wire': '$wire_tgz',
			'@modular/pi-natives': '$natives_tgz',
			'@modular/pi-natives-$host_tag': '$natives_leaf_tgz',
			'@modular/hashline': '$hashline_tgz',
			'@modular/pi-ai': '$ai_tgz',
			'@modular/pi-catalog': '$catalog_tgz',
			'@modular/pi-mnemopi': '$mnemopi_tgz',
			'@modular/snapcompact': '$snapcompact_tgz',
			'@modular/pi-agent-core': '$agent_tgz',
			'@modular/pi-tui': '$tui_tgz',
			'@modular/modular-stats': '$stats_tgz',
			'@modular/pi-coding-agent': '$coding_agent_tgz',
			'@modular/collab-web': '$collab_web_tgz'
		};
   ...
   leaf_dir="node_modules/@modular/pi-natives-$host_tag"
   [ -d "$leaf_dir" ] || {
      echo "Platform leaf package not installed: $leaf_dir"
      exit 1
   }
   wire_proto="$(bun -e 'import { COLLAB_PROTO } from "@modular/pi-wire"; process.stdout.write(String(COLLAB_PROTO));')"
   [ "$wire_proto" = "3" ] || {
      echo "Unexpected @modular/pi-wire COLLAB_PROTO: $wire_proto"
      exit 1
   }
   [ -f "node_modules/@modular/collab-web/dist/index.html" ] || {
      echo "Collab web tarball did not install built dist/index.html"
      exit 1
   }
   smoke_cli ./node_modules/.bin/modular
)
