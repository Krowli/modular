...
from modular_rpc import (  # noqa: E402
    AgentEndEvent,
    AutoRetryEndEvent,
    AutoRetryStartEvent,
    ExtensionUiRequest,
...
def resolve_mod_bin(raw: str | None) -> str:
    ...
    if not found:
        raise SystemExit("Could not find `modular` on PATH. Set --modular-bin or MODULAR_BIN.")
    return found
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run OpenRouter fixture evaluations through modular RPC mode."
    )
    parser.add_argument("--modular-bin", default=os.environ.get("MODULAR_BIN"))
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    results_dir = (
        ...
        if args.results_dir
        else tmp_root / f"modular-fixture-runs-{timestamp}"
    )
