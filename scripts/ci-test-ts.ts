...
// runners (so it flakes/times out there); robodular-web lives under python/robodular
// and is outside every CI TS bucket.
const localOnlyWorkspacePackages = ["packages/mnemopi", "python/robodular/web"];
...
// full run also exercises the release-notes and link-modular tests. (A
...
async function commandsForMode(mode: Mode): Promise<TestCommand[]> {
	switch (mode) {
		...
		// `all` set PLUS mnemopi and robodular-web, which CI omits) and every repo
...
// The modular-kata runner pods inject sccache S3 credentials (`AWS_*`) and config
...
// Override with MODULAR_TEST_CHUNK_TIMEOUT (seconds).
function chunkTimeoutMs(): number {
	const raw = Number(Bun.env.MODULAR_TEST_CHUNK_TIMEOUT?.trim());
...
// Defaults to the machine's available parallelism; `MODULAR_TEST_CONCURRENCY`
...
function testConcurrency(total: number): number {
	const raw = Bun.env.MODULAR_TEST_CONCURRENCY?.trim().toLowerCase();
	if (raw === "all" || raw === "max") {
		return total;
	}
...
export async function runTestCommandsInParallel(commands: TestCommand[], concurrency: number): Promise<void> {
	...
	console.log(
		`Running ${commands.length} test command(s), up to ${concurrency} in parallel ` +
			`(MODULAR_TEST_CONCURRENCY=<n>|all to change).`,
	);
	...
	async function worker(): Promise<void> {
		for (;;) {
			...
			const outcome: ChunkOutcome = {
				...
				output: `${stdout.text}${stderr.text}${timedOut ? `\n[watchdog] chunk exceeded ${Math.round(chunkTimeoutMs() / 1000)}s; killed with SIGKILL (MODULAR_TEST_CHUNK_TIMEOUT to change)\n` : ""}`,
			};
			if (quiet) {
