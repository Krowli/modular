...
sys.path.insert(0, str(REPO_ROOT / "python/modular-rpc/src"))

from modular_rpc import MessageEndEvent, MessageStartEvent, MessageUpdateEvent, RpcClient, ToolExecutionStartEvent  # noqa: E402
...
def resolve_repo_mod_bin() -> str | None:
...
def resolve_mod_bin(raw: str | None) -> str:
    if raw:
        return raw
    repo_bin = resolve_repo_mod_bin()
    if repo_bin:
        return repo_bin
    found = shutil.which("modular")
    if not found:
        raise SystemExit("Could not find `modular` on PATH and could not resolve the repo CLI. Set --modular-bin or MODULAR_BIN.")
    return found
...
async def run_all(spec: BenchmarkSpec, args: argparse.Namespace) -> dict[str, dict[str, Any]]:
    modular_bin = resolve_mod_bin(args.modular_bin)
...
def parse_args(description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--modular-bin",
        default=os.environ.get("MODULAR_BIN"),
        help="Executable to launch. Defaults to the repo checkout CLI, then falls back to `modular` on PATH.",
    )
    parser.add_argument(
