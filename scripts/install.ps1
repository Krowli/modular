# MODULAR Coding Agent Installer for Windows
# Usage: irm https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.ps1))) -Source -Ref v3.20.1
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.ps1))) -Source -Ref main
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.ps1))) -Binary -Ref v3.20.1
...
$Repo = "Krowli/modular"
$Package = "@modular/pi-coding-agent"
$InstallDir = if ($env:PI_INSTALL_DIR) { $env:PI_INSTALL_DIR } else { "$env:LOCALAPPDATA\modular" }
$BinaryName = "modular-windows-x64.exe"
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".modular\agent"
        ...
        if ($bashPath) {
            ...
            Write-Host "  MODULAR requires a bash shell on Windows. Options:" -ForegroundColor Yellow
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("modular-install-" + [System.Guid]::NewGuid().ToString("N"))
        ...
        if (-not (Test-GitInstalled)) {
            throw "git is required to install modular from source"
        }
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("modular-" + [System.Guid]::NewGuid().ToString("N"))
        try {
            git clone --depth 1 "https://github.com/$Repo.git" $tmpRoot | Out-Null
            if ($LASTEXITCODE -ne 0) { throw "Failed to clone https://github.com/$Repo.git" }
            if (Test-GitLfsInstalled) {
                Push-Location $tmpRoot
                try { git lfs pull | Out-Null } finally { Pop-Location }
            }
            bun install -g (Join-Path $tmpRoot "packages\coding-agent")
...
if ($Source) {
    ...
    # Default: prebuilt binary from GitHub Releases (no bun/npm needed)
    Install-Binary
}
