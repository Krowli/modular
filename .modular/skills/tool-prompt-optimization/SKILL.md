...
`scripts/probe.ts` routes through `@modular/pi-ai` (`completeSimple`) so model/auth/provider behavior matches production.

```bash
bun .modular/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .modular/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@modular/pi-ai"; // or toolWireSchema(tool)
