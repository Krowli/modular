...
| `packages/stats`        | Local observability dashboard (`modular stats`)          |
| `packages/utils`        | Shared utilities (logger, streams, temp files)       |
| `crates/pi-natives`     | Rust crate for performance-critical text/grep ops    |

**Catalog import convention**: code in this repo imports catalog *values* (bundled models, model-thinking helpers, identity, descriptors, model manager/cache) from `@modular/pi-catalog/<module>` — never via `@modular/pi-ai`. The pi-ai barrel re-exports only the model/effort *types* its own signatures use (`Model`, `Api`, `ThinkingConfig`, `Effort`, …); type-only imports of those from `@modular/pi-ai` are fine.
...
- **Worker scripts**: workers re-enter the CLI entrypoint; never spawn separate worker entry modules. `cli.ts` declares itself as the worker host at startup (`declareWorkerHostEntry()` from `@modular/pi-utils/env`) and dispatches hidden argv selectors (`__modular_worker_stats_sync`, `__modular_worker_tab`, `__modular_worker_js_eval`, `__modular_worker_tiny_inference`) before loading the command registry. Spawn sites use:
  ```ts
  import { workerHostEntry } from "@modular/pi-utils";
  const hostEntry = workerHostEntry();
  const worker = hostEntry
  	? new Worker(hostEntry, { type: "module", argv: ["__modular_worker_<name>"] })
  	: new Worker(new URL("./<worker>.ts", import.meta.url).href, { type: "module" });
  ```
  When the process was started from the modular CLI — source `cli.ts`, npm-bundle `dist/cli.js`, or compiled binary — `workerHostEntry()` is `Bun.main` and the worker re-enters the single entry module, so no per-worker `--compile` entrypoints or bundle entries exist. Outside a CLI host (`bun test`, SDK embedding, standalone `modular-stats`) it returns `null` and the direct-module fallback loads the worker source. New worker kinds MUST add their selector to the dispatch table in `cli.ts` and keep the fallback branch.
  ...
  Validate any new worker with the dedicated smoke probe: `modular --smoke-test` spawns the stats sync worker and the tiny-model subprocess, pings them, and exits — it's wired into `ci:test:smoke` and `scripts/install-tests/run-ci.sh` so binary, source-link, and tarball installs all exercise it. Add a sibling smoke if the new worker is on a different module graph.
...
| Binary lookup   | `$which("git")` from `@modular/pi-utils` | `spawnSync(["which", "git"])`   |
...
- `if (await file.exists()) { await file.json() }` → two syscalls plus race. Use try-catch with `isEnoent`:
  ```typescript
  import { isEnoent } from "@modular/pi-utils";
  try {
...
```typescript
import { logger } from "@modular/pi-utils";
...
Logs go to `~/.modular/logs/modular.YYYY-MM-DD.log` with automatic rotation.
...
**Rules:**
- **Tabs → spaces** via `replaceTabs()` (from `@modular/pi-tui` or `../tools/render-utils`).
...
**Attribution:**
- Internal (from issues): `Fixed foo bar ([#123](https://github.com/Krowli/modular/issues/123))`.
- External contributions: `Added feature X ([#456](https://github.com/Krowli/modular/pull/456) by [@username](https://github.com/username))`.
