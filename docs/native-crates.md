...
`@modular/pi-natives` and the embedded shell/PTY runtime. They are intentionally
internal: end users see `@modular/pi-natives` exports, not these crate APIs.
...
- Native API surface and loader (`@modular/pi-natives`):
...
- Porting cross-references:
  [`porting-from-modular.md`](./porting-from-modular.md),
...
package (`@modular/pi-natives`) and the architecture pages above. Promote a
