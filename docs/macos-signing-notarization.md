...
The compiled macOS `modular` binaries shipped on GitHub Releases are signed with a
...
submission (see [#776](https://github.com/Krowli/modular/issues/776)).
...
- `curl https://raw.githubusercontent.com/Krowli/modular/main/scripts/install.sh | sh` — `curl` sets no quarantine bit, so
...
Drop these into a working directory (default `~/modular-signing`):
...
```sh
scripts/ci-macos-upload-secrets.sh ~/modular-signing --dry-run   # validate first
scripts/ci-macos-upload-secrets.sh ~/modular-signing             # upload all five
gh secret list --repo Krowli/modular                       # confirm
```
...
APPLE_API_KEY_ID=… APPLE_API_ISSUER_ID=… APPLE_API_KEY=… \
  bash scripts/ci-macos-sign.sh packages/coding-agent/binaries/modular-darwin-arm64
```
