...
description: Use when creating a new modular marketplace. Covers marketplace.json schema, source types, install commands, and publishing.
---

# Authoring Marketplaces

A marketplace is a Git repository (or local directory) that contains a catalog file at either `.modular-plugin/marketplace.json` (preferred for modular-specific catalogs) or `.claude-plugin/marketplace.json` (Claude Code-compatible; used as the fallback). Anyone can author one. Users add it with `/marketplace add owner/repo` and then install individual plugins from it.
...
The catalog file lives at either `.modular-plugin/marketplace.json` or `.claude-plugin/marketplace.json` in the repository root. modular prefers the `.modular-plugin/` path and falls back to the Claude path; a repository may publish both to expose tool-specific catalogs from a single source tree.
...
```json
{
  ...
  "metadata": {
    "description": "Official Acme plugins for modular"
  },
  "plugins": [
    {
      "name": "acme-linter",
    ...
    },
    {
      ...
      "source": {
        "source": "github",
        "repo": "acme-corp/modular-deploy-plugin",
        "ref": "main"
      }
    }
  ]
...
> Note: extension modules declared via `package.json` `modular.extensions` are **not** loaded from marketplace installs — that mechanism only applies to npm-installed or `modular plugin link`ed plugins. Ship marketplace plugin behavior through the conventional directories above.
...
```
modular plugin marketplace add owner/repo
modular plugin install name@marketplace-name
```

Scope behavior:

- **user** (default) — installed in `~/.modular/plugins/installed_plugins.json`, available in all projects
- **project** — installed in `<project>/.modular/plugins/installed_plugins.json`, available only in that project
...
1. Create `marketplace.json` at `.modular-plugin/marketplace.json` (modular-only) or `.claude-plugin/marketplace.json` (shared with Claude Code) in a new Git repo.
