...
description: Use when creating a new modular hook. Covers HookAPI, event catalog, blocking/overriding tool calls, and context modification.
---
...
```ts
import type { HookAPI } from "@modular/pi-coding-agent/extensibility/hooks";

export default function myHook(modular: HookAPI): void {
  modular.on("tool_call", async (event, ctx) => {
...
```ts
modular.on("tool_result", async (event, ctx) => {
...
```ts
modular.on("context", async (event, ctx) => {
...
export default function rmRfBlocker(modular: HookAPI): void {
  modular.on("tool_call", async (event, ctx) => {
...
export default function apiKeyRedactor(modular: HookAPI): void {
  modular.on("tool_result", async (event) => {
...
export default function contextFilter(modular: HookAPI): void {
  modular.on("context", async (event) => {
