...
This runbook describes how `@modular/pi-natives` produces `.node` addons, generated declarations, and compiled-binary embedded payloads, and how to debug loader/build failures.
...
- `bun scripts/gen-npm-packages.ts` (`gen:npm`) → generate per-platform npm leaf packages (`@modular/pi-natives-<platform>-<arch>`, installed as optional dependencies of the core package) under `npm/` from built addon files.
...
2. Loader resolves platform npm leaf-package candidates (`@modular/pi-natives-<platform>-<arch>`, when resolvable), then package-local `native/` and executable-dir fallback candidates.
...
3. Runtime candidate order includes:
   ...
   - versioned cache dir,
   - legacy compiled-binary dir (`%LOCALAPPDATA%/modular` on Windows, `~/.local/bin` elsewhere),
...
- Root: `/data/cache/pi-natives` (provisioned by `entrypoint.sh` alongside the cargo caches, owned `root:modular`, mode `02770` setgid so cached files inherit `gid=modular` and stay readable by every slot user).
...
### Configuration (settings on `robodular.config.Settings`)

| Env var                                     | Default                  | Effect                                                                                              |
| ------------------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------- |
| `ROBODULAR_NATIVES_CACHE_ENABLED`              | `true`                   | Master switch. When false the populate/capture hooks no-op and every workspace builds from scratch. |
| `ROBODULAR_NATIVES_CACHE_ROOT`                 | `/data/cache/pi-natives` | Cache root directory. Must be `root:modular 02770` for cross-slot reads.                                |
| `ROBODULAR_NATIVES_CACHE_MAX_ENTRIES_PER_REPO` | `8`                      | LRU entry-count cap, per repo slug.                                                                 |
| `ROBODULAR_NATIVES_CACHE_MAX_BYTES`            | `4294967296` (4 GiB)     | LRU byte cap, per repo slug.                                                                        |
| `ROBODULAR_NATIVES_CACHE_GC_INTERVAL_SECONDS`  | `3600`                   | Period of the background GC loop in `WorkerPool`.                                                   |
