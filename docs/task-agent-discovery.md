...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from MODULAR-native roots and Claude plugin roots before appending bundled definitions. Cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped — their frontmatter schema is not the MODULAR task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".modular"` filters both dir lists).

### Discovery inputs

1. Nearest project `.modular` agents dir from `findAllNearestProjectConfigDirs("agents", cwd)` (filtered to `.modular`; first hit only)
2. User `.modular` agents dir from `getConfigDirs("agents", { project: false })` (filtered to `.modular`; first hit only)
...
1. project `.modular/agents`
2. user `~/.modular/agent/agents`
...
- Project `.modular` overrides user `.modular`.
