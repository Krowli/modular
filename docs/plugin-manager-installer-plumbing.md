...
This document describes how `modular plugin` npm/git/link operations mutate plugin state on disk and how installed npm/git/link plugins become runtime capabilities (tools and extensions today, hooks/commands path resolution available). Marketplace installs use separate marketplace registries and cache plumbing; see `docs/marketplace.md`.
...
```text
modular plugin <npm/link action> ...
  -> src/commands/plugin.ts
  -> runPluginCommand(...) in src/cli/plugin-cli.ts
  -> PluginManager method (install/list/uninstall/link/...)
  -> mutate ~/.modular/plugins/{package.json,node_modules,modular-plugins.lock.json}
...
modular plugin install name@marketplace / modular install name@marketplace
  -> MarketplaceManager
  -> mutate ~/.modular/marketplaces.json, ~/.modular/plugins/installed_plugins.json, cache dirs
...
Global plugin state lives under `~/.modular/plugins`:
...
- `modular-plugins.lock.json` — runtime state for npm/link plugins:
...
- `<cwd>/.modular/plugin-overrides.json`
...
- `~/.modular/marketplaces.json` — configured marketplace catalogs
- `~/.modular/plugins/installed_plugins.json` — user-scoped marketplace installs
- `<cwd>/.modular/plugins/installed_plugins.json` — project-scoped marketplace installs when available
- `~/.modular/plugins/cache/{marketplaces,plugins}/` — cached catalogs and plugin directories
...
- A package missing `modular`/`pi` is still installable and listable.
- Runtime plugin loading (`getEnabledPlugins`) skips packages without `modular`/`pi` manifest.
...
3. Ensure plugin `package.json` exists (`modular-plugins`, private dependencies map).
4. Run `bun install <packageSpec>` in `~/.modular/plugins`.
...
- `modular plugin install pkg@newVersion` updates dependency and lockfile version.
...
1. Read plugin dependency map from `~/.modular/plugins/package.json`.
2. Load lockfile runtime config (missing file -> empty defaults).
3. Load project overrides (`<cwd>/.modular/plugin-overrides.json`, parse/read errors -> empty object with warning).
...
`link` supports local plugin development by symlinking a local package into `~/.modular/plugins/node_modules/<pkg.name>`.
...
- skip if manifest (`modular`/`pi`) absent
