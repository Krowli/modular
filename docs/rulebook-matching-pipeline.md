...
- [`packages/coding-agent/src/discovery/modular-plugins.ts`](../packages/coding-agent/src/discovery/modular-plugins.ts)
...
- `native` (priority `100`)
- `modular-plugins` (priority `90`) — `rules/*.{md,mdc}` inside configured extension package roots, normalized via the shared `buildRuleFromMarkdown` path
- `agents` (priority `70`)
- `cursor` (priority `50`)
...
Loads `.modular` rules from:

- project: `<cwd>/.modular/rules/*.{md,mdc}` when the cwd `.modular` directory exists
- user: `~/.modular/agent/rules/*.{md,mdc}`
- sticky user rule: `~/.modular/agent/RULES.md`
- sticky project rule: nearest ancestor `.modular/RULES.md` while walking from cwd toward the repository root
...
- `native` appends project `.modular/rules`, user `~/.modular/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
- `modular-plugins` appends `rules/` results per configured extension package root.
...
1. The rule providers currently loaded for `rules` are `native`, `modular-plugins`, `agents`, `cursor`, `windsurf`, `cline`, and embedded `builtin-defaults`; provider files for other tools may parse other config formats but do not register rule loaders.
