...
```text
~/.modular/agent/sessions/<dir-encoded>/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.modular/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.modular/agent/terminal-sessions/<terminal-id>
```
...
- `listAllSessions(storage)` (a.k.a. `SessionManager.listAll()`) -> sessions across all project scopes under `~/.modular/agent/sessions`
