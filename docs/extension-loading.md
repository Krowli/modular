...
- Project directory: `<cwd>/.modular/extensions`
- User directory: `~/.modular/agent/extensions`
- Native legacy/settings JSON entries: `<cwd>/.modular/settings.json#extensions` and `~/.modular/agent/settings.json#extensions`

The project root is the native provider's `.modular` directory (`SOURCE_PATHS.native.projectDir`), cwd-only; it does not walk ancestors. The user root is the active profile's agent directory via `getAgentDir()`, so under `modular --profile <name>` it becomes `~/.modular/profiles/<name>/agent/extensions` (and it honors `PI_CODING_AGENT_DIR`). See [Profiles](./config-usage.md#profiles).

Notes:

- Native auto-discovery is currently `.modular` based.
...
Plugin extension entries come from package `modular.extensions` / `pi.extensions` manifests, including enabled feature entries.
...
- User: `~/.modular/agent/config.yml` (or custom agent dir via `PI_CODING_AGENT_DIR`)
- Project/native settings capability: `<cwd>/.modular/config.yml` and `<cwd>/.modular/settings.json`

Native extension-module discovery also reads legacy JSON extension lists from:

- `~/.modular/agent/settings.json`
- `<cwd>/.modular/settings.json`
...
```json
{
  "extensions": ["./.modular/extensions/my-extra"]
}
```
...
1. `package.json` in that directory with `modular.extensions` (or legacy `pi.extensions`) -> use declared entries
2. `index.ts`
3. `index.js`
4. Otherwise scan one level for extension entries:
   - direct `*.ts` / `*.js`
   - subdir `index.ts` / `index.js`
   - subdir `package.json` with `modular.extensions` / `pi.extensions`
...
- a scoped Bun `onLoad` hook rewrites legacy pi-package specifiers (`@modular/*`, `@modular/*`) and bare `@sinclair/typebox` onto the host-bundled copies before evaluation
