...
1. Capability providers (`toolCapability`), including:
   - Native MODULAR config (`~/.modular/agent/tools`, `.modular/tools`)
   - Claude config (`~/.claude/tools`, `.claude/tools`)
   - Codex config (`~/.codex/tools`, `.codex/tools`)
   - Claude marketplace plugin cache provider
2. Installed plugin manifests (`~/.modular/plugins/node_modules/*` via plugin loader)
...
```ts
import type { CustomToolFactory } from "@modular/pi-coding-agent";
...
- `pi`: injected `@modular/pi-coding-agent` exports
