# LSP configuration in MODULAR

This guide explains how to configure language servers for the MODULAR coding agent.
...
When no LSP config file is present, MODULAR auto-detects servers by intersecting two conditions:
...
MODULAR merges LSP config from multiple files, lowest to highest priority:

| Priority    | Location                                                                                                                    |
| ----------- | --------------------------------------------------------------------------------------------------------------------------- |
| 5 (lowest)  | `~/lsp.json`, `~/.lsp.json`, `~/lsp.yaml`, `~/.lsp.yaml`, `~/lsp.yml`, `~/.lsp.yml`                                         |
| 4           | Plugin LSP configs (marketplace / `--plugin-dir` roots)                                                                     |
| 3           | User config dirs: `~/.modular/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.modular/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.modular/agent/lsp.json`
- Project-specific overrides → `<project>/.modular/lsp.json`

> **Note:** Auto-detection is skipped only when at least one config file contributes server overrides. A config file that only sets `idleTimeoutMs` still lets MODULAR auto-detect built-in servers. When server overrides exist, MODULAR merges them with defaults and then loads servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
The `capabilities` object enables optional server-specific features that MODULAR supports on a per-server basis:
...
Place the override in `<project>/.modular/lsp.json`:
...
The user-level config in `~/.modular/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
