...
```text
Config sources (.modular/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.modular/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.modular/mcp.json` (project) or `~/.modular/agent/mcp.json` (user) for explicit control.
...
MODULAR-native MCP config (`.modular/mcp.json`, `~/.modular/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
