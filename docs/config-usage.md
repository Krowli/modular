...
┌───────────────────────────────────────┐
│ 1) ~/.modular/agent, ~/.claude, ...       │
│ 2) <cwd>/.modular, <cwd>/.claude, ...     │
└───────────────────────────────────────┘
                    │
                    ▼
        capability providers enumerate items
 (native provider scans project .modular before user .modular;
...
`CONFIG_DIR_NAME` is `.modular` (`packages/utils/src/dirs.ts`).

## Profiles

A named profile (`modular --profile <name>`, the `--alias` shortcut, or `MODULAR_PROFILE` / `PI_PROFILE`) relocates the MODULAR user base. When a profile is active, every MODULAR-native user-level path written here as `~/.modular/agent/...` resolves to `~/.modular/profiles/<name>/agent/...` instead.

The relocation is uniform across the native provider (`builtin.ts`) and the generic `config.ts` helpers, so it covers slash commands, rules, prompts, instructions, hooks, tools, extensions, settings, skills, and MCP, plus the top-level `SYSTEM.md` / `RULES.md` / `AGENTS.md` files and runtime state (sessions, blobs, `agent.db`). A profile sees only its own MODULAR config, never the default profile's `~/.modular/agent`.

Keybindings are the one exception: a named profile merges the default profile's `~/.modular/agent/keybindings.*` under its own `~/.modular/profiles/<name>/agent/keybindings.*`, with the profile file overriding per binding ([#4867](https://github.com/Krowli/modular/issues/4867)). Keybindings describe the terminal/keyboard in front of the user, which doesn't change with the active profile, so user-level remaps keep working in every profile unless the profile explicitly overrides them. The inherited file is read-only for the profile process — legacy-format migration of the default profile's file only happens when the default profile itself runs.

The other source bases are not profile-scoped and load identically under every profile: the external-tool bases (`~/.claude`, `~/.codex`, `~/.gemini`) belong to those tools, and the project-level bases (`<cwd>/.modular`, `<cwd>/.claude`, ...) are keyed to the working directory. Throughout this document, read `~/.modular/agent` as shorthand for the active profile's agent directory.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.modular`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: `~/.modular/agent/config.yml`
2. Project settings: discovered via settings capability (`settings.json` and `config.yml` from providers)
3. CLI config overlays: `modular --config <path>` / repeated `--config` files, loaded as `config.yml`-style YAML for this process only
...
1. Migrate from `~/.modular/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.modular` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- Skills scan `<ancestor>/.modular/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.modular/agent/skills`, without requiring the root `.modular` directory itself to be non-empty.
- `SYSTEM.md` and `AGENTS.md` read user-level files directly and use nearest-ancestor project `.modular` lookup for project files, but the project `.modular` directory must be non-empty. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract (replace vs. append, templating).

### Scope-specific loading

- Skills: `<ancestor>/.modular/skills/*/SKILL.md` and `~/.modular/agent/skills/*/SKILL.md`
...
## For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.modular` directory search (walk-up) and still requires the project `.modular` dir to be non-empty.
...
```text
# ~/.modular/agent/TITLE_SYSTEM.md
Generate a session name using lowercase `<type>:<primary-objective>`.
```

- Missing `TITLE_SYSTEM.md` keeps the bundled title prompts.
- Discovery uses the same project-then-user config directory pattern as `SYSTEM.md`: project `.modular/TITLE_SYSTEM.md` first, then user `~/.modular/agent/TITLE_SYSTEM.md` and the other supported config bases.
