...
This document covers the runtime loader shipped by `@modular/pi-natives`: how `native/index.js` decides which `.node` file to require, how compiled-binary embedded payloads are extracted, and what startup failures report.
...
- **Core directories**:
  - `leafPackageDir`: directory of the platform leaf package, resolved via `require.resolve("@modular/pi-natives-<tag>/package.json")`; `null` when no leaf is installed (e.g. local dev) and forced to `null` in compiled-binary mode.
  - `nativeDir`: package-local `packages/natives/native`.
  - `execDir`: directory containing `process.execPath`.
  - `versionedDir`: `<getNativesDir()>/<packageVersion>`.
  - `userDataDir` fallback:
    - Windows: `%LOCALAPPDATA%/modular` or `%USERPROFILE%/AppData/Local/modular`.
    - Non-Windows: `~/.local/bin`.
- **Natives cache root** (`getNativesDir()`):
  - if `$XDG_DATA_HOME/modular` exists, `$XDG_DATA_HOME/modular/natives`;
  - otherwise `~/.modular/natives`.
...
- release sentinel mismatch details when a loadable `.node` belongs to another `@modular/pi-natives` version.

### Non-compiled startup failures

Normal package/runtime diagnostics include:

- reinstall hint (`bun install @modular/pi-natives`),
