...
The SDK is the in-process integration surface for `@modular/pi-coding-agent`.
...
```bash
bun add @modular/pi-coding-agent
```

## Entry points

`@modular/pi-coding-agent` exports the SDK APIs from the package root (and also via `@modular/pi-coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@modular/pi-coding-agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.modular/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@modular/pi-coding-agent";
...
```ts
import { SessionManager } from "@modular/pi-coding-agent";
