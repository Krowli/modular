...
A **marketplace** is a Git repository (or local directory) containing a catalog file at `.modular-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). The catalog lists available plugins with their sources, descriptions, and metadata.

A **plugin** is a directory containing Claude/MODULAR plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Extension modules (`package.json` `modular.extensions` entry points) are not loaded from marketplace installs — they only load for npm-installed or `modular plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.modular/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.modular/plugins/installed_plugins.json`
...
```
modular plugin marketplace add <source>
modular plugin marketplace remove <name>
modular plugin marketplace update [name]
modular plugin marketplace list
modular plugin discover [marketplace]
modular plugin install [--force] [--scope user|project] name@marketplace
modular plugin uninstall [--scope user|project] name@marketplace
modular plugin upgrade [--scope user|project] [name@marketplace]
modular plugin enable [--scope user|project] name@marketplace
modular plugin disable [--scope user|project] name@marketplace
```
...
Git and local sources must contain a catalog at `.modular-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.modular-plugin/marketplace.json` in the repository root. When modular is the only intended consumer, prefer this path. To remain Claude Code-compatible (modular loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — modular uses it as a fallback when `.modular-plugin/marketplace.json` is absent. A repository may ship both: modular reads the `.modular-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
