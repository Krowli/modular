...
This document explains how token/tool streaming is normalized in `@modular/pi-ai`, then propagated through `@modular/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@modular/pi-ai`
