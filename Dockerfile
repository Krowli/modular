...
#   wheel-builder   — modular_rpc Python wheel
#   pi-base         — python + bun + rustup launcher + natives + modular_rpc
#                     + /usr/local/bin/modular shim
#   pi-runtime      — pi-base + pi source + bun install      (DEFAULT, runnable)
#
# Build:
#     docker build -t modular/pi:dev .                          # default = pi-runtime
#     docker build --target pi-base -t modular/pi-base:dev .    # base for derived images
#
# Run:
#     docker run --rm modular/pi:dev --help
#     docker run --rm -it -v "$PWD":/work modular/pi:dev cli    # interactive modular
#
# Consume as a base in another Dockerfile (see Dockerfile.robodular):
#     ARG PI_BASE=modular/pi:dev
...
COPY --parents \
    ...
    python/robodular/web/package.json \
    crates/*/Cargo.toml \
    /pi/
...
############################
# 2) wheel-builder — modular-rpc wheel
############################
...
WORKDIR /src
COPY python/modular-rpc /src
RUN python -m build --wheel --outdir /out

############################
# 3) pi-base — python + bun + rustup + natives + modular_rpc + modular shim
#
# Sharable runtime base. Derived images (pi-runtime below, Dockerfile.robodular)
...
# modular-rpc Python wheel.
COPY --from=wheel-builder /out/*.whl /tmp/wheels/
RUN pip install /tmp/wheels/modular_rpc-*.whl && rm -rf /tmp/wheels

# `modular` shim — runs the coding-agent CLI against $PI_ROOT via Bun. Derived
...
#
# A self-contained, runnable modular image. `docker run modular/pi:dev --help`
...
ENTRYPOINT ["/usr/bin/tini", "--", "/usr/local/bin/modular"]
CMD ["--help"]
