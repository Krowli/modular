...
Ownership: cache root is provisioned ``root:modular 02770`` by ``entrypoint.sh``
so slot subprocesses (group ``modular``) can capture under setgid inheritance.
...
def _git_safe_directory_env(repo_dir: Path) -> dict[str, str]:
    ...
    ``robodular.sandbox._safe_directory_env`` but kept local to avoid a circular
        ...
        *,
        ...
        with _flock(self.lockfile(repo)):
            ...
            try:
                ...
                # the setgid `modular` model that lets other slots read it.
                # A copy creates a fresh inode owned by the orchestrator
                # (root) and inherits gid `modular` from the setgid 2770
                # cache root.
