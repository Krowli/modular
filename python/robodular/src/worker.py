...
from robodular import host_tools, persona, pragmas
from robodular.cancellation import register_cancel_hook, unregister_cancel_hook
from robodular.config import Settings
from robodular.db import Database, issue_key
from robodular.git_ops import DirtyState, inspect_dirty_state
from robodular.github_backend import GitHubBackend
from robodular.github_client import CommentInfo, IssueInfo, PullRequestInfo, RepoInfo
from robodular.host_tools import AbortController, ToolBindings, _git_identity_env
from robodular.natives_cache import NativesCache
from robodular.natives_cache import compute_key as natives_compute_key
from robodular.sandbox import GitTransport, Workspace, _prepare_slot_runtime_env, _safe_directory_env
...
_SCRUBBED_ENV_KEYS: tuple[str, ...] = (
    ...
    "ROBODULAR_REPLAY_TOKEN",
    "ROBODULAR_GH_PROXY_HMAC_KEY",
)
...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".modular/agent")):
    ...
    for root, dirs, files in os.walk(_AGENT_HOME):
        root_path = Path(root)
        if root_path == _AGENT_HOME / ".modular":
            # ~/.modular/run is slot-writable daemon presence state, not template
...
def _ensure_agent_run_dir() -> None:
    """Keep ``~/.modular/run`` writable by every sandbox slot.

    modular registers daemon project presence under ``~/.modular/run`` at startup,
    ...
    The tree stays group ``modular``, setgid, group-writable; slot subprocesses
    spawn with umask 0002 so their entries inherit group write.
    """
    if os.geteuid() != 0:
        return
    run_dir = _AGENT_HOME / ".modular" / "run"
    try:
        gid = grp.getgrnam("modular").gr_gid
    except KeyError:
        return
    try:
...
def _build_extra_env(settings: Settings) -> dict[str, str]:
    """Build the env overlay passed to the modular subprocess.

    `modular_rpc` merges this dict on top of `os.environ`, so overlaying empty
...
def _has_prior_session(session_dir: Path) -> bool:
    """Return True iff `session_dir` already contains an modular JSONL transcript.
...
def _run_rpc_blocking(
    ...
    *,
    ...
    # can resolve workspace packages (@modular/pi-*) and actually run tests.
    ...
    with RpcClient(
        executable=settings.modular_command,
        ...
        extra_groups=["modular"] if inputs.slot_uid is not None else None,
    ) as client:
        # Arm cancellation: from this point the API can kill the modular subprocess
        ...
        # it's idempotent.
        #
        # NOTE: modular_rpc.RpcClient.stop() has a bug where it sets `_stopping=True`
        ...
        try:
            ...
            if hard_timeout_fired.is_set():
                raise TimeoutError("modular task exceeded hard timeout")
            if turn is not None and turn.assistant_message is not None:
                ...
                if stop_reason == "error":
                    ...
                    raise RuntimeError(
                        f"modular agent error (stopReason=error): {error_msg}"
                    )
            log.info(
                "rpc_done",
                extra={
