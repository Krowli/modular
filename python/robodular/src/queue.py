...
from robodular.cancellation import clear_current_event, set_current_event
from robodular.config import Settings
from robodular.db import Database, EventRow
from robodular.github_backend import GitHubBackend
from robodular.sandbox import GitTransport, SandboxManager, _reap_slot
from robodular.slot_pool import SlotPool
...
class WorkerPool:
    ...
    async def start(self) -> None:
        ...
        self._workers.append(asyncio.create_task(self._dispatch_loop(), name="robodular-dispatch"))
        ...
        if self.sandbox.natives_cache is not None and self.settings.natives_cache_gc_interval_seconds > 0:
            self._workers.append(asyncio.create_task(self._natives_cache_gc_loop(), name="robodular-natives-gc"))
    ...
    async def stop(self, *, drain_timeout: float = 25.0, kill_timeout: float = 5.0) -> None:
        ...
        `reset_stuck_running()`. The resumed modular session then picks up via
        ...
        #    if one was registered (kills the modular subprocess); otherwise
        ...
        for task in still_running:
            ...
            if hook is not None:
                ...
                continue
            # No hook armed yet — the worker hasn't reached the modular spawn
    ...
    async def _dispatch_loop(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                task = asyncio.create_task(self._run_event(row), name=f"robodular-event-{row.delivery_id[:8]}")
    ...
    async def cancel_event(self, delivery_id: str) -> bool:
        ...
        # doesn't stall the event loop for up to the modular shutdown grace period.
        try:
            await asyncio.to_thread(hook)
        except Exception:
    ...
    async def _run_event(self, row: EventRow) -> None:
        ...
        try:
            ...
            if row.delivery_id in self._shutdown_cancelled:
                ...
                # resumed modular session picks up via `--continue`.
