"""Host tools exposed to the agent through `modular_rpc.host_tool`.
...
from modular_rpc import HostTool, HostToolContext, RpcCommandError, host_tool

from robodular import persona
from robodular.config import Settings
from robodular.db import Database, IssueState, issue_key
from robodular.git_ops import GitCommandError, HeadDriftError
from robodular.github_backend import GitHubBackend
from robodular.github_client import GitHubError, IssueInfo, PullRequestFileInfo, RepoInfo
...
class AbortController:
    ...
    teardown of the modular subprocess. The worker pre-populates `stop` with a
...
class ToolBindings:
    ...
    # Set by the worker before launching modular. Carries the abort-task signal
...
def ensure_workspace_dependencies(bindings: ToolBindings) -> None:
    ...
    (``@modular/pi-*``) only exist after an install, so without one any
...
def _repair_commit_message_escapes(bindings: ToolBindings, args: Mapping[str, Any], *, tool_name: str) -> None:
    ...
    update = _run_repo_command(
        bindings,
        ["git", "update-ref", "-m", "robodular: repaired commit message escapes", "HEAD", new_head, old_head],
    )
...
def _build_abort_task(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        # races against the imminent modular teardown.
