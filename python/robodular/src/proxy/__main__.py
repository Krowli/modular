"""`python -m robodular.proxy serve` — run the gh-proxy FastAPI app."""
...
from robodular.config import Settings, load_proxy_settings
...
def _settings_or_die() -> Settings:
    ...
    `ROBODULAR_GH_PROXY_HMAC_KEY` — the orchestrator's webhook secret,
...
def serve() -> None:
    ...
    if cfg.gh_proxy_hmac_key is None:
        click.echo("gh-proxy: ROBODULAR_GH_PROXY_HMAC_KEY is required in proxy mode", err=True)
        sys.exit(2)
