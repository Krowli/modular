...
PAT injection via `--config-env` (see `robodular.git_ops`); the `ProxyGitTransport`
in `robodular.proxy_client` forwards the same set of operations over HMAC RPC.
...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.modular-session/`, `context/`, `artifacts/`, `.modular-tmp/`, `.modular-xdg`):
   single-owner. Owned by the active slot UID/GID (`modular-N`) when slot
...
2. **Clone pool** (`/data/workspaces/_pool/<owner>__<repo>/`): genuinely
   multi-slot. Owned by `root:modular` (gid 2000) with setgid `02770`; cross-slot
...
3. **Language tool caches** (`/data/cache/{cargo,cargo-target,rustup,bun-cache}`):
   multi-slot. Owned by `root:modular` with setgid `02770`; provisioned by
   `entrypoint.sh`.
...
from robodular.natives_cache import CacheHit, NativesCache
...
class GitTransport(Protocol):
    ...
    - `robodular.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.modular-tmp`` (symlink-protection: a user
    ...
    tmpdir = workspace.root / ".modular-tmp"
    try:
        st = tmpdir.lstat()
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    and are group-shared via ``modular``. Bun's install cache is explicitly
    ...
    xdg_root = workspace.root / ".modular-xdg"
    ...
    for base in (xdg_data, xdg_state, xdg_cache):
        ...
        (base / "modular").mkdir(parents=True, exist_ok=True)
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.modular-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.modular-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".modular-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
...
def _share_git_metadata_with_slots(repo_dir: Path, slot_uid: int | None) -> None:
    ...
    `modular-N` user, so the pool-side worktree gitdir, refs, reflogs, and object
    directories must stay writable through the shared `modular` group.
    """
    if not _slot_permissions_active(slot_uid):
        return
...
class SandboxManager:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        with self._repo_lock(repo):
            ...
            session_dir = ws_root / ".modular-session"
            ...
            # keeps its `root:modular` ownership (the slot reads through group `modular`);
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        ...
        `.node` files are LEFT at `root:modular` ownership — chowning them
        would chown the cache file too (shared inode), breaking the
        cross-slot sharing model. The slot reads them via group `modular`.
        """
    ...
    def _chown_natives_for_slot(native_dir: Path, hit: CacheHit, *, slot_uid: int) -> None:
        ...
        hardlinks back into the root:modular cache and the slot reads them via
        group `modular`. Everything else (the directory itself, copied
