...
#
# Used by both the orchestrator (CMD: `python -m robodular serve`) and the
# sibling gh-proxy (compose command: `python -m robodular.proxy serve`). The
# proxy role does NOT need a $PI_ROOT pi checkout — it never runs modular.
set -euo pipefail

# Shared git metadata under /data/workspaces/_pool is intentionally group
# writable by the `modular` group so interrupted work can resume on a different
...
# as $@ here (after tini --), so $1=python, $2=-m, $3=robodular.proxy is the
# canonical shape; we also accept a single concatenated arg for safety.
is_proxy_role=0
if [ "${1:-}" = "python" ] && [ "${2:-}" = "-m" ] && [[ "${3:-}" == robodular.proxy* ]]; then
    is_proxy_role=1
elif [[ "${1:-}" == *"robodular.proxy"* ]]; then
    is_proxy_role=1
fi

/usr/sbin/groupadd -f -g 2000 modular
max_slots="${ROBODULAR_MAX_CONCURRENCY:-8}"
for i in $(seq 1 "$max_slots"); do
    user="modular-$i"
    slot_group="modular-$i"
    slot_id=$((2000 + i))
    /usr/sbin/groupadd -f -g "$slot_id" "$slot_group"
    id -u "$user" >/dev/null 2>&1 || /usr/sbin/useradd -u "$slot_id" -g "$slot_group" -G modular -M -N -s /usr/sbin/nologin "$user"
    /usr/sbin/usermod -g "$slot_group" -a -G modular "$user"
done
...
chown -R root:modular /data/cache /data/workspaces/_pool
...
rm -rf /srv/agent-home/.agent /srv/agent-home/.modular/agent
mkdir -p /srv/agent-home/.agent /srv/agent-home/.modular/agent
if [ -e /srv/agent-home-stage/.agent ]; then
    cp -a /srv/agent-home-stage/.agent/. /srv/agent-home/.agent/
fi
if [ -e /srv/agent-home-stage/.modular/agent ]; then
    cp -a /srv/agent-home-stage/.modular/agent/. /srv/agent-home/.modular/agent/
fi
...
# modular registers daemon project presence under ~/.modular/run at startup, nesting
...
# modular keeps the whole tree group-writable (entrypoint umask 0002 carries into
# slot processes, so new entries stay group-writable too).
mkdir -p /srv/agent-home/.modular/run
chgrp -R modular /srv/agent-home/.modular/run
chmod -R g+rwX /srv/agent-home/.modular/run
find /srv/agent-home/.modular/run -type d -exec chmod g+s {} +
chmod 2770 /srv/agent-home/.modular/run

touch /data/robodular.sqlite
chown root:root /data/robodular.sqlite
chmod 0600 /data/robodular.sqlite
for db_file in /data/robodular.sqlite-wal /data/robodular.sqlite-shm; do
    if [ -e "$db_file" ]; then
