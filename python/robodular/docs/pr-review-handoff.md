...
Wire robodular to **review pull requests opened by contributors** (and other bots), in two
phases: (1) classify + rank, (2) a real line-by-line review posted as one GitHub review.
robodular **never merges, closes, approves, or pushes** — the rank label is the verdict; the
       ...
       └─ WorkerPool._dispatch(row)   re-derives handler from (event_type, action)
            └─ tasks.<entry>(settings, db, github, sandbox, git_transport, payload, delivery_id, …)
                 ...
                 └─ worker.run_task(task_kind=..., inputs=TaskInputs, …)
                      ...
                      ├─ RpcClient(modular --mode rpc, cwd=worktree, custom_tools=host_tools.build(bindings))
...
Add `pr_review_enabled: bool = True` (`ROBODULAR_PR_REVIEW_ENABLED`) so the whole flow can be
...
Do **not** enable the integration smoke (`ROBODULAR_INTEGRATION=1`) in the default suite.

---

## 14. Open questions for @modular
