...
from robodular.worker import DirectiveInfo, ThreadMessage
...
def test_directive_prompt_embeds_thread_and_directive_body() -> None:
    ...
    out = persona.directive(
        ...
        directive=DirectiveInfo(body="apply fix Y", author="modular", thread=thread),
...
def test_followup_comment_prompt_embeds_thread_context() -> None:
    thread = (
        ...
        ThreadMessage(kind="comment", author="modular", body="prior request", created_at="2026-05-01T10:00:00Z"),
    )
...
def test_kickoff_directive_prompt_embeds_thread_and_classify_instruction() -> None:
    ...
    out = persona.kickoff_directive(
        ...
        issue=_Issue(),
        workspace=_Workspace(),
        directive=DirectiveInfo(body="reproduce + fix", author="modular", thread=thread),
    )
