...
from robodular.github_events import (
    extract_mention,
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@robodular-bot please re-review",
            },
            "issue": {"number": 9, "user": {"login": "contributor"}, "pull_request": {"url": "x"}},
            "repository": {"full_name": "octo/widget"},
        },
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @robodular-bot please look", "robodular-bot") == "hey please look"
    assert extract_mention("@robodular-bot do X", "robodular-bot") == "do X"
...
def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "robodular-bot") is None
    assert extract_mention(None, "robodular-bot") is None
    assert extract_mention("", "robodular-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @ROBODULAR-BOT", "robodular-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @robodular-bot-helper must NOT match @robodular-bot.
    assert extract_mention("@robodular-bot-helper hi", "robodular-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@robodular-bot one, then @robodular-bot two", "robodular-bot") == "one, then two"


def test_is_maintainer_recognizes_explicit_allowlist() -> None:
    assert is_maintainer("modular", None, maintainers=frozenset({"modular"}))
    assert is_maintainer("modular", "NONE", maintainers=frozenset({"modular"}))
...
def test_is_implementation_authorizer_accepts_allowlist_and_owner() -> None:
    assert is_implementation_authorizer("modular", None, maintainers=frozenset({"modular"}))
    assert is_implementation_authorizer("modular", "NONE", maintainers=frozenset({"modular"}))
...
def test_route_directive_set_on_issue_comment_when_owner_mentions_bot() -> None:
    ...
    assert decision.directive_author == "modular"
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@robodular-bot go ahead and push",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "modular/widget", "owner": {"login": "modular", "type": "User"}},
        },
        allowlist=frozenset({"modular/widget"}),
        bot_login=BOT,
    )
    assert decision.directive is True
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@robodular-bot go ahead with the plan",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "modular"},
                "author_association": "OWNER",
                "body": "@robodular-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "modular"},
                "author_association": "OWNER",
                "body": "@robodular-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "modular"},
                "author_association": "OWNER",
                "body": "@robodular-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
