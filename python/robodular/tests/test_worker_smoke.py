...
Runs only when ROBODULAR_INTEGRATION=1 and `modular` is available on PATH (or via
ROBODULAR_OMP_COMMAND). Spins up:

- a local bare git repo with a trivial failing test,
- a fake GitHub API via httpx.MockTransport that records comments + PRs,
- a real `modular --mode rpc` subprocess driven by `worker.run_task`.
...
INTEGRATION = os.environ.get("ROBODULAR_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBODULAR_INTEGRATION=1 required to run the modular-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from robodular.sandbox import LocalGitTransport, SandboxManager
    from robodular.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("ROBODULAR_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("ROBODULAR_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("ROBODULAR_BOT_LOGIN", "robodular-bot")
    monkeypatch.setenv("ROBODULAR_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("ROBODULAR_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("ROBODULAR_SQLITE_PATH", str(tmp_path / "robodular.sqlite"))
    monkeypatch.setenv("ROBODULAR_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("ROBODULAR_TASK_TIMEOUT_SECONDS", "300")
