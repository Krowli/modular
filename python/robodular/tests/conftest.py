...
from robodular.dashboard import reset_index_cache, static_dir
from robodular.db import Database, close_database

# Minimum HTML the dashboard handler needs to render: `<title>` plus a script
# block carrying the `__ROBODULAR_CONFIG__` sentinel. The real Vite-built bundle
# adds JS/CSS asset links; tests only care about the rendering contract.
_PLACEHOLDER_INDEX_HTML = (
    "<!doctype html>\n"
    '<html lang="en">\n'
    '  <head><meta charset="utf-8"><title>robodular</title></head>\n'
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="robodular-config" type="application/json">__ROBODULAR_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "ROBODULAR_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "ROBODULAR_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "ROBODULAR_BOT_LOGIN": "robodular-bot",
        "ROBODULAR_GIT_AUTHOR_NAME": "robodular-bot",
        "ROBODULAR_GIT_AUTHOR_EMAIL": "robodular-bot@example.invalid",
        "ROBODULAR_REPO_ALLOWLIST": "octo/widget",
        "ROBODULAR_MODEL": "anthropic/claude-sonnet-4-5",
        "ROBODULAR_THINKING": "high",
        "ROBODULAR_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "ROBODULAR_SQLITE_PATH": str(tmp_path / "robodular.sqlite"),
        "ROBODULAR_LOG_DIR": str(tmp_path / "logs"),
        ...
        # cache flip `ROBODULAR_NATIVES_CACHE_ENABLED=true` explicitly.
        "ROBODULAR_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "ROBODULAR_NATIVES_CACHE_ENABLED": "false",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("ROBODULAR_PROVIDER", raising=False)
    monkeypatch.setenv("ROBODULAR_REPLAY_TOKEN", "")
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("ROBODULAR_GH_PROXY_URL", None)
    baseline.pop("ROBODULAR_GH_PROXY_HMAC_KEY", None)
    ...
    monkeypatch.setenv("ROBODULAR_GH_PROXY_URL", "")
    monkeypatch.setenv("ROBODULAR_GH_PROXY_HMAC_KEY", "")
    monkeypatch.delenv("ROBODULAR_PROVIDER", raising=False)
    monkeypatch.setenv("ROBODULAR_REPLAY_TOKEN", "")
    reset_settings_cache()
    yield baseline
