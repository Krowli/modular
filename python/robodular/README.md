...
Self-hosted GitHub triage bot. Drives [`modular --mode rpc`](https://github.com/Krowli/modular)
...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `ROBODULAR_QUESTION_AUTOCLOSE_HOURS`
...
Follow-up issue comments and PR review comments resume the same modular session
...
- **robodular** — FastAPI + sqlite event queue + `WorkerPool` running `modular` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from robodular, executes REST + `git push`.
...
→ `worker.run_task` spawns `modular --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `ROBODULAR_MODEL` (CSV).

The agent uses modular's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.modular/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host modular doesn't route through the gateway). roboomp lives inside the modular
monorepo at `python/robodular/`; both the docker build context and the
`/work/pi` bind mount default to the parent monorepo (`../..`). Override
`PI_ROOT` only if you want a different modular checkout backing the build
and runtime.

Bot account needs **Write** on every repo in `ROBODULAR_REPO_ALLOWLIST`. A
...
cp .env.example .env
$EDITOR .env
openssl rand -hex 32              # ROBODULAR_GH_PROXY_HMAC_KEY
openssl rand -hex 32              # GITHUB_WEBHOOK_SECRET

bun run pi:image                  # build modular/pi:dev (one-time / on pi change)
bun run robodular:build && bun run robodular:up
...
comment out `ROBODULAR_GH_PROXY_URL` / `ROBODULAR_GH_PROXY_HMAC_KEY` and set
...
runtime layer; editing pi source rebuilds `modular/pi:dev`, which
roboomp's `Dockerfile.robodular` extends via `FROM ${PI_BASE}`.
...
The container entrypoint is `python -m robodular serve`. Other commands run
inside the running container:

```bash
docker compose exec robodular robodular triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec robodular robodular replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec robodular robodular status                   # dump issues table
docker compose exec robodular robodular cleanup owner/repo#123   # force workspace removal, state=abandoned
```

`bun run robodular:…` shortcuts in the root `package.json` cover the common
lifecycle commands (`robodular:dev`, `robodular:build`, `robodular:up`, `robodular:down`,
`robodular:logs`, `robodular:restart`, `robodular:reset`).

## Tests

```bash
pytest -x tests/                              # unit suite, no network
ROBODULAR_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```

The integration test spawns a real `modular --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `modular` on
...
- gh-proxy has no host port. The `robodular_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `ROBODULAR_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `ROBODULAR_GIT_AUTHOR_NAME` +
  `ROBODULAR_GIT_AUTHOR_EMAIL`. Commit messages carrying shell-literal
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `ROBODULAR_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `ROBODULAR_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
  `stop_grace_period: 30s` covers both.
- **Logs.** Structured JSON on stdout, rotated to
  `/data/logs/robodular.log.jsonl`.
...
| Container exits with `PI_ROOT … missing` | `/work/pi` mount empty inside the container; on the host either run `docker compose` from `python/robodular/` so `PI_ROOT` defaults to `../..`, or export `PI_ROOT` to a valid modular checkout. |
| `git push: Authentication required` | Bot PAT lacks push, or `ROBODULAR_BOT_LOGIN` does not identify the PAT account's mention handle (production: `roboomp`, no `@`/`[bot]`). |
| `refusing to push: commit author identity mismatch` | Some commit not authored as `ROBODULAR_GIT_AUTHOR_*`. The error lists the offending shas; `git commit --amend --reset-author --no-edit`. |
...
| `Failed to load pi_natives` | Wrong arch / missing native. `bun run pi:image` then `bun run robodular:build`. |
| `No API key found for <provider>` | `~/.modular/agent/models.container.yml` mount missing or provider id mismatch with `ROBODULAR_MODEL`. |
...
src/
  ...
  worker.py          synchronous modular RPC driver, prompt assembly, env scrubbing
...
tests/               pytest unit suite + one ROBODULAR_INTEGRATION=1 smoke test
