...
`roboomp` is a self-hosted GitHub triage-and-fix bot that drives [`modular --mode rpc`](https://github.com/Krowli/modular) as a subprocess. On every issue opened in an allowlisted repository it classifies the issue, applies labels, then branches into one of: reproduce → fix → PR (`bug` / `documentation`), single-comment answer (`question`), single thoughtful comment (`enhancement` / `proposal`), or brief comment (`invalid` / `duplicate`). Follow-up comments and PR review comments resume the same modular session so the agent keeps its prior reasoning. If the orchestrator restarts mid-task, the dispatcher resumes the same session via `modular --continue` from the per-issue `session_dir`, so an interrupted task re-enters its prior reasoning instead of restarting from scratch. The orchestrator runs as a single FastAPI process inside Docker with SQLite-backed durable event state.

## Architecture & Data Flow

Webhook → durable queue → async dispatcher → per-issue git worktree → modular RPC subprocess + host tools.
...
4. `queue.WorkerPool._dispatch_loop` atomically claims `state='queued'` rows under `BEGIN IMMEDIATE`, guarded by an in-process `_inflight` set keyed by `(owner, repo, number)` to serialize per-issue work. Cap: `ROBODULAR_MAX_CONCURRENCY` (default 8).
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `modular --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked model from `ROBODULAR_MODEL` (CSV pool). When `<session_dir>/*.jsonl` already exists the worker passes `--continue`, so both follow-up events and crash-restarted events resume the same session.
7. Inside the subprocess the agent uses **built-in modular tools** (read/edit/write/bash/lsp, scoped to the worktree) and **host tools** from `host_tools.py` (the only surface allowed to mutate GitHub or write audit rows).
...
- `tests/` — pytest suite. `test_worker_smoke.py` is gated on `ROBODULAR_INTEGRATION=1`.
- `data/` — runtime state (sqlite + WAL, `workspaces/`, `logs/`). Never committed.
- `/Dockerfile` (pi root) — produces `modular/pi:dev` (pi runtime image: python + bun + rustup + pi-natives + modular_rpc + `/usr/local/bin/modular` shim + the full pi source under `/pi`). Stages: `natives-builder` → `wheel-builder` → `pi-base` → `pi-runtime` (default). Built via `bun run pi:image`. Robodular's image extends `pi-base` via `FROM ${PI_BASE}` in `/Dockerfile.robodular`.

## Development Commands

Task runner is `bun` against the **monorepo root** `package.json`. roboomp itself no longer ships a `package.json`; every recipe lives at the root under the `robodular:*` namespace. Local venv (no docker): `bun run robodular:install` runs `pip install -e 'python/robodular[dev]'`. From there:

```
bun run test:py                   # pytest -x python/modular-rpc/tests python/robodular/tests
bun run robodular:test:integration   # ROBODULAR_INTEGRATION=1, requires modular on PATH
bun run robodular:serve              # python -m robodular serve on the host
```

Docker inner loop:

```
bun run pi:image                  # build modular/pi:dev (one-time / on pi change)
bun run pi:run                    # docker run -it modular/pi:dev (smoke-test the shim)
bun run robodular:build              # pi:image (if pi changed) + docker compose build
bun run robodular:dev                # build + up -d + follow logs
bun run robodular:up / robodular:down / robodular:restart / robodular:logs
bun run robodular:rebuild            # docker compose build --no-cache
bun run robodular:reset              # `down -v` + drop the pi image
```

Frontend (Vite + SolidJS, in `web/` — still a bun workspace):

```
bun run robodular:web:dev            # vite dev server with proxy to :8080
bun run robodular:web:build          # produce src/static/ bundle
bun --cwd=python/robodular/web run typecheck   # tsc --noEmit
```

In-container CLI (`robodular` console script → `robodular.cli:main`): no root aliases — invoke directly:

```
docker compose --project-directory python/robodular exec robodular robodular triage owner/repo#N
docker compose --project-directory python/robodular exec robodular robodular replay <delivery_id>
docker compose --project-directory python/robodular exec robodular robodular status
docker compose --project-directory python/robodular exec robodular robodular cleanup owner/repo#N
```

HTTP / sqlite / webhook inspection is unaliased — use `curl http://localhost:${ROBODULAR_BIND_PORT:-8080}/{healthz,readyz,events,issues}` and `docker compose --project-directory python/robodular exec robodular sqlite3 /data/robodular.sqlite` directly.
...
- **Async style**: FastAPI handlers and `queue.WorkerPool` are async. `worker.run_task` is **synchronous** and runs in a worker thread because `modular-rpc` is blocking — keep it that way; don't try to async it. CLI commands wrap with `asyncio.run`.
- **Config**: `pydantic-settings` `Settings` in `config.py` with `ROBODULAR_*` env prefix (e.g. `ROBODULAR_MAX_CONCURRENCY`, `ROBODULAR_REPO_ALLOWLIST`). Access only via `get_settings()` (`@cache` singleton). Tests must call `reset_settings_cache()` after mutating env.
...
- `src/worker.py` — synchronous modular RPC driver, prompt assembly via `persona`.
...
- `/Dockerfile.robodular` (pi root) — robodular's image. `FROM ${PI_BASE}` (default `modular/pi:dev`), adds the SolidJS dashboard bundle, the robodular Python package, and the `robodular-entrypoint` shim. Tini entrypoint, exposes `8080`, `VOLUME /data`. The toolchain (python + bun + rustup + pi-natives + modular_rpc + `modular` shim) comes from `pi-base` — no duplication in this file.
- `docker-compose.yml` — `build.args.PI_BASE`, mounts `$PI_ROOT:/work/pi:ro`, `./data:/data`, `~/.modular/agent/models.container.yml:ro` (mapped to `models.yml` inside the container — kept separate from the host's `~/.modular/agent/models.yml` so the host modular doesn't pick up gateway routing intended only for the container), `extra_hosts: llm-gateway.internal:host-gateway`.
...
- **Container runtime**: Docker Compose v2. The image embeds Bun 1.3.14 + a rustup launcher and exposes `modular` via a `/usr/local/bin/modular` shim; `ROBODULAR_OMP_COMMAND=modular` should not need changing.
- **Required env** (set in `.env`, see `.env.example`): `GITHUB_WEBHOOK_SECRET`, `ROBODULAR_BOT_LOGIN`, `ROBODULAR_GIT_AUTHOR_NAME`, `ROBODULAR_GIT_AUTHOR_EMAIL`, `ROBODULAR_REPO_ALLOWLIST`, plus model knobs (`ROBODULAR_MODEL`, `ROBODULAR_THINKING`, optional `ROBODULAR_PROVIDER`) and rate-limit / concurrency / timeout overrides. Set `ROBODULAR_BOT_LOGIN` to the lowercase mention handle (`roboomp` in production, no leading `@` or `[bot]`; config normalizes common variants). `ROBODULAR_MAINTAINER_LOGINS` is optional comma-separated bare logins (`@`/`[bot]` optional, case-insensitive) for non-owner implementation authorizers. **GitHub auth is mode-exclusive**: either set `ROBODULAR_GH_PROXY_URL` + `ROBODULAR_GH_PROXY_HMAC_KEY` (gh-proxy mode; PAT lives only in the sidecar container — the bundled compose default), or set `GITHUB_TOKEN` directly (single-process PAT mode). `Settings._validate_proxy_or_pat` rejects a `.env` that sets both.
- **PI_ROOT resolution**: roboomp lives inside the modular monorepo at `python/robodular/`. `bun run pi:image` builds the parent monorepo (`../..`) as its docker build context to produce `modular/pi:dev`; `docker-compose.yml` extends that image via `PI_BASE` and mounts the same parent path read-only at `/work/pi` for the orchestrator to see live source. Override `PI_ROOT` only when pointing the build/mount at a different modular checkout. Inside the container the path is always `/work/pi`. Build invalidation stays bounded: Python-only edits in roboomp never trigger a natives recompile.
...
- **Fixtures** (`tests/conftest.py`):
  - `env` — `monkeypatch`-sets all required `ROBODULAR_*` env vars and calls `reset_settings_cache()` before/after.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `ROBODULAR_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `modular` on `PATH`. Don't enable it in default `bun run test:py`.
