...
Typed Python bindings for the `modular --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `modular --mode rpc` flags such as thinking level,
...
```python
from modular_rpc import RpcClient
...
```python
from modular_rpc import MessageUpdateEvent, RpcClient
...
from modular_rpc import RpcClient, host_tool
...
```python
from modular_rpc import RpcClient, host_uri
...
```python
from modular_rpc import assistant_text, assistant_text_with_thinking
